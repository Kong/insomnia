...
- `pm.request.certificate` is a `Certificate` instance. Per `insomnia.ts`, it's initialized either as an empty placeholder certificate (`name: 'Default Certificate'`, no `key`/`cert`/`pfx`) when the request URL contains unrendered template tags or no client certificate in Settings matches the request's host, or as `{ name: 'The first matched certificate from Settings', matches: [matchedCertificates[0].host], key: {src: ...}, cert: {src: ...}, passphrase, pfx: {src: ...} }` when exactly one (the first) matching certificate is found via `filterClientCertificates`.
...
- **Empty/placeholder certificate is normal, not an error state.** If the request URL contains a template tag (e.g. `{{ baseUrl }}`) or there's no matching client certificate configured in Settings, `pm.request.certificate` is silently initialized to an empty certificate (a debug-level message is logged, not a warning/error) — scripts should not assume `key`/`cert`/`pfx` are populated.

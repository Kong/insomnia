...
1. **Konnect access is account-scoped** — read from `GET /v1/user/entitlements?feature=konnectControlPlanes`
   and cached in localStorage. (An earlier revision OR-ed the per-organization `konnectSync` feature
   flag across every organization; the entitlements endpoint replaced it.)
...
4. **Control Planes visibility** — shown if the account holds the `konnectControlPlanes_default`
   entitlement; also shown
...
- Organization list / user / plan — `useOrganizations()`, `useCurrentUser()`, `useCurrentPlan()` in
  `packages/insomnia/src/ui/hooks/use-account-server-data.ts`. `organization.tsx` no longer has a
  `clientLoader`; develop's account-data refactor replaced it with these TanStack Query hooks.
- Features — `useOrganizationPermissions()` in
  `packages/insomnia/src/ui/hooks/use-organization-features.tsx` (Scratchpad short-circuits to
  `fallbackFeatures`). The old `organization.$organizationId.permissions.tsx` route was deleted by
  the same refactor, and with it the `throw redirect('/organization')` hazard for unknown ids.
...
### Phase 1 — Account-scoped Konnect entitlement (_can run in parallel with early Phase 2_)
...
> window. Binding the lookup to it is wrong for a slow-changing permission bit.
>
...
> `entry.client.tsx` → pathname stays `/` → `routes/_index.tsx` → `redirect('/organization')`
> → `organization._index.tsx` `clientLoader`. The migration and the entitlement lookup both run in
> `entry.client.tsx` before hydration (steps 4 and 32), so neither depends on that route being the
> landing point.
>
> **Superseded — the flag is no longer an aggregation.** The first implementation OR-ed
> `features.konnectSync.enabled` across every organization in `${accountId}:spaces`, because the
> only source of truth was the per-organization features endpoint while the Konnect organization is
> account-wide. The backend now exposes `GET /v1/user/entitlements`, which is account-scoped by
> definition, so the N-request fan-out is gone and `features.konnectSync` was deleted from
> `FeatureList` / `fallbackFeatures` / the smoke mock server as dead weight.
>
> **Superseded again — no TanStack Query.** An intermediate version wrapped the whole thing in a
> `useServerQuery` keyed `['konnect-organization', accountId]`. That was wrong on three counts and
> was replaced by a module-level store (see "Why this is not a query" below).

3. Add `getUserEntitlements({ sessionId, feature })` to `packages/insomnia-api/src/user.ts`
   (`GET /v1/user/entitlements?feature=<name>`), plus the `Entitlement` type and the
   `KONNECT_CONTROL_PLANES_FEATURE` / `KONNECT_CONTROL_PLANES_FEATURE_KEY` constants.
   Response shape: `{ entitlements: Entitlement[] }`, where `featureKey` is
   `` `${featureName}_${entitlementKey}` ``, `type` is `'metered'`, `hasAccess` is the boolean this
   change consumes, and `allowance` is the quota (`null` = unlimited). **An account without the
   feature gets an empty array, not an entry with `hasAccess: false`** — so "missing" and "denied"
   must both resolve to `false`. `allowance` is deliberately ignored: today the only value shipped
   is `null`, and enforcing a control-plane cap would have to happen in `konnect/sync.ts`, which is
   out of scope.

   Then add `refreshKonnectAccess(sessionId, accountId, { force })` plus a module-level store to
   `packages/insomnia/src/ui/organization-utils.ts`. The store holds two booleans:
   - `hasEntitlement` — some entitlement has
     `featureKey === 'konnectControlPlanes_default' && hasAccess`. Mirrored to localStorage
     `${accountId}:konnectSyncEnabled` (a bare `'true'` / `'false'`) purely as the **offline
     fallback**: if the request throws, the last known answer is kept rather than hiding an
     organization the user owns. An empty `entitlements` array is a _successful_ response and does
     downgrade to `false`.
   - `isOrganizationVisible` — `hasEntitlement || localKonnectProjectCount > 0` (step 6).

   `refreshKonnectAccess` is a no-op when the account was already resolved, unless `force` is
   passed; there is no TTL and no request dedup beyond that, because it is called from a small
   fixed set of places rather than from a render path.

4. Call sites — four, all explicit:
   - `packages/insomnia/src/entry.client.tsx`, immediately after the migration (step 32) and before
     `hydrateRoot()`. This is what lets every reader downstream be **synchronous**; it also means
     the project count in step 6 sees post-migration parents.
   - `packages/insomnia/src/routes/organization._index.tsx` `clientLoader`. Signing in does not
     reload the renderer (`absorbKey` just writes session data and notifies the main process), so
     the startup resolution ran against the logged-out session. This loader is the post-login
     bootstrap — it already owns `syncOrganizations` — and the accountId guard makes it free on a
     cold start, where `entry.client.tsx` already resolved the same account.
   - the sidebar's `onDisconnect`, with `force: true` — deleting the last Konnect projects can hide
     the organization.
   - the migration conflict modal's `onDone`, with `force: true` — the migration can reveal it.

   Not call sites:
   - `packages/insomnia/src/routes/organization.sync.tsx` (SSE path): entitlements do not change on
     `OrganizationChanged`.
   - `useOrganizationPermissions()` in `use-organization-features.tsx`: **nothing to do.** The old
     aggregation needed a `mergeKonnectSyncEnabledForOrganization` call there to keep the current
     organization's contribution fresh; the entitlement is account-scoped, so that coupling is gone.
   - Do **not** add an `AsyncTask` entry — see the note above.

5. Readers subscribe to the store with `useSyncExternalStore`, each selecting a primitive so there
   are no identity churn problems:
   - `useKonnectOrganization()` → `isOrganizationVisible`, then `useMemo`s
     `buildKonnectOrganization(accountId)`. Consumed by `useOrganizations()` (step 8).
   - `useKonnectSyncEnabled()` → `hasEntitlement`. The sidebar uses it directly rather than taking a
     prop, to decide whether the Sync button is enabled.

   Because the store lives outside React, `useKonnectOrganization()` works in `organization.tsx`,
   which renders **above** `AppDataCacheProvider` and therefore cannot use `useDBQuery`.

6. Visibility condition = `hasEntitlement || hasLocalKonnectProjects`, resolved inside
   ...
   - **`$ne: null` alone is not enough.** `konnectControlPlaneId` is in `project.optionalKeys` and is
     ...
     The pre-existing `konnect/sync.ts` compensated for this with a redundant
     `.filter(p => p.konnectControlPlaneId != null)` after the query; it now uses `$exists` and the
     filter is gone.
   ...
   - In the unresolved-conflict case nothing has been migrated yet, so the count is 0 and the
     ...
     and the modal's `onDone` calls `refreshKonnectAccess(…, { force: true })`.
   - `$exists` had to be added to `SpecificQuery` in `packages/insomnia-data/src/database/types.ts`;
     the type did not previously expose it. `konnect/sync.ts`'s own lookup was switched to the same
     predicate, which let its redundant `.filter(p => p.konnectControlPlaneId != null)` go away.

7. Sync button `isDisabled = !konnectSyncEnabled`, with a tooltip explaining why.

#### Why this is not a query

The obvious shape for "async value shared by many components" in this codebase is `useServerQuery`,
and that is what the first implementation used — `['konnect-organization', accountId]` on the
server-data client. It was replaced because three things were wrong with it:

1. **The payload is not server data.** What got cached was `buildKonnectOrganization(accountId)`, a
   pure function of the account id. The only genuine unknowns are a remote boolean and a local
   project count.
2. **Three caches for one boolean.** The query entry, a localStorage record with a 6h TTL, and a
   module-level "already checked this session" latch all coexisted. And since the server-data client
   uses the default `staleTime: 0` (see `ui/context/app/server-data-context.tsx`, which only
   overrides `retry` and `refetchOnWindowFocus`), the query refetched on every new observer mount —
   so react-query was not the thing preventing repeat entitlement requests; the hand-written layers
   were.
3. **The half that changes was on the wrong client.** "Does this account have local Konnect
   projects" is local DB state that changes exactly when sync creates projects or disconnect deletes
   them, yet it sat behind the pull-based server-data client, which never sees `db.changes`. That is
   what forced `useInvalidateKonnectOrganization()` to exist, and the fact that the disconnect flow
   never called it was the step 29 gap — a symptom of the placement, not an independent oversight.

A fourth, smaller point: two consumers read two different layers (the sidebar read localStorage
synchronously, the dropdown went through the query), so they could disagree for a render and the
sidebar never re-rendered when the flag changed.

The alternative considered and rejected was to keep a query for the entitlement and source the local
half from `useOrganizationData(getKonnectOrganizationId(accountId))`, which _is_ `db.changes`-driven.
It is the more "idiomatic" split, but `organization.tsx` calls `useOrganizations()` above the
`AppDataCacheProvider` it renders, so `useDBQuery` throws there; making it work means hoisting that
provider, which is a much larger change than this feature warrants.
...
> (a) readers going through `useOrganizations()` get the injected fake org for free
...
> one — `untracked-projects.tsx`, `git-credentials.$id.related-projects.tsx`,
> `use-command-search.ts`, `insomnia-event-stream-context.tsx`, `router.ts`.
> Do not assume the injection in `useOrganizations()` is enough.
...
3. ~~**Circular dependency.**~~ Historical: while the flag was an aggregation over
   `${accountId}:spaces`, the fake id would have produced a doomed `getOrganizationFeatures` request
   on every refresh — while that same flag decided whether the entry should exist at all. The
   account-scoped entitlements endpoint no longer reads that list, so this reason has lapsed.
...
- **Staleness.** Visibility is `hasEntitlement || hasLocalKonnectProjects`, held in the Phase 1
  store, so a migration or a disconnect can make the organization appear or disappear on demand.
  Baked into the cache it would survive until the next `syncOrganizations`, and `syncOrganizations`
  would have to await the entitlement lookup plus a NeDB count — re-introducing a coupling Phase 1
  rejected, on a function whose third call site is driven by the `OrganizationChanged` SSE event.
...
added: the explicit call sites are what keep the two readers that must see **only** server truth
(`findMigrationTargetSpaceId`, `detectKonnectOrgMigration`) distinguishable from the rest.

#### 2A — Injection & visibility

8. Injection point — `useOrganizations()` in `packages/insomnia/src/ui/hooks/use-account-server-data.ts`
   calls `useKonnectOrganization()` and prepends the result when it is non-null (it must not
   participate in name sorting). Every consumer of `useOrganizations()` therefore sees the fake
   organization, which restores the single-injection-point property the old route loader had.
   `useKonnectOrganization()` lives in `organization-utils.ts` and reads the module store from
   Phase 1 — no query client, so it is safe to call above `AppDataCacheProvider`.
...
10. `packages/insomnia/src/ui/hooks/use-organization-features.tsx`: replace `isScratchpadOrganizationId`
    with `isLocalOrganizationId` in the `enabled` condition of `useOrganizationPermissions`, so the
    query never runs and the hook falls back to `fallbackFeatures` / `fallbackBilling`.
    The `throw redirect('/organization')` hazard the plan originally guarded against disappeared
    with the `organization.$organizationId.permissions.tsx` route itself.
    Note: the sidebar's `konnectSyncEnabled` does not come from this hook — it reads the
    account-level entitlement cache from Phase 1.
...
12. `syncProjects` — now in `packages/insomnia-data/node-src/services/project.ts`, not
    `ui/organization-utils.ts`: bail out for local-only organizations. The guard has to
    sit **before** `getAllTeamProjects()`, not after — the pre-existing scratchpad guard sat after it,
    so the `fetchTeamProjects` request had already gone out by the time it ran. With the guard in
    place the trailing `!isScratchpadOrganizationId(organizationId)` condition is redundant and was
    removed.
...
13. **Collaborator / member routes — all currently unguarded**, add `isLocalOrganizationId`
    short-circuits. Loaders return empty results so their consumers render normally; actions throw
    via `invariant` (or return an error) because reaching them at all is a bug:
    - `organization.$organizationId.collaborators.tsx` — empty page shape
    - `organization.$organizationId.collaborators-search.tsx` — `[]`
    - `organization.$organizationId.collaborators-check-seats.tsx` — `{ isAllowed: false }`
    - `organization.$organizationId.collaborators.invites.$invitationId.tsx` — `invariant`
    - `organization.$organizationId.collaborators.invites.$invitationId.reinvite.tsx` — `invariant`
    - `organization.$organizationId.members.$userId.roles.tsx` — returns an `error` string
...
16. Guard the invite surface. The whole surface is reachable only through `HeaderInviteButton`, so a
    single early return in that component's `useEffect` — before `getOrgUserPermissions` — covers
    `getOrganizationRoles`, `getOrganizationMemberRoles`, `revokeInvitation`,
    `deleteOrganizationMember` and `unlinkCollaborator` as well. The button is also hidden
    (step 18), so this is defence-in-depth.
17. **Existing scratchpad guards on the storage-rules / workspaces fetchers — extend only
    ...
    - `packages/insomnia/src/ui/hooks/use-organization-features.tsx` — **extend** (this is step 10).
      Skipping the query leaves `features`/`billing` at `fallbackFeatures`/`fallbackBilling`, which
      is exactly what a local organization should see.
...
18. `packages/insomnia/src/routes/organization.tsx` component layer: hide `HeaderInviteButton` and
    `PresentUsers` (one `isLocalOrganizationId` branch in `LoginUserActions`, so Scratchpad gets the
    same treatment); also hide the untracked-data banner. Keep `HeaderPlanIndicator` — it is
...
22. `project-navigation-sidebar.tsx`:
    - Delete `SideBarTabList` (and its file), the `Tabs` wrapper, `ProjectNavigationSidebarTabId`,
      and the `activeTab` / `setActiveTab` props.
    - Replace every `isProjectTabActive` branch with `isKonnectOrganizationId(organizationId)`.
    - Drop the `konnectSyncEnabled` prop too: read it from the Phase 1 store via
      `useKonnectSyncEnabled()`, which also removes the sidebar's last use of
      `useOrganizationPermissions`.
    - `showKonnectSyncIntro` becomes `isKonnectOrganization && !hasKonnectPat`.
    - Drop the `konnectControlPlaneId` filtering in `use-navigation-sidebar-data.ts` — the
      `isProjectTabActive` option, `activeProjects`, and the `nonKonnectProjects` /
      `konnectProjects` split all go away; the sidebar renders `organizationProjects` directly.
    - Do not render `NewProjectButton` inside the Konnect organization, and make the
      `sidebar_showCreateDropdown` shortcut a no-op there.
...
23. `EmptyProjectNavigationSidebar`: render `KonnectSyncIntro` when inside the Konnect organization
    ...
    **Known limitation, deliberately not fixed here:** that gear row has no Sync button, so with a
    ...
    `EmptyProjectNavigationSidebar` exists because `project._index` is a separate route from
    `project.$projectId` and renders its own sidebar/pane layout, so the full sidebar is not mounted
    when the organization has no project selected. (The older reason — `ProjectNavigationSidebarInner`
    calling `useProjectLoaderData()!` — no longer applies; the sidebar has since moved to
    TanStack Query and would tolerate the missing loader data.) In a real organization that state is
    rare; in Control Planes it is the _initial_ state, which is why the gap is visible here.
    Collapsing the two routes into one is out of scope. `NoProjectView`'s copy therefore says
...
24. `organization.$organizationId.project.$projectId.tsx` — four separate things, not one:
    ...
    - Remove the `useLocalStorage('${organizationId}:sidebar-tab')` state and the
      `activeSidebarTab = !features.konnectSync.enabled ? 'projects' : …` derivation — which also
      removes this route's only use of `useOrganizationPermissions`.
    - Remove `activeTab` / `setActiveTab` / `konnectSyncEnabled` from the
      `<ProjectNavigationSidebar>` props.
...
28. `use-konnect-sync.ts` needs **no change**: its call sites already pass the route's
    `organizationId`, which inside Control Planes _is_ the Konnect organization id. `konnect/sync.ts`
    internals are unchanged apart from the `$exists` query fix (step 6).
29. `konnect-settings-modal.tsx` / sync orchestration ownership:
    - **Move the sync trigger out of the sidebar** so the modal no longer depends on being handed a
      ref by whichever component renders it, per decision 8. No Settings panel is added in this
      change.
    - After disconnecting, `onDisconnect` calls `refreshKonnectAccess(…, { force: true })` so the
      organization disappears from the dropdown in the same session when it was only visible because
      of local data. Navigating away from the now-dead route is still **not** implemented.
...
31. New file `packages/insomnia/src/konnect/migrate-konnect-organization.ts`:
    ...
    - Copy `${keepOrganizationId}:konnect-last-synced-at` to
      `${getKonnectOrganizationId(accountId)}:konnect-last-synced-at` (only when the target key is
      not already set, so a re-run never clobbers a newer sync), then remove the key for every
      source organization in the migrated set (decision 10). The set is derived from the `parentId`s
      captured **before** the re-parent, so orphans are naturally excluded. The whole timestamp
      block is wrapped in its own `try/catch`: a stale timestamp is cosmetic and must never fail the
      migration.
...
33. New component `packages/insomnia/src/ui/components/modals/konnect-org-migration-modal.tsx`:
    single-select list of candidate organizations (name + Konnect project/collection counts), an
    explicit warning that the non-selected data will be deleted, then calls `runKonnectOrgMigration`
    and `refreshKonnectAccess(…, { force: true })`. It is mounted from `organization.tsx`, which re-runs
    `detectKonnectOrgMigration` after hydration and only surfaces the modal for a `conflict`.
    **Known gap:** as built the modal has no dismiss affordance (`isDismissable` / close button), so
    the "dismiss and be re-prompted next start" path from decision 10 is currently unreachable.
...
35. Smoke tests:
    - `packages/insomnia-smoke-test/tests/smoke/konnect.test.ts` — replace the
      `sidebar-tab-konnect` / `sidebar-tab-projects` clicks with selecting Control Planes from the
      organization dropdown; replace the "tab hidden when flag off" case with "organization absent
      from the dropdown when flag off"; assert the New Project button is hidden inside Control
      Planes.
    - `packages/insomnia-smoke-test/playwright/pages/components/navigation-sidebar.ts` — replace
      `clickProjectsTab()` / `clickKonnectTab()` with `selectOrganization(name)` /
      `openControlPlanesOrganization()`.
    - `packages/insomnia-smoke-test/server/insomnia-api.ts` — serve `GET /v1/user/entitlements`
      (filtered by the `feature` query param) and add a `POST /v1/test-utils/user/entitlements`
      override so a test can revoke the entitlement; drop `konnectSync` from the default org
      features.
    - `packages/insomnia-smoke-test/tests/smoke/disable-git-sync.test.ts` no longer needs to carry
      `konnectSync` in its feature payloads.
...
3. The account has no `konnectControlPlanes_default` entitlement and there is no local data →
   Control Planes is absent from the dropdown. Revoking it server-side takes effect on the next app
   start or sign-in, not mid-session.
...
5. Inside Control Planes with DevTools Network open → no `features`, `storage-rule`,
   ...
   A single `/v1/user/entitlements?feature=konnectControlPlanes` request per app start is expected
...
13. Inside Control Planes, open a request's settings → "Move/Copy to Workspace" lists the other
    ...
    project's workspaces (regression guards for step 17).
14. Open the command palette from a real organization and search for the name of a Konnect
    collection or request → it appears, labelled "Control Planes" (regression guard for step 20).
15. Reset the vault key on a second device → the `VaultKeyChanged` event clears the secret
...
17. Disconnect the PAT while inside Control Planes with the flag off → all Konnect projects are
    removed. **Known gap (step 29):** the organization does not disappear from the dropdown until the
    `['konnect-organization', accountId]` query refetches, and there is no automatic navigation to a
    real organization.
...
20. Dismiss the conflict modal without choosing → nothing is migrated, and the modal appears again
    on the next start (no idempotency flag suppressing it). **Currently unreachable** — see the
    known gap in step 33.
...
- **Steps 4 / 8 — the biggest deviation.** The plan assumed `organization.tsx` had a `clientLoader`
  to inject the fake organization into and to sequence the feature-flag refresh. Develop's
  account-data refactor deleted that loader (and the whole
  `organization.$organizationId.permissions.tsx` route) in favour of TanStack Query hooks, so the
  first rewrite was query-based: `useKonnectOrganization()` wrapped `getKonnectOrganization()` in a
  server-data query keyed `['konnect-organization', accountId]`, with
  `useInvalidateKonnectOrganization()` to force a re-check.

  That was then **replaced by a module-level store** for the reasons in "Why this is not a query".
  The landing design is: `refreshKonnectAccess()` resolves `{ hasEntitlement, isOrganizationVisible }`
  into module state at four explicit points (pre-hydration, post-login loader, disconnect, migration
  modal), and `useKonnectOrganization()` / `useKonnectSyncEnabled()` subscribe with
  `useSyncExternalStore`. `useOrganizations()` (`use-account-server-data.ts`) prepends the result, so
  every consumer sees the fake organization — the single-injection-point property the loader used to
  provide.

  The entitlement lookup itself moved three times before that: first hung off a new `AsyncTask`
  entry, then off the `organization._index.tsx` loader, then inlined into the query's `queryFn`. The
  ordering problem that motivated the last move (writer and reader in two concurrently running
  loaders) disappears once resolution happens before hydration.

- **Step 3 — the flag source changed mid-flight.** The first implementation OR-ed
  `features.konnectSync.enabled` across every organization (`Promise.allSettled` over
  `getOrganizationFeatures`) and kept the current organization's value fresh through
  `mergeKonnectSyncEnabledForOrganization`, called from `useOrganizationPermissions`'s `queryFn`.
  Both are gone: `GET /v1/user/entitlements?feature=konnectControlPlanes` is account-scoped, so a
  single request answers the question. Fallout: `konnectSync` was removed from `FeatureList`,
  `fallbackFeatures`, the smoke mock server's default features and `disable-git-sync.test.ts`; the
  disabled Sync button's tooltip no longer talks about organizations.

  The 6h TTL and the `hasCheckedThisSession` latch that version introduced are both gone too — with
  resolution pinned to startup and sign-in there is nothing to dedupe against except a duplicate
  cold-start call, which the `konnectAccessResolvedFor` account check handles. localStorage survives
  only as the offline fallback for the entitlement boolean.
...
- **Step 29** — instead of a React context, the sync trigger lives in a small module-level registry
  ...
  `KonnectSettingsModal` entirely and no-ops when the sidebar is not mounted. The post-disconnect
  redirect described in the original step is still **not** implemented, though the organization now
  does disappear from the dropdown.
- **Step 16** — the guard sits in `HeaderInviteButton` itself (returning before
  ...
  through that button.
- **Step 12** — `syncProjects` had moved from `ui/organization-utils.ts` into
  `insomnia-data/node-src/services/project.ts`; the guard was placed _above_ `getAllTeamProjects`,
  where the original scratchpad guard sat after it and the `fetchTeamProjects` request had already
  been issued.
- **Step 22** — the sidebar no longer takes a `konnectSyncEnabled` prop either; it calls
  `useKonnectSyncEnabled()`, which dropped its dependency on `useOrganizationPermissions`. The
  tab-based project filtering was removed from `use-navigation-sidebar-data.ts` rather than from the
  component.
- **Step 32** — the entry point is `migrateKonnectProjectsIfUnambiguous(accountId)`, which runs
  ...
  carrying a defensive allow-list. `refreshKonnectAccess` runs immediately after it, in the same
  block, for the same reason. The conflict modal is mounted in `organization.tsx` and calls
  `runKonnectOrgMigration` directly rather than going through the `showModal` registry, because it
  needs to refresh Konnect access on completion.

Changes the plan did not anticipate:

- `packages/insomnia-data/src/database/types.ts` — `SpecificQuery` gained `$exists?: boolean`. The

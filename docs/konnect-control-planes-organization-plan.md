...
> landing point though — the migration runs before hydration (step 32) and the entitlement lookup
> hangs off `useKonnectOrganization()` (step 4), so both survive a future change of startup
> destination.
>
...
3. Add `getUserEntitlements({ sessionId, feature })` to `packages/insomnia-api/src/user.ts`
   ...
   Then add a **standalone** `syncKonnectSyncEnabled(sessionId, accountId, { force })` to
   `packages/insomnia/src/ui/organization-utils.ts`:
   - One `getUserEntitlements({ sessionId, feature: KONNECT_CONTROL_PLANES_FEATURE })` call;
     `enabled` = some entitlement has `featureKey === 'konnectControlPlanes_default' && hasAccess`.
   - Cache to localStorage `${accountId}:konnectSyncEnabled = { enabled, checkedAt }`.
   - Self-deduplicating via a TTL (6h) **plus a module-level `hasCheckedThisSession` latch**: the
     first call in a renderer always hits the network, so a revoked entitlement takes effect on the
     next app start instead of lingering for up to 6h. `force` bypasses both.
   - Treat a request failure as false but never overwrite an existing cached `true` (offline
     resilience). An empty `entitlements` array is a _successful_ response and does downgrade.

4. Call site — `getKonnectOrganization(sessionId, accountId)` in `organization-utils.ts`, which
   **awaits `syncKonnectSyncEnabled` immediately before reading the cache**. Not a route loader:
   `organization.tsx` no longer has one.
   Two reasons for making the consumer await its own precondition:
   - **Ordering.** The writer (`syncKonnectSyncEnabled`) and the reader (`getKonnectSyncEnabled`)
     must not sit in two concurrently-running loaders/queries — the reader would silently see the
     previous session's value and simply hide the organization for one render, with no error
     anywhere. Co-locating them removes the cross-file ordering assumption entirely.
   - **Reliability.** `getKonnectOrganization` is driven by `useKonnectOrganization()`, which every
     `useOrganizations()` consumer mounts, so it runs whether the app opens on `/organization`, on
     `/organization/:id`, or deep-links straight into a workspace. A route that only matches one
     exact path would silently stop firing if the startup destination ever changed.

   The TTL and the session latch make this a no-op on all but the first load of a session.
...
5. Add a `getKonnectSyncEnabled(accountId)` helper for reading the cache. It is a plain synchronous
   localStorage read, so it must only ever be called after `syncKonnectSyncEnabled` has been
   awaited — which `getKonnectOrganization` guarantees before anything renders. The sidebar reads it
   directly (rather than taking a prop) to decide whether the Sync button is enabled.
6. Visibility condition = `konnectSyncEnabled || hasLocalKonnectProjects`, computed in
   `getKonnectOrganization()`.
   ...
   - In the unresolved-conflict case nothing has been migrated yet, so the count is 0 and the
     ...
     and `useInvalidateKonnectOrganization()` re-runs this query.
...
- **Staleness.** Visibility is `konnectSyncEnabled || hasLocalKonnectProjects`. Computed in
  `getKonnectOrganization()` it is re-evaluated whenever the query is invalidated, so a migration or
  a disconnect can make the organization appear or disappear on demand. Baked into the cache it
  would survive until the next `syncOrganizations`, and `syncOrganizations` would have to await
  `syncKonnectSyncEnabled` plus a NeDB count — re-introducing a coupling Phase 1 rejected, on a
...
8. Injection point — `useOrganizations()` in `packages/insomnia/src/ui/hooks/use-account-server-data.ts`
   ...
   `useKonnectOrganization()` / `useInvalidateKonnectOrganization()` live in `organization-utils.ts`
   and are bound to the server-data query client under the key `['konnect-organization', accountId]`.
...
22. `project-navigation-sidebar.tsx`:
    ...
    - Drop the `konnectSyncEnabled` prop too: read it from the Phase 1 cache via
      `getKonnectSyncEnabled(userSession.accountId)`, which also removes the sidebar's last use of
...
29. `konnect-settings-modal.tsx` / sync orchestration ownership:
    - **Move the sync trigger out of the sidebar** so the modal no longer depends on being handed a
      ...
      change.
    - **Not implemented:** redirecting away after a disconnect that makes the organization
      invisible, and invalidating the `['konnect-organization', accountId]` query at that point. With
      the feature flag on, the organization stays visible after disconnecting, which is correct; with
      the flag off it lingers until the next query refetch. Left as follow-up.
...
33. New component `packages/insomnia/src/ui/components/modals/konnect-org-migration-modal.tsx`:
    ...
    and `useInvalidateKonnectOrganization()`. It is mounted from `organization.tsx`, which re-runs
...
3. The account has no `konnectControlPlanes_default` entitlement and there is no local data →
   ...
   start (session latch), not necessarily on the next render.
...
- **Steps 4 / 8 — the biggest deviation.** The plan assumed `organization.tsx` had a `clientLoader`
  ...
  implementation is hook-based instead:
  - `getKonnectOrganization(sessionId, accountId)` in `organization-utils.ts` awaits
    `syncKonnectSyncEnabled` and then applies the visibility rule.
  - `useKonnectOrganization()` wraps it in a server-data query keyed
    `['konnect-organization', accountId]`; `useInvalidateKonnectOrganization()` forces a re-check.
  - `useOrganizations()` (`use-account-server-data.ts`) prepends the result, so every consumer sees
    the fake organization — the single-injection-point property the loader used to provide.

  `syncKonnectSyncEnabled` was first hung off a new `AsyncTask` entry, then off the
  `organization._index.tsx` loader, before landing here. Only awaiting it inside its own consumer
  guarantees the write happens before the read; the earlier placements relied on two concurrently
  running loaders happening to execute in the right order, which fails silently.
...
- **Step 3 — the flag source changed mid-flight.** The first implementation OR-ed
  ...
  disabled Sync button's tooltip no longer talks about organizations. A `hasCheckedThisSession`
  latch was added so the 6h TTL cannot outlive a revoked entitlement across an app restart — which
  is also what makes the "hides the Control Planes organization" smoke test's `page.reload()`
  meaningful.
...
- **Step 29** — instead of a React context, the sync trigger lives in a small module-level registry
  ...
  redirect described in the original step was **not** implemented.
...
- **Step 12** — `syncProjects` had moved from `ui/organization-utils.ts` into
  ...
  been issued.
- **Step 22** — the sidebar no longer takes a `konnectSyncEnabled` prop either; it reads
  `getKonnectSyncEnabled(userSession.accountId)` directly, which dropped its dependency on
  `useOrganizationPermissions`. The tab-based project filtering was removed from
  `use-navigation-sidebar-data.ts` rather than from the component.- **Step 32** — the entry point is `migrateKonnectProjectsIfUnambiguous(accountId)`, which runs
  ...
  needs to invalidate the Konnect-organization query on completion.
...
(step 33), and disconnecting the PAT neither invalidates the Konnect-organization query nor
navigates away (step 29).

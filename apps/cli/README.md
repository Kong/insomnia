...
$PWD/apps/cli/bin/inso run test
# will use config, useful for testing with fewer args
$PWD/apps/cli/bin/inso -w apps/cli/src/db/fixtures/git-repo script runTest
```
...
$PWD/apps/cli/bin/inso run test "Echo Test Suite" -w $PWD/packages/insomnia-smoke-test/fixtures/inso-nedb --env Dev --verbose
```
...
$PWD/apps/cli/binaries/inso run test "Echo Test Suite" -w $PWD/packages/insomnia-smoke-test/fixtures/inso-nedb --env Dev --verbose
...
./apps/cli/bin/inso -w <INSO_NEDB_PATH>

# using a binary
./apps/cli/binaries/insomnia-inso -w <INSO_NEDB_PATH>
```
...
1. The docs appear in your vscode diff, or you can look in `./apps/cli/reference/` if the version number looks wrong you might need to check what branch you're on, it should be run from develop ideally as the only changes in the release branch should be hotfixes that dont affect inso docs

```sh
npm i && npm run build -w insomnia-inso && $PWD/apps/cli/bin/inso generate-docs
```

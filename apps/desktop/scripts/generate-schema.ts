 ...
 *
 * The output is written to the repo-root `schemas/` directory as a versioned,
 * immutable file for the current `INSOMNIA_SCHEMA_VERSION`
 * (e.g. `insomnia.schema.5.1.json`). Bumping the version produces a new file
 * alongside the previous ones, keeping every published schema addressable.
 *
...
// apps/desktop/scripts/, so go up three levels to the repo root.
const OUTPUT_DIR = path.resolve(__dirname, '../../../schemas');
// Versioned, immutable file for the current schema version.
...
const main = async () => {
  ...
  const schema = {
    ...
    $id: `${SCHEMA_BASE_URL}/${VERSIONED_FILENAME}`,
    title: 'Insomnia File',
    description:
  ...
  const outputPath = path.join(OUTPUT_DIR, VERSIONED_FILENAME);

...
test.describe("after-response script features tests", async () => {
  ...
  test("post: insomnia.test and insomnia.expect can work together", async ({
    page,
  }) => {
    await page
      .getByLabel("Request Collection")
      .getByTestId("tests with expect and test")
      .press("Enter");
  ...
  test("environment and baseEnvironment can be persisted", async ({ page }) => {
    ...
    await page
      .getByLabel("Request Collection")
      .getByTestId("persist environments")
      .press("Enter");

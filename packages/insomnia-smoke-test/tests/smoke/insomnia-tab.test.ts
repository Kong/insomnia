...
test.describe("multiple-tab feature test", () => {
  ...
  test("add tab & close tab", async ({ page }) => {
    ...
    const tab = await page
      ...
      .getByLabel(`tab-${DEFAULT_REQUEST_NAME}`, { exact: true });
  ...
  test("active tab sync with the sidebar active request", async ({ page }) => {
    ...
    await page
      .getByRole("textbox", { name: "GET New Request" })
      .fill("New Request A");
    await page.getByLabel("Create in collection").click();
    await page.getByLabel("HTTP Request").click();
    await page.getByTestId("New Request").dblclick();
  ...
  test("change icon after change request method", async ({ page }) => {
    ...
    await page.waitForTimeout(1000);
    expect(
      await page.getByLabel("Insomnia Tabs").getByLabel("Tab Tag").innerText(),
    ).toEqual("GET");
  ...
  test("click + button to add a new request", async ({ page }) => {
    ...
    await page
      .getByRole("menuitem", { name: "add request to current" })
      .click();
    ...
    await page
      ...
      .selectOption({ label: "My Collection" });
    await page.getByRole("dialog").getByRole("button", { name: "Add" }).click();
    await page.waitForTimeout(1000);
    expect(
      await page.getByTestId("workspace-context-dropdown").innerText(),
    ).toEqual("My Collection");
    await page.getByTestId(DEFAULT_REQUEST_NAME).click();
  });

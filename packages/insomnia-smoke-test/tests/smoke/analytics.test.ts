...
test("analytics events are sent", async ({ page, app }) => {
  await app.evaluate(async ({ session }) => {
    ...
    session.defaultSession.webRequest.onBeforeRequest((details, callback) => {
      if (details.url.includes("segment")) {
        globalThis.segmentLogs.push({
          url: details.url,
          data: details.uploadData,
        });
      }
  ...
  for (let i = 0; i < 10; i++) {
    ...
    await page
      .getByRole("menuitemradio", { name: "HTTP Request" })
      .press("Enter");
  }
    ...
    (log: { url: string; data: { type: string; bytes: number[] }[] }) => {
      return {
        ...
        data: log.data.map((data) =>
          JSON.parse(Buffer.from(Object.values(data.bytes)).toString("utf-8")),
        ),
      };
    },
  );

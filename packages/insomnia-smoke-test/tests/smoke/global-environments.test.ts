...
test.describe("Global Environments", async () => {
  test("import and use a global environment from a collection", async ({
    app,
    page,
  }) => {
    ...
    await page
      ...
      .getByLabel("GET New Request", { exact: true })
      .click();
    // check if it has error message
    await page.getByText("Body", { exact: true }).click();
    await expect(
      page.getByTitle(
        "Failed to render environment variables: _['global-base']",
      ),
    ).toHaveText("_['global-base']");
    // check if it appears as a custom message when sending the request
    await page.getByRole("button", { name: "Send" }).click();
    await page
      .getByRole("heading", { name: "2 environment variables are" })
      .click();

...
test("can make websocket connection", async ({ app, page }) => {
  ...
  await page.getByRole("tab", { name: "Console" }).click();
  await expect(responseBody).toContainText(
    "> authorization: Bearer insomnia-cool-token-!!!1112113243111",
  );

  // Can handle redirects
  await page
    .getByLabel("Request Collection")
    .getByTestId("redirect")
    .press("Enter");
  await expect(page.locator(".app")).toContainText(
  ...
  const webSocketActiveConnections = page.locator(
  ...
  await page
    .locator('button[name="DisconnectDropdown__DropdownButton"]')
    .click();

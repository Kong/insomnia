...
test.describe("pre-request features tests", async () => {
  ...
  const testCases = [
    ...
    },
    {
      ...
      customVerify: (bodyJson: any) => {
        ...
        expect(bodyJson.headers["authorization"]).toEqual(
          "CustomTokenPrefix tokenValue",
        );
      },
    },
    {
      ...
      customVerify: (bodyJson: any) => {
        const authzHeader = bodyJson.headers["authorization"];
        expect(authzHeader != null).toBeTruthy();
        const expectedEncCred = Buffer.from("myName:myPwd", "utf-8").toString(
          "base64",
        );
        expect(bodyJson.headers["authorization"]).toEqual(
          `Basic ${expectedEncCred}`,
        );
      },
    },
    {
  ...
  for (let i = 0; i < testCases.length; i++) {
    ...
    test(tc.name, async ({ page }) => {
      const statusTag = page.locator(
        '[data-testid="response-status-tag"]:visible',
      );
  ...
  test("send request with content type", async ({ page }) => {
    ...
    await page
      .getByLabel("Request Collection")
      .getByTestId("echo pre-request script result")
      .press("Enter");
    ...
    const preRequestScriptEditor = page
      .getByTestId("CodeEditor")
      .getByRole("textbox");
  ...
  test("insomnia.request / update proxy configuration", async ({ page }) => {
    ...
    await page
      .getByLabel("Request Collection")
      .getByTestId("test proxies manipulation")
      .press("Enter");
  ...
  test("update clientCertificate if request url contains tag", async ({
    page,
  }) => {
    const responsePane = page.getByTestId("response-pane");
  ...
  test("insomnia.request / update clientCertificate", async ({ page }) => {
    ...
    await page
      .getByLabel("Request Collection")
      .getByTestId("test certificate manipulation")
      .press("Enter");
  ...
  test("pre: insomnia.test and insomnia.expect can work together", async ({
    page,
  }) => {
    await page
      .getByLabel("Request Collection")
      .getByTestId("insomnia.test")
      .press("Enter");
  ...
  test("environment and baseEnvironment can be persisted", async ({ page }) => {
    ...
    await page
      .getByLabel("Request Collection")
      .getByTestId("persist environment")
      .press("Enter");
    ...
    await page
      .getByRole("button", { name: "Manage collection environments" })
      .click();
    const responseBody = page
      .getByRole("dialog")
      .getByTestId("CodeEditor")
  ...
  test("kv pair environment can be updated", async ({ page }) => {
    ...
    await page
      .getByLabel("Request Collection")
      .getByTestId("update kv pair environment")
      .press("Enter");

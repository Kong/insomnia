...
test.describe('Environment Editor', () => {
  test('manage environment', async ({ page, app }) => {
    ...
    await page.getByRole('dialog').getByTestId('CodeEditor').getByRole('textbox').press('Enter');
    await page.getByRole('dialog').getByTestId('CodeEditor').getByRole('textbox').fill('"testNumber":9000,');
    await page.getByRole('dialog').getByTestId('CodeEditor').getByRole('textbox').press('Enter');
    await page.getByRole('dialog').getByTestId('CodeEditor').getByRole('textbox').fill('"testString":"Gandalf",');

...
test.describe("Environment Editor", async () => {
  ...
  test("create a new environment", async ({ page }) => {
    ...
    await page
      .getByRole("menuitemradio", { name: "Shared Environment" })
      .press("Enter");
  ...
  test("duplicate an environment", async ({ page }) => {
    ...
    await page
      ...
      .click();
    await page
      .getByRole("row", { name: "ExampleA" })
      .getByLabel("Environment Actions")
      .click();
    await page.getByText("Duplicate").click();
    await page
      .getByLabel("Environments", { exact: true })
      .getByText("ExampleA (Copy)")
      .click();
  });
  ...
  test("Rename an existing environment", async ({ page }) => {
    ...
    await page
      ...
      .click();
    await page
      .getByRole("row", { name: "ExampleB" })
      .locator("[data-editable=true]")
      .dblclick();
    await page
      .getByRole("row", { name: "ExampleB" })
      .locator("input")
  ...
  test("Add new variables to an existing environment", async ({ page }) => {
    ...
    await page
      .locator("pre")
      .filter({ hasText: "_.exampleObject.anotherNumber" })
      .press("Enter");
  ...
  test("Switch to table view and edit environment", async ({ page }) => {
    ...
    await bodyEditor.fill(
      '"anotherString":"kvAnotherStr","anotherNumber": 12345',
    );

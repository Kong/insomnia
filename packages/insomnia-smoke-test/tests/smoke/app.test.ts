...
test("can send requests", async ({ app, page }) => {
  ...
  await page
    .getByRole("button", { name: "Workspace actions menu button" })
    .click();
  ...
  await page
    .locator("pre")
    .filter({ hasText: "< Content-Type: application/pdf" })
    .click();

  await page
    .getByLabel("Request Collection")
    .getByTestId("sends request with basic authentication")
    .press("Enter");
  await page
    .getByTestId("request-pane")
    .getByRole("button", { name: "Send" })
  ...
  await expect(responseBody).toContainText(
    "Set-Cookie: insomnia-test-cookie=value123",
  );
});

...
const testVaultSalt =
  "e619272433fc739d52ff1ba1b45debedfe55cb42685af10a46e2b1285acb7120";
const tesSrpSecret =
  "b424e8700ef89f77a6cffc648b9c6d42bb7de58914d88cd79966684ffe5b4ebe";
...
test("Check vault key generation", async ({ page }) => {
  ...
  const vaultKeyValue = await page
    .getByTestId("VaultKeyDisplayPanel")
    .innerText();
  ...
  await page
    .getByRole("grid", { name: "Environments" })
    .getByText("New Environment")
    .click();

  const kvTable = await page.getByRole("listbox", {
    name: "Environment Key Value Pair",
  });
  ...
  await expect(firstRow.getByTestId("OneLineEditor").nth(1)).toContainText(
    "bar",
  );
});
...
test.describe("Vault key actions", async () => {
  ...
  test("check reset vault key", async ({ page }) => {
    ...
    const vaultKeyValueInModal = await modal
      .getByTestId("VaultKeyDisplayPanel")
      .innerText();
  ...
  test("check reset vault key in private environment", async ({
    page,
    app,
  }) => {
    ...
    await page.getByText("Reset Vault Key").click();
    await page.getByText("Yes").click();
    const vaultKeyValueInModal = await page
      .getByTestId("VaultKeyDisplayPanel")
      .innerText();
    expect(vaultKeyValueInModal.length).toBeGreaterThan(0);
  });
...
test.describe("Check vault used in environment", async () => {
  ...
  test("create global private sub environment to store vaults", async ({
    page,
    app,
  }) => {
    // import request
    ...
    await page
      ...
      .click();
    await page
      .getByPlaceholder("Enter a name for your Environment")
      .fill("New Global Vault Environment");
    await page.getByRole("button", { name: "Create", exact: true }).click();
    await page.getByTestId("CreateEnvironmentDropdown").click();
    await page.getByText("Private environment").click();
    // activate created private environment
    ...
    await page
      .getByRole("option", { name: "New Global Vault Environment" })
      .click();
    ...
    await page
      .getByTestId("normal")
      .getByLabel("GET normal", { exact: true })
      .click();
  ...
  test("test vault environment to be applied", async ({ app, page }) => {
    ...
    await page.getByPlaceholder("Choose a global environment").click();
    await page
      .getByRole("option", { name: "Global env with secret vault" })
      .click();
    ...
    await expect(
      page.getByText("vault is a reserved key for insomnia vault"),
    ).toBeVisible();
  });
});

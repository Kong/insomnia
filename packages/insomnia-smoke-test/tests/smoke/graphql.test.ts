...
test("can render schema and send GraphQL requests", async ({ app, page }) => {
  ...
  await expect(graphqlExplorer).toContainText(
  ...
  const exepctResult = JSON.stringify({
    query: "query($inputVar:Int){echoNum(intVar:$inputVar)}",
    variables: { inputVar: 3 },
  });
  // expect exported curl body to be JSON string
  expect(
    clipboardContent.split("--data ")[1]?.replace(/[\n\s\']/g, ""),
  ).toContain(exepctResult);

  await page.getByRole("button", { name: "Send" }).click();

  const responseBody = page.locator(
...
test("can render schema and send GraphQL requests with object variables", async ({
  app,
  page,
}) => {
  test.slow(
  ...
  await page
    .getByLabel("Request Collection")
    .getByTestId("GraphQL request with variables")
    .press("Enter");
  await page.getByRole("tab", { name: "Body" }).click();
  // Assert the schema is fetched after switching to GraphQL request
  await expect(page.getByText("Schema fetched just now")).toBeVisible();
  ...
  await expect(graphqlExplorer2).toContainText(
    "Characters who at any time bore a Ring of Power.",
  );
  await page.click("text=QueryRingBearer >> button");
...
test("can render numeric environment", async ({ app, page }) => {
  ...
  await page
    .getByLabel("Request Collection")
    .getByTestId("GraphQL request with number")
    .press("Enter");
  await page.getByRole("tab", { name: "Body" }).click();
  // Assert the schema is fetched after switching to GraphQL request
  await expect(page.getByText("Schema fetched just now")).toBeVisible();
...
test("can send GraphQL requests after editing and prettifying query", async ({
  app,
  page,
}) => {
  test.slow(

...
test("can send gRPC requests using mTLS requests (with reflection)", async ({
  app,
  page,
}) => {
  test.slow(
  ...
  await page
    ...
    .press("Enter");
  await expect(
    page.getByRole("button", { name: "Select Method" }),
  ).toBeDisabled();
  ...
  ).setFiles(path.join(fixturePath, "rootCA.pem"));
  ...
  await page
    .locator('[data-test-id="add-client-certificate-file-chooser"]')
    .click();
  await (
    await fileChooserPromise
  ).setFiles(path.join(fixturePath, "client.crt"));

  fileChooserPromise = page.waitForEvent("filechooser");
  await page
    .locator('[data-test-id="add-client-certificate-key-file-chooser"]')
    .click();
  await (
    await fileChooserPromise
  ).setFiles(path.join(fixturePath, "client.key"));

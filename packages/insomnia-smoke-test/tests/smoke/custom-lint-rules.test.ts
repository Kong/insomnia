...
import { launchInsomnia } from '../../playwright/launch';
import { InsomniaApp } from '../../playwright/pages';
import { getFixturePath, randomDataPath } from '../../playwright/paths';
import { test } from '../../playwright/test';

// @ts-expect-error playwrightConfig.webServer.url must exist
const devServerUrl: string = playwrightConfig?.webServer?.url || 'http://127.0.0.1:4010';
...
async function commitAndPush(page: Page, message: string) {
  ...
  const plusIcons = page.locator('[data-icon="plus"]');
...
test.describe('Custom Spectral Lint Rules', () => {
  ...
  // ---------------------------------------------------------------------------
  // 3. Cloud sync — upload, reload, and three sync round-trips
  //
  ...
  //
  // Sub-tests:
  //   3a. basic upload within a synced design project (smoke: UI works)
  //   3b. same user, different machine (push → pull round-trip)
  //   3c. different users, same project (user A push → user B pull)
  // ---------------------------------------------------------------------------
  test.describe('within a cloud-sync project', () => {
    ...
    // -------------------------------------------------------------------------
    // 3a. Basic upload in a cloud-sync design project
    // -------------------------------------------------------------------------
    test('upload custom ruleset in a synced design project', async ({ page, insomnia }) => {
      await insomnia.navigationSidebar.fetchUnsyncedWorkspace('Design Project');
    ...
    // -------------------------------------------------------------------------
    test.describe.serial('same user, different machine (push → pull round-trip)', () => {
      test.beforeAll(async () => {
        await fetch(`${devServerUrl}/__test-config/cloud-sync/reset`, { method: 'POST' });
      });
      ...
      test('machine A - upload ruleset and push to remote', async ({ page, insomnia }) => {
        await insomnia.navigationSidebar.fetchUnsyncedWorkspace('Design Project');
      ...
      test('machine B - pull from fresh state and see the ruleset', async ({ page, insomnia }) => {
        await insomnia.navigationSidebar.fetchUnsyncedWorkspace('Design Project');
        ...
        await expect
          ...
          .toBeVisible({ timeout: 15_000 });
        await expect
          .soft(page.getByRole('option', { name: new RegExp(RULESET_RULE_NAME) }))
          .toBeVisible({ timeout: 15_000 });
      });
    });

    // -------------------------------------------------------------------------
    // 3c. Different users, same project
    //
    ...
    test.describe.serial('different users, same project', () => {
      ...
      test('user A - upload ruleset and push', async ({ page, insomnia }) => {
        await insomnia.navigationSidebar.fetchUnsyncedWorkspace('Design Project');
      ...
      // Launch user B directly to avoid an idle user-A Electron process.
      test('user B - pull project and see user A ruleset', async ({ playwright, userConfig }) => {
        ...
        const userBApp = await launchInsomnia(playwright, cloneEnv as any);
        (userBApp as any).__launchEnv = cloneEnv;
        (userBApp as any).__playwright = playwright;
        const userBPage = await userBApp.firstWindow({ timeout: 60_000 });
        await userBPage.waitForLoadState();
        await userBPage.evaluate(() => (window as any).main.secretStorage.setSecret('konnectPat', 'kpat_test'));
        const machineB = new InsomniaApp(userBPage, userBApp);

        await machineB.navigationSidebar.fetchUnsyncedWorkspace('Design Project');

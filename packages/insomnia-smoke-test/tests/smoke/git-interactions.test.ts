...
test.skip('Git Interactions (clone, checkout branch, pull, push, stage changes, ...)', async ({ page }) => {
  ...
  if (!gitSyncSmokeTestToken) {
    console.log(
      'Skipping, set GIT_SYNC_SMOKE_TEST_TOKEN to run, TIP: "gh auth login to get a token" and "export GIT_SYNC_SMOKE_TEST_TOKEN=$(gh auth token)"',
    );
    test.skip();
    return;
  }
  ...
  await page
    ...
    .fill('https://github.com/Kong/insomnia-git-example.git');

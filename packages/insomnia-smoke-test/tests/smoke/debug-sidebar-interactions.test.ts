...
test.describe("Debug-Sidebar", async () => {
  ...
  test.describe("Interact with sidebar", async () => {
    test("Open Properties in Request Sidebar", async ({ page }) => {
      ...
      const grpc = page
        .getByLabel("Request Collection")
        .getByRole("row", { name: "example grpc" });
      await grpc.click();
      ...
      const ws = page
        .getByLabel("Request Collection")
        .getByRole("row", { name: "example websocket" });
      await ws.click();
      ...
      const gql = page
        .getByLabel("Request Collection")
        .getByRole("row", { name: "example graphql" });
      await gql.click();
      await gql.getByLabel("Request Actions").click();
      await page.getByRole("menuitemradio", { name: "Settings" }).click();
      // Close settings modal
      await page.locator(".app").press("Escape");
      const folderLocator = page
        .getByLabel("Request Collection")
    ...
    test("Pin a Request", async ({ page }) => {
      ...
      const pinnedRequestLocator = page
        .getByLabel("Pinned Requests")
        .getByRole("row", { name: "example http" });
    ...
    test.skip("Delete Request", async ({ page }) => {
      const requestLocator = page
        ...
        .getByRole("row", { name: "example http" });
      await requestLocator.click();
      const numberOfRequests = await page
        .getByLabel("Request Collection")
        .getByRole("row")
        .count();
      await requestLocator.getByLabel("Request Actions").click();
      ...
      expect(
        page.getByLabel("Request Collection").getByRole("row"),
      ).toHaveCount(numberOfRequests - 1);
    });
    ...
    test("Rename a request", async ({ page }) => {
      ...
      await page
        .getByRole("textbox", { name: "GET example http" })
        .fill("example http1");
      await requestLocator.click();
      await page
        .getByLabel("Request Collection")
        .getByRole("row", { name: "example http1" })
        .click();
    });
    ...
    test("Update a request folder via settings", async ({ page }) => {
      ...
      await page
        .getByLabel("Request Collection")
        .getByRole("row", { name: "test folder1" })
        .click();
    });

    test("Rename a request by clicking", async ({ page }) => {
      await page
        .getByTestId("example http")
        .getByLabel("GET example http", { exact: true })
        .dblclick();
      await page
        .getByRole("textbox", { name: "GET example http" })
        .fill("new name");
      ...
      await expect(
        page
          .getByTestId("new name")
          .getByLabel("GET new name", { exact: true }),

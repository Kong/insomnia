...
test("Command palette - can switch between requests and workspaces", async ({
  app,
  page,
}) => {
  test.slow(
  ...
  await app.evaluate(
    async ({ clipboard }, swaggerDoc) => clipboard.writeText(swaggerDoc),
    swaggerDoc,
  );
  ...
  await page
    ...
    .getByText("http://127.0.0.1:4010/cookies")
    .click();
  const requestSwitchKeyboardShortcut =
  ...
  await page
    .getByTestId("OneLineEditor")
    .getByText("http://127.0.0.1:4010/pets/")
    .click();
  ...
  await page.getByPlaceholder("Search and switch between").press("Enter");
  await expect(
    page.getByTestId("workspace-context-dropdown").locator("span"),

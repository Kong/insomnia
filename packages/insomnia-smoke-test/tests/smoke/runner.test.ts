...
test.describe("runner features tests", async () => {
  ...
  const verifyResultRows = async (
    ...
    const testResults = page
      .getByTestId(`runner-test-result-iteration-${iteration}`)
      .getByTestId("test-result-row");
    ...
    expect(passedResultCount + failedResultCount + skippedResultCount).toEqual(
      expectedTotal,
    );
  };
  ...
  test("run collection runner", async ({ page }) => {
    ...
    const verifyTestCounts = async (
      ...
      const testResultCounts = await page
        .locator(".test-result-count")
        .allInnerTexts();
  ...
  test("run collection runner with data upload", async ({ page }) => {
    ...
    expect(await page.locator('input[name="Iterations"]').inputValue()).toBe(
      "2",
    );
  ...
  test("run req4 3 times with setNextRequest the pre-request script", async ({
    page,
  }) => {
    await page.getByTestId("run-collection-btn-quick").click();
  ...
  test("run req5 3 times with setNextRequest in the after-response script", async ({
    page,
  }) => {
    await page.getByTestId("run-collection-btn-quick").click();

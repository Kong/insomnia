...
test.describe("Dashboard", async () => {
  ...
  test.describe("Interactions", async () => {
    ...
    test("Can create, rename and delete a document", async ({ page }) => {
      ...
      await page
        .getByRole("menuitemradio", { name: "Design Document" })
        .click();
      await page.getByRole("button", { name: "Create", exact: true }).click();
      ...
      await page
        .getByLabel("Files")
        .getByLabel("My Design Document")
        .getByRole("button")
        .click();
      await page.getByRole("menuitem", { name: "Rename" }).click();
      await page
        .locator('text=Rename DocumentName Rename >> input[type="text"]')
        .fill("test123");
    ...
    test("Can create, rename and delete a collection", async ({ page }) => {
      ...
      await page.getByRole("button", { name: "Create in project" }).click();
      await page
        .getByRole("menuitemradio", { name: "Request Collection" })
        .click();
      await page.getByRole("button", { name: "Create", exact: true }).click();
      ...
      await page.getByRole("menuitem", { name: "Rename" }).click();
      await page
        .locator('text=Rename CollectionName Rename >> input[type="text"]')
        .fill("test123");
      await page.click('#root button:has-text("Rename")');
      await expect(page.locator(".app")).toContainText("test123");

...
test("Can invite users in app", async ({ page }) => {
  ...
  await expect(
    page.getByLabel("Invitation list").getByRole("option"),
  ).toHaveCount(15);

  // Change the role
  await page
    .getByTestId("collaborator-test-iteration-2")
    .getByLabel("Menu")
    .click();
  ...
  // Delete the member
  await page
    .getByTestId("collaborator-test-iteration-3")
    .getByRole("button")
    .nth(2)
    .click();
  ...
  // Unlink the team
  await page
    .getByTestId("collaborator-test-iteration-0")

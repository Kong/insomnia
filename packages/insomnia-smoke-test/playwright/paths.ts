...
export const getFixturePath = (fixturePath: string) =>
...
export const copyFixtureDatabase = async (
  fixturePath: string,
  dataPath: string,
) => {
...
export const randomDataPath = () =>
  path.join(os.tmpdir(), "insomnia-smoke-test", `${uuidv4()}`);
...
if (typeof platformPath === "string") {
  ...
  throw new Error(
    `Cannot find binary path for ${process.platform} ${process.arch}`,
  );
}
...
export const executablePath =
  bundleType() === "package" ? insomniaBinary : electronBinary;

// NOTE: main.min.js is built by app-build in /build and also by the watcher in /src
export const mainPath = path.join(
  bundleType() === "dev" ? "src" : "build",
  "main.min.js",
);

...
export const test = baseTest.extend<{
  ...
  app: async ({ playwright, trace, dataPath, userConfig }, use, testInfo) => {
    ...
    const options: EnvOptions = {
      ...
      ...(userConfig.session
        ? { INSOMNIA_SESSION: JSON.stringify(userConfig.session) }
        : {}),
    };
    ...
    const traceMode: TraceMode =
      typeof trace === "string" ? (trace as TraceMode) : trace.mode;
    ...
    const traceOptions =
      typeof trace === "string"
        ? defaultTraceOptions
        : { ...defaultTraceOptions, ...trace, mode: undefined };
    const captureTrace =
      traceMode === "on" ||
    ...
    try {
      ...
      if (
        ...
        await appContext.tracing.stop({
          path: path.join(
            testInfo.outputDir,
            `trace-${testInfo.title}-${testInfo.status}.zip`,
          ),
        });
      } else {

...
import {
  Collaborator,
  CollaboratorType,
} from "../../insomnia/src/ui/routes/invite";
import {
  getRandomId,
  getTeamName,
  getUserEmail,
} from "../tests/smoke/test-utils";
...
const orgInfo = {
  ...
  branding: {
    logo_url:
      "https://d2evto68nv31gd.cloudfront.net/org_98e187f8-a753-4abf-b0b2-58cdb852eba6",
  },
  metadata: {
    organizationType: "team",
...
collaboratorsList.total =
  collaboratorsList.collaborators.length + emailsAndGroupsToInvite.length;
...
export default (app: Application) => {
  ...
  app.delete(
    "/v1/organizations/:orgId/team-projects/:projectId",
    json(),
    (_req, res) => {
      const projects = projectsByOrgId
        .get(_req.params.orgId)
        ?.filter((project) => project.id !== _req.params.projectId);
      if (!projects) {
        res.status(500).send();
        return;
      }
  ...
  app.patch(
    ...
    json(),
    (_req, res) => {
      const updatedProjects = projectsByOrgId
        .get(_req.params.orgId)
        ?.map((project) => {
      ...
      updatedProjects &&
        projectsByOrgId.set(_req.params.orgId, updatedProjects);
      res.status(200).send();
    },
  );

  app.post(
    "/v1/organizations/:organizationId/team-projects",
    json(),
    (_req, res) => {
      ...
      if (organizationId === "personal") {
        const personalOrg = organizations.find(
          (org) => org.metadata.organizationType === "personal",
        );
  ...
  app.get(
    "/v1/organizations/:organizationId/members/:accountId/roles",
    (_req, res) => {
  ...
  app.get(
    "/v1/desktop/organizations/:organizationId/collaborators",
    (_req, res) => {
  ...
  app.post(
    "/v1/desktop/organizations/:organizationId/collaborators/start-adding",
    (_req, res) => {
      res.json({
  ...
  app.post(
    "/v1/desktop/organizations/:organizationId/collaborators/finish-adding",
    (_req, res) => {
      res.json(null);
    },
  );
  ...
  app.get(
    "/v1/desktop/organizations/:organizationId/collaborators/search/*",
    (_req, res) => {
  ...
  app.post(
    "/v1/organizations/:organizationId/invites/:invitationId/reinvite",
    (_req, res) => {
  ...
  app.patch(
    "/v1/organizations/:organizationId/invites/:invitationId",
    (_req, res) => {
      res.json({ enabled: true });
    },
  );
  ...
  app.delete(
    "/v1/desktop/organizations/:organizationId/collaborators/:collaboratorId/unlink",
    (_req, res) => {
      res.json(null);
    },
  );

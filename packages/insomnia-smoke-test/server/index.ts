...
app.get("/events", (request, response) => {
  ...
  setInterval(() => {
    ...
    const data = JSON.stringify({
      message: "Time: " + new Date().toISOString().slice(11, 19),
    });
...
app.post("/send-event", (request, response) => {
  ...
  subscribers.forEach((subscriber) =>
    subscriber.response.write(`data: ${JSON.stringify(request.body)}\n\n`),
  );
...
startWebSocketServer(
  createServer(
    {
      cert: readFileSync(
        join(__dirname, "../fixtures/certificates/localhost.pem"),
      ),
      key: readFileSync(
        join(__dirname, "../fixtures/certificates/localhost-key.pem"),
      ),

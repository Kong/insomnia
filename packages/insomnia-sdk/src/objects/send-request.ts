    ...
    try {
      const nodejsCurlRequest =
        process.type === "renderer"
          ...
          : (await import("insomnia/src/main/network/libcurl-promise"))
              .curlRequest;
...
function requestToCurlOptions(
  req: string | Request | RequestOptions,
  settings: Settings,
) {
  const id = uuidv4();
  const settingFollowRedirects: "global" | "on" | "off" =
    settings.followRedirects ? "on" : "off";
  ...
  if (typeof req === "string") {
    ...
    const urlencodedParams = finalReq.body?.urlencoded
      ?.all()
      .map((param) => ({ name: param.key, value: param.value }));
    ...
    return {
      ...
      req: {
        headers: finalReq.headers.map(
          (header) => ({ name: header.key, value: header.value }),
          {},
        ),
        ...
        body: {
          ...
          params,
          fileName:
            finalReq.body?.mode === "file"
              ? finalReq.body?.toString()
              : undefined,
        },

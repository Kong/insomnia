...
function canNotBeModifiedWarning(originalUrl: string | undefined) {
  getExistingConsole().warn(
    `The url "${originalUrl || "undefined"}" can not be parsed, only 'insomnia.request.url.update(..)' will take effect.`,
  );
}
...
export class QueryParam extends Property {
  ...
  static parse(queryStr: string) {
    ...
    return Array.from(params.entries()).map((entry) => ({
      key: entry[0],
      value: entry[1],
    }));
  }
  ...
  static unparse(params: QueryParamOptions[] | Record<string, string>) {
    ...
    if (Array.isArray(params)) {
      params.forEach((entry: QueryParamOptions) =>
  ...
  update(param: string | { key: string; value: string; type?: string }) {
    if (typeof param === "string") {
      ...
      throw Error(
        "the param for update must be: string | { key: string; value: string }",
      );
    }
  }
}
...
export class Url extends PropertyBase {
  ...
  get query(): PropertyList<QueryParam> {
    const queryList = this.urlObject?.searchParams
      ? Array.from(this.urlObject.searchParams.entries()).map(
          (queryEntry) =>
            new QueryParam({ key: queryEntry[0], value: queryEntry[1] }),
          {},
        )
      : [];
    return new PropertyList<QueryParam>(QueryParam, undefined, queryList);
  }
  ...
  private initFields(urlOptions: UrlOptions | string | undefined) {
    if (typeof urlOptions === "string") {
      ...
      const protocolStr = (urlOptions.protocol || "").trim()
        ? urlOptions.protocol.trim()
        : "https://";
      const authStr = urlOptions.auth
        ? `${urlOptions.auth.username}:${urlOptions.auth.password}@`
        : "";
  ...
  static parse(urlStr: string): UrlOptions | undefined {
    if (URL.canParse(urlStr)) {
      ...
      const auth =
        urlObject.username === ""
          ? undefined
          : { username: urlObject.username, password: urlObject.password };
      const query = Array.from(urlObject.searchParams.entries()).map(
  ...
  addQueryParams(params: { key: string; value: string }[] | string) {
    if (this.urlObject !== undefined) {
      if (typeof params === "string") {
        ...
        throw Error(
          `addQueryParams: failed to add params: ${JSON.stringify(params)}`,
        );
      }
    } else {
...
> extends PropertyList<T> {
  ...
  test(urlStr: string) {
    return (
      this.filter((matchPattern) => matchPattern.test(urlStr), {}).length > 0
    );
  }
}

import type {
  Cookie as InsomniaCookie,
  CookieJar as InsomniaCookieJar,
} from "insomnia/src/models/cookie-jar";
...
export class Cookie extends Property {
  ...
  static parse(cookieStr: string) {
    ...
    if (hostOnly) {
      cookieObj.extensions =
        cookieObj.extensions?.filter((item) => item !== "HostOnly") || [];
    }
    if (session) {
      cookieObj.extensions =
        cookieObj.extensions?.filter((item) => item !== "Session") || [];
    }

    // Tough Cookies extensions works well with string[], but not {key: string; value: string}[]
    const extensions = cookieObj.extensions?.map(
      (entry: string | { key: string; value: string }) => {
        if (typeof entry === "string") {
          const equalPos = entry.indexOf("=");
          if (equalPos > 0) {
            return {
              key: entry.slice(0, equalPos),
              value: entry.slice(equalPos + 1),
            };
          }
  ...
  override toString = () => {
    ...
    const sessionPart = this.cookie.extensions?.includes("session")
      ? "; Session"
      : "";
    const extensionPart =
  ...
  override toJSON = () => {
    return {
      ...
      expires:
        this.cookie.expires === "Infinity" ? undefined : this.cookie.expires,
...
export class CookieObject extends CookieList {
  ...
  constructor(cookieJar: InsomniaCookieJar | null) {
    ...
    super(cookies);
    const scriptCookieJar = cookieJar
      ? new CookieJar(cookieJar.name, cookies)
      : new CookieJar("", []);
...
export class CookieJar {
      ...
      cookies.forEach((cookie) => {
        ...
        if (!properties.domain) {
          getExistingConsole().warn(
            `domain is not specified for the cookie "${cookie.key}" so it is omitted`,
          );
          return;
        }
        ...
        this.jar.set(
          properties.domain,
          domainCookies.set(properties.key, cookie),
        );
      });
    }
  }
  ...
  toInsomniaCookieJar() {
    ...
    Array.from(this.jar.values()).forEach(
      (domainCookies: Map<string, Cookie>) => {
...
export function mergeCookieJar(
  ...
  const cookiesWithId = updatedCookieJar.cookies.map(
    (cookie): InsomniaCookie => {
      if (!cookie.id) {

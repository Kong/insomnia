...
export class ParentFolders {
  ...
  get = (idOrName: string) => {
    const folder = this.folders.find(
      (folder) => folder.name === idOrName || folder.id === idOrName,
    );
    if (!folder) {
  ...
  findValue = (valueKey: string) => {
    const targetEnv = [...this.folders]
      .reverse()
      .find((folder) => folder.environment.has(valueKey));
    return targetEnv !== undefined
      ? targetEnv.environment.get(valueKey)
      : undefined;
  };

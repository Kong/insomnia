import {
  ...
  init as initClientCertificate,
} from "insomnia/src/models/client-certificate";
import type {
...
function requestOptionsToClassFields(options: RequestOptions) {
  ...
  if (options.header != null) {
    if (Array.isArray(options.header)) {
      ...
      headers = new HeaderList(
        undefined,
        Object.entries(options.header).map(
          (entry) => new Header({ key: entry[0], value: entry[1] }),
        ),
      );
    }
  } else {
  ...
  const pathParameters = options.pathParameters
    ...
    : new Array<RequestPathParameter>();
...
export class Request extends Property {
  ...
  authorizeUsing(
    authType: AuthOptionTypes | AuthOptions,
    options?: VariableList<Variable>,
  ) {
    const selectedAuth =
  ...
  getHeaders(options?: {
    ...
    this.headers.each((header) => {
      ...
      const enabled = options?.enabled
        ? header.disabled == null || !header.disabled
        : true;
  ...
  override toJSON() {
    return {
      ...
      body: {
        mode: this.body?.mode,
        file: this.body?.file,
        formdata: this.body?.formdata?.map(
          (formParam) => formParam.toJSON(),
          {},
        ),
        graphql: this.body?.graphql,
        raw: this.body?.raw,
        urlencoded: this.body?.urlencoded?.map(
      ...
      certificate: this.certificate
        ? {
            ...
            matches: this.certificate?.matches?.map(
...
export function toScriptRequestBody(
  insomniaReqBody: InsomniaRequestBody,
): RequestBodyOptions {
  ...
  if (insomniaReqBody.text !== undefined) {
    ...
    insomniaReqBody.fileName !== undefined &&
    insomniaReqBody.fileName !== ""
  ) {
    reqBodyOpt = {
      mode: "file",
    ...
    reqBodyOpt = {
      mode: "urlencoded",
      urlencoded: insomniaReqBody.params.map((param: RequestBodyParameter) => ({
        key: param.name,
        value: param.value,
        type: param.type,
...
export function calculateHeadersSize(headers: HeaderList<Header>): number {
  const headerSize = new Blob([
    headers.reduce((acc, header) => acc + header.toString() + "\n", "", {}),
  ]).size;

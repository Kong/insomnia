...
describe("test Variables object", () => {
  ...
  it("test environment overriding", () => {
    ...
    const variablesWithFolderLevelData = new Variables({
      ...
      folderLevelVars: [
        new Environment("folderLevel1", {
          scope: "folderLevel1",
          value: "folderLevel1-value",
        }),
        new Environment("folderLevel2", {
          scope: "folderLevel2",
          value: "folderLevel2-value",
        }),
      ],
    ...
    expect(variablesWithIterationData.get("value")).toEqual(
      "iterationData-value",
    );
    expect(variablesWithFolderLevelData.get("value")).toEqual(
      "folderLevel2-value",
    );
    expect(variablesWithLocalData.get("value")).toEqual("local-value");

    expect(variablesWithFolderLevelData.replaceIn("{{ value}}")).toEqual(
      "folderLevel2-value",
    );
  });

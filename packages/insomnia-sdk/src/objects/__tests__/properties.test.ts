...
describe("test Property objects", () => {
  ...
  it("Property: basic operations", () => {
    const prop = new Property("id", "name", false, {
      id: "real_id",
      name: "real_name",
    });
    ...
    expect(Property.replaceSubstitutions("{{ hehe }}", { hehe: 777 })).toEqual(
      "777",
    );
    expect(
  ...
  it("PropertyList: basic operations: add, append, count, all, clear", () => {
    const propList = new PropertyList({}, undefined, [
      new Property("id1", "p1"),
    ]);
  ...
  it("PropertyList: basic operations: assimilate, each, filter, find", () => {
    ...
    propList.assimilate(
      [new Property("id1", "p1"), new Property("id2", "p2")],
      false,
    );
    ...
    expect(
      propList.find((prop) => prop?.name === "p2", {}) != null,
    ).toBeTruthy();
  });
  ...
  it("PropertyList: basic operations: one, has, indexOf, insert, insertAfter, prepend, populate, map, reduce", () => {
    ...
    expect(propList.reduce((acc, prop) => (acc += prop.id), "", {})).toEqual(
      "id0id1id2",
    );
  });

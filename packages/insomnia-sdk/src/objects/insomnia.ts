  ...
  constructor(rawObj: {
    ...
    return new Proxy(this, {
      get: (target, prop, receiver) => {
        if (prop === "test") {
          const testHandler: TestHandler = async (
            msg: string,
            fn: () => Promise<void>,
          ) => {
...
export async function initInsomniaObject(
  ...
  const baseEnvironment = new Environment(
    rawObj.baseEnvironment.name || "",
    rawObj.baseEnvironment.data,
  );
  ...
  const localVariables = rawObj.transientVariables
    ? new Environment(
        rawObj.transientVariables.name,
        rawObj.transientVariables.data,
      )
  ...
  const parentFolders = new ParentFolders(
    rawObj.parentFolders.map(
      (folderObj) =>
        new Folder(folderObj.id, folderObj.name, folderObj.environment),
    ),
  );
  ...
  const matchedCertificates = filterClientCertificates(
  ...
  const response = rawObj.response
    ? toScriptResponse(request, rawObj.response, responseBody)
    : undefined;

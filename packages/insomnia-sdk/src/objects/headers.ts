...
export class Header extends Property {
  ...
  static create(
    input?: { key: string; value: string } | string,
    name?: string,
  ): Header {
  ...
  static unparse(
    headers: { key: string; value: string }[] | PropertyList<Header>,
    separator?: string,
  ): string {
    const headerArray: { key: string; value: string }[] = [
      ...headers.map((header) => this.unparseSingle(header), {}),
    ];

    return headerArray.join(separator || "\n");
  }

  static unparseSingle(

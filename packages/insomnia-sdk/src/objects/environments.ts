...
export class Environment {
  ...
  set = (
    variableName: string,
    variableValue: boolean | number | string | undefined | null,
  ) => {
...
function mergeFolderLevelVars(folderLevelVars: Environment[]) {
  const mergedFolderLevelObject = folderLevelVars.reduce(
    (merged: object, folderLevelEnv: Environment) => {
...
export class Variables {
  ...
  has = (variableName: string) => {
    ...
    const folderLevelVarsHas = this.folderLevelVars.some((vars) =>
      vars.has(variableName),
    );

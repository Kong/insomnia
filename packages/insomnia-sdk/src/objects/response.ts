...
export class Response extends Property {
  ...
  constructor(options: ResponseOptions) {
    ...
    const detectedStatus =
      options.reason || RESPONSE_CODE_REASONS[options.code];
    if (!detectedStatus) {
      throw Error(
        "Response constructor: reason or code field must be set in the options",
      );
    } else {
  ...
  contentInfo(): ResponseContentInfo {
    ...
    const contentType = this.headers.find(
    ...
    const contentDisposition = this.headers.find(
  ...
  get to() {
    ...
    const haveHeader = (expected: string, checkEquality: boolean) =>
      verify(
        this.headers.toObject().find((header) => header.key === expected) !==
          undefined,
        checkEquality,
      );
    const haveBody = (expected: string, checkEquality: boolean) =>
      verify(this.text(), expected, checkEquality);
    const haveJsonBody = (expected: object, checkEquality: boolean) =>
      verify(this.json(), expected, checkEquality);
    const haveJsonSchema = (expected: object, checkEquality: boolean) => {
      ...
      try {
        ...
        throw Error(
          `Failed to verify response body schema, response could not be a valid json: "${e}"`,
        );
      }
...
export function toScriptResponse(
  ...
  const partialResponse =
    partialInsoResponse as sendCurlAndWriteTimelineResponse;
...
export async function readBodyFromPath(
  ...
  const nodejsReadCurlResponse =
    process.type === "renderer"
      ? window.bridge.readCurlResponse
      : readCurlResponse;

...
function matchesHostname(hostname: string, noProxyZoneHostname: string) {
  const wildcardNeedle = noProxyZoneHostname.startsWith(".*.")
    ? noProxyZoneHostname.slice(2)
    : noProxyZoneHostname;
  const isMatchedAt = hostname.indexOf(wildcardNeedle);
  return (
    isMatchedAt > -1 && isMatchedAt === hostname.length - wildcardNeedle.length
  );
}

export function isUrlMatchedInNoProxyRule(

...
import {
  ...
  type TransformedExecuteScriptContext,
} from "./concurrency";
...
export const getOrInheritAuthentication = ({
  ...
  const hasValidAuth =
    getAuthObjectOrNull(request.authentication) &&
    isAuthEnabled(request.authentication);
  if (hasValidAuth) {
  ...
  const closestParentFolderWithAuth = requestGroups.find(
    ({ authentication }) =>
      getAuthObjectOrNull(authentication) && isAuthEnabled(authentication),
  );
  const closestAuth = getAuthObjectOrNull(
    closestParentFolderWithAuth?.authentication,
  );
...
export function getOrInheritHeaders({
  ...
  headers.forEach(({ name, value, disabled }) => {
    ...
    if (httpHeaders.has(normalizedCase)) {
      httpHeaders.set(
        normalizedCase,
        `${httpHeaders.get(normalizedCase)}, ${value}`,
      );
      return;
    }
    httpHeaders.set(normalizedCase, value);
...
export const fetchRequestGroupData = async (requestGroupId: string) => {
  ...
  const ancestors = await db.withAncestors<
    RequestGroup | Workspace | MockRoute | MockServer
  >(requestGroup, [
  ...
  const responsesDir = pathJoin(
    (process.type === "renderer" ? window : require("electron")).app.getPath(
      "userData",
    ),
    "responses",
  );
...
export const fetchRequestData = async (requestId: string) => {
  ...
  const ancestors = await db.withAncestors<
    Request | RequestGroup | Workspace | Project | MockRoute | MockServer
  >(request, [
    models.request.type,
    models.requestGroup.type,
    models.workspace.type,
  ...
  if (workspaceMeta?.activeGlobalEnvironmentId) {
    activeGlobalEnvironment =
      (await models.environment.getById(
...
export const tryToExecutePreRequestScript = async (
  ...
  const requestGroups = ancestors.filter(
    (doc) => isRequest(doc) || isRequestGroup(doc),
  ) as RequestGroup[];
  ...
  const originalRequestGroups = requestGroups.filter((group) =>
    isRequestGroup(group),
  );
...
export async function savePatchesMadeByScript(
  ...
  if (mutatedContext.cookieJar) {
    ...
    await models.cookieJar.update(mutatedContext.cookieJar, {
      cookies: [
        ...(responseCookies || []),
        ...mutatedContext.cookieJar.cookies,
      ],
    });
  }
  // when base environment is activated, `mutatedContext.environment` points to it
  const isActiveEnvironmentBase =
    mutatedContext.environment?._id === baseEnvironment._id;
  const hasEnvironmentAndIsNotBase =
    mutatedContext.environment && !isActiveEnvironmentBase;
  const updateEnvironment = async (
    originEnvironment: Environment,
    mutatedContextEnvironment: Environment,
  ) => {
  ...
  if (activeGlobalEnvironment && mutatedContext) {
    invariant(
      mutatedContext.globals,
      "globals must be defined when there is selected one",
    );
    await updateEnvironment(activeGlobalEnvironment, mutatedContext.globals);
  }

  mutatedContext.parentFolders.forEach((mutatedFolder) => {
    const originalFolder = originalRequestGroups.find(
      (originalFolder) => originalFolder._id === mutatedFolder.id,
    );
    if (originalFolder) {
...
export const tryToExecuteScript = async (
  ...
  if (
    globals &&
    vaultEnvironmentPath in globals.data &&
    settings.enableVaultInScripts
  ) {
  ...
  try {
    ...
    if ("error" in originalOutput) {
      return {
        error: `Script executor returns error: ${originalOutput.error}`,
      };
    }
    ...
    await fs.promises.appendFile(
      timelinePath,
      serializeNDJSON([
        { value: err.message, name: "Text", timestamp: Date.now() },
      ]),
    );

    const requestId = request._id;
    const errMessage = err.stack
      ? `message: ${err.messsage}; stack: ${err.stack}`
      : err.message;
    const responsePatch = {
      _id: responseId,
      parentId: requestId,
    ...
    const res = await models.response.create(
      responsePatch,
      settings.maxHistoryResponses,
    );
    models.requestMeta.updateOrCreateByParentId(requestId, {
      activeResponseId: res._id,
    });
...
export async function tryToExecuteAfterResponseScript(
  context: RequestAndContextAndResponse,
) {
  const requestGroups = context.ancestors.filter(
    (doc) => isRequest(doc) || isRequestGroup(doc),
  ) as RequestGroup[];
  const folderScripts = requestGroups
    .reverse()
  ...
  const joinedScript = [...folderScripts].join("\n");
  const postMutatedContext = await tryToExecuteScript({
    script: joinedScript,
    ...context,
    eventName: "test",
  });
  ...
  const respondedWithoutError =
    context.response && !("error" in context.response);
...
export async function sendCurlAndWriteTimeline(
  ...
  const requestOptions = {
    ...
    socketPath,
    settings,
    certificates: filterClientCertificates(
      clientCertificates,
      renderedRequest.url,
      "https:",
    ),
  ...
  if ("error" in output) {
    if (runtime) {
      await runtime.appendTimeline(
        timelinePath,
        serializeNDJSON(timeline).split("\n"),
      );
    }
  ...
  rejectedCookies.forEach((errorMessage) =>
    timeline.push({
      value: `Rejected cookie: ${errorMessage}`,
      name: "Text",
      timestamp: Date.now(),
    }),
  );
  if (totalSetCookies) {
    ...
    timeline.push({
      value: `Saved ${totalSetCookies} cookies`,
      name: "Text",
      timestamp: Date.now(),
    });
  }
...
export const responseTransform = async (
  ...
  if (response.error) {
    console.log(
      `[network] Response failed req=${patch.parentId} err=${response.error || "n/a"}`,
    );
    return response;
  }
  console.log(
    `[network] Response succeeded req=${patch.parentId} status=${response.statusCode || "?"}`,
  );
...
export const transformUrl = (
  ...
  const customUrl = joinUrlAndQueryString(
    url,
    buildQueryStringFromParams(
      authQueryParam ? params.concat([authQueryParam]) : params,
      true,
  ...
  if (!isUnixSocket) {
    return {
      finalUrl: smartEncodeUrl(customUrl, shouldEncode, {
        strictNullHandling: true,
      }),
    };
  }
  // URL prep will convert "unix:/path" hostname to "unix/path"
  const match = smartEncodeUrl(customUrl, shouldEncode, {
    strictNullHandling: true,
  }).match(/(https?:)\/\/unix:?(\/[^:]+):\/(.+)/);
...
const extractCookies = async (
  ...
  if (settingStoreCookies) {
    ...
    const redirects: string[][] = headerResults.map(({ headers }: any) =>
      getSetCookiesFromResponseHeaders(headers),
    );

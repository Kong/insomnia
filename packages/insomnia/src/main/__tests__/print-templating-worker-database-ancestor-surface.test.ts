...
import { describeAncestorSurface, formatAncestorSurfaceEntries } from '../templating-worker-database-ancestor-surface';
...
// Not a regression check — prints the ancestor-check surface for eyeballing. Run via
// `npm run sandbox:ancestor`.
it('prints the ancestor-check surface', async () => {
  const { pluginToMainAPI } = await import('../templating-worker-database');
  const entries = describeAncestorSurface(pluginToMainAPI);
  const lines = formatAncestorSurfaceEntries(entries);

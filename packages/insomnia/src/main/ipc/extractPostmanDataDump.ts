...
export default async function extractPostmanDataDumpHandler(
  _event: unknown,
  dataDumpFilePath: string,
) {
  ...
  const archiveJsonFile = files.find(
    (file) => path.basename(file.path) === "archive.json",
  );
  if (!archiveJsonFile) {
    return {

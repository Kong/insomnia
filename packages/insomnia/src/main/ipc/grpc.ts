...
export function registergRPCHandlers() {
  ...
  ipcMainHandle("grpc.loadMethodsFromReflection", (_, requestId) =>
    loadMethodsFromReflection(requestId),
  );
}
...
const loadMethodsFromFilePath = async (
  filePath: string,
  includeDirs: string[],
): Promise<MethodDefs[]> => {
  try {
...
export const getMethodsFromPackageDefinition = (
...
): definition is ServiceDefinition => {
  return !!asServiceDefinition(definition);
};
const asServiceDefinition = (
  definition: AnyDefinition,
...
const isMessageDefinition = (
  definition: AnyDefinition,
): definition is MessageTypeDefinition => {
  return (
    (definition as MessageTypeDefinition).format ===
    "Protocol Buffer 3 DescriptorProto"
  );
};
const isEnumDefinition = (
  definition: AnyDefinition,
): definition is EnumTypeDefinition => {
  return (
    (definition as EnumTypeDefinition).format ===
...
export const start = (event: IpcMainEvent, ipcParams: GrpcIpcRequestParams) => {
  const { request, rejectUnauthorized, clientCert, clientKey, caCertificate } =
    ipcParams;
    ...
    ?.then((method) => {
      if (!method) {
        event.reply(
          ...
          request._id,
          new Error(
            `The gRPC method ${request.protoMethodName} could not be found`,
          ),
        );
        return;
      }
      ...
      try {
        ...
        switch (methodType) {
          case "unary":
            ...
            unaryCall.on("status", (status: StatusObject) =>
              event.reply("grpc.status", request._id, status),
            );
          ...
          case "client":
            ...
            clientCall.on("status", (status: StatusObject) =>
              event.reply("grpc.status", request._id, status),
            );
...
export const commit = (requestId: string): void =>
  grpcCalls.get(requestId)?.end();
export const cancel = (requestId: string): void =>
  grpcCalls.get(requestId)?.cancel();
...
const onStreamingResponse = (
  ...
  call.on("status", (status: StatusObject) =>
    event.reply("grpc.status", requestId, status),
  );
  ...
  call.on("end", () => {
    ...
    if (channel) {
      ...
      console.log(
        `[gRPC] failed to close channel for req=${requestId} because it was not found`,
      );
    }
    grpcCalls.delete(requestId);
  });
...
const onUnaryResponse =
  (event: IpcMainEvent, requestId: string) =>
  (err: ServiceError | null, value?: Record<string, any>) => {
    if (!err) {
...
const filterDisabledOrInvalidMetaData = (
  metadata: GrpcRequestHeader[],
): Metadata => {

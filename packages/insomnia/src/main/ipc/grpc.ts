...
const getMethodsFromReflectionServer = async (reflectionApi: GrpcRequest['reflectionApi']): Promise<MethodDefs[]> => {
  const { url, module: refApiModule, apiKey } = reflectionApi;
  ...
  try {
    const response = await fetch(endpoint, {
      method: 'POST',
      headers,
      body: JSON.stringify({ module: refApiModule }),
    });
    const fallback = new ConnectError(`HTTP ${response.status}`, codeFromHttpStatus(response.status), response.headers);
    if (!response.ok) {
    ...
    return getMethodsFromPackageDefinition(protoLoader.loadFileDescriptorSetFromBuffer(
      Buffer.from(
        toBinary(

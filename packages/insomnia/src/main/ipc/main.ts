...
export function registerMainHandlers() {
  ipcMainOn(
    "addExecutionStep",
    (_, options: { requestId: string; stepName: string }) => {
  ...
  ipcMainHandle(
    ...
    async (_, options: { parentId: string; path: string }) => {
  ...
  ipcMainHandle(
    ...
    (_, options: Parameters<typeof authorizeUserInWindow>[0]) => {
      const { url, urlSuccessRegex, urlFailureRegex, sessionId } = options;
      return authorizeUserInWindow({
        url,
        urlSuccessRegex,
        urlFailureRegex,
        sessionId,
  ...
  ipcMainHandle(
    "writeFile",
    async (_, options: { path: string; content: string }) => {
      try {
  ...
  ipcMainHandle(
    "readFile",
    async (_, options: { path: string; encoding?: string }) => {
      ...
      if (detecedEncoding) {
        ...
        throw new Error(
          `Unsupported encoding: ${detecedEncoding} to read file`,
        );
      }
  ...
  ipcMainHandle(
    "curlRequest",
    (_, options: Parameters<typeof curlRequest>[0]) => {
  ...
  ipcMainOn(
    ...
    (
      _,
      options: { event: SegmentEvent; properties?: Record<string, unknown> },
    ): void => {
  ...
  ipcMainOnce(
    ...
    (
      _,
      {
        landingPage,
        tags = {},
      }: { landingPage: LandingPage; tags?: Record<string, string> },
    ) => {

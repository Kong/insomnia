...
export function registerElectronHandlers() {
    ...
    (
      ...
      try {
        ...
        const localTemplate: MenuItemConstructorOptions[] = localTemplateTags
          // sort alphabetically
          .sort((a, b) =>
            fnOrString(a.templateTag.displayName).localeCompare(
              fnOrString(b.templateTag.displayName),
            ),
          )
          .map((l) => {
            const actions = l.templateTag.args?.[0];
            const needsEnterprisePlan =
              l.templateTag.needsEnterprisePlan || false;
  ...
  ipcMainOn(
    "getPath",
    (event, name: Parameters<(typeof Electron.app)["getPath"]>[0]) => {
    ...
    (
      ...
      const menuItems = options.menuItems.map((item) => {
        return {
          ...item,
          click: () => {
            event.sender.send("contextMenuCommand", {
              key: options.key,
              label: item.label,
              extra: options.extra,
            });

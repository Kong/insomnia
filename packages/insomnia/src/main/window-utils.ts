...
const stopAndWaitForHiddenBrowserWindow = async (
  runningHiddenBrowserWindow: BrowserWindow,
) => {
  return await new Promise<void>((resolve) => {
    // overwrite the closed handler
    runningHiddenBrowserWindow.on("closed", () => {
      console.log(
        "[main] restarting hidden browser window:",
        runningHiddenBrowserWindow.id,
      );
...
export async function createHiddenBrowserWindow() {
  ...
  ipcMain.handle(
    ...
    async (event, isPortAlive: boolean) => {
      const runningHiddenBrowserWindow = browserWindows.get(
        "HiddenBrowserWindow",
      );
      ...
      hiddenBrowserWindow.webContents.postMessage("renderer-listener", null, [
        port1,
      ]);
      ...
      event.senderFrame?.postMessage(
        "hidden-browser-window-response-listener",
        null,
        [port2],
      );
...
export function createWindow({
  firstLaunch,
}: { firstLaunch?: boolean } = {}): ElectronBrowserWindow {
  ...
  if (firstLaunch) {
    ...
    Sentry.metrics.distribution(
      SentryMetrics.MAIN_PROCESS_START_DURATION,
      duration,
      {
        unit: "millisecond",
      },
...
export function initElectronStorage() {
  const electronStoragePath = path.join(
    process.env["INSOMNIA_DATA_PATH"] || app.getPath("userData"),
    "localStorage",
  );
  if (!electronStorage) {
...
export function createWindowsAndReturnMain({
  ...
  const mainWindow =
    browserWindows.get("Insomnia") ?? createWindow({ firstLaunch });
  if (!browserWindows.get("HiddenBrowserWindow")) {
    createHiddenBrowserWindow();
  }
  return mainWindow;

...
export const resolveDbByKey = async (request: Request) => {
  ...
  if (url.host === "request.getAncestors".toLowerCase()) {
    result = await db.withAncestors<DBRequest | RequestGroup | Workspace>(
      body.request,
      body.types,
    );
  }
  ...
  if (url.host === "response.getLatestForRequestId".toLowerCase()) {
    result = await models.response.getLatestForRequest(
      body.requestId,
      body.environmentId,
    );
  }
  if (url.host === "response.getBodyBuffer".toLowerCase()) {
    result = await models.response.getBodyBuffer(
      body.response,
      body.readFailureValue,
    );
  }

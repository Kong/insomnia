...
const getDeviceId = async () => {
  ...
  return (
    settings.deviceId ||
    (await models.settings.update(settings, { deviceId: uuidv4() })).deviceId
  );
};
...
export async function trackSegmentEvent(
  ...
  if (!userSession?.hashedAccountId) {
    userSession.hashedAccountId = userSession?.accountId
      ? hashString(userSession.accountId)
      : "";
  }
  const allowAnalytics =
    settings.enableAnalytics || userSession?.hashedAccountId;
  if (allowAnalytics) {
    try {
      ...
      console.warn(
        "[analytics] Unexpected error while sending segment event",
        error,
      );
    }
  }
...
export async function trackPageView(name: string) {
  ...
  if (allowAnalytics) {
    try {
      ...
      analytics.page(
        { name, context, anonymousId, userId: userSession?.hashedAccountId },
        (error) => {
          if (error) {
            console.warn("[analytics] Error sending segment event", error);
          }

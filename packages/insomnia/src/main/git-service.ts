...
> => {
  ...
  const files = await Promise.all(
    insomniaFiles.map(async file => {
      ...
      // Failed to parse the file, most probably due to file generated by a newer version of Insomnia
      console.warn(
        `Failed to parse ${file}: ${fileSchemaParser.error.message}. This file maybe generated by a newer version of Insomnia.`,
      );
      return {
        name: 'Untitled',
        path: file,
  ...
  return {
    files: files.filter(f => f.scope) as {
      scope: WorkspaceScope;
      name: string;
      path: string;
    }[],
    unsupportedFiles: files.filter(f => !f.scope),
  };
};

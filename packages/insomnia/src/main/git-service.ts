...
export const getGitBranches = async ({
  ...
  try {
    ...
    const errorMessage =
      e instanceof Error ? e.message : "Could not fetch remote branches.";
    return {
      errors: [errorMessage],
    };
  }
...
export const gitFetchAction = async ({
  projectId,
  workspaceId,
}: {
  projectId: string;
...
export const gitLogLoader = async ({
  projectId,
  workspaceId,
}: {
  projectId: string;
...
const recursivelyFindInsomniaFiles = async (
  ...
  for (const file of dirFiles) {
    ...
    const isDirectory = (
      await fsClient.promises.stat(repoRelativePath)
    ).isDirectory();
...
> => {
  ...
  const insomniaFiles = await recursivelyFindInsomniaFiles(
    inMemoryFsClient,
    GIT_CLONE_DIR,
  );
  ...
  const files = await Promise.all(
    insomniaFiles.map(async (file) => {
      const fileContents = await inMemoryFsClient.promises.readFile(
        path.join(GIT_CLONE_DIR, file),
        "utf8",
      );
...
export const cloneGitRepoAction = async ({
  ...
  try {
    if (!projectId) {
      ...
      const rootDirFiles: string[] =
        await inMemoryFsClient.promises.readdir(GIT_CLONE_DIR);
      const insomniaFiles = rootDirFiles.filter((fileOrFolder) =>
        fileOrFolder.startsWith("insomnia."),
      );
      ...
      if (insomniaFilesIds.length > 0) {
        ...
        if (existingWorkspaces.length > 0) {
          return {
            errors: [
              "The repository being cloned contains workspaces that already exist in Insomnia.",
            ],
          };
        }
      }
      ...
      const gitRepository =
        await models.gitRepository.create(repoSettingsPatch);
    ...
    const containsInsomniaWorkspaceDir = async (
      ...
      const rootDirs: string[] =
        await fsClient.promises.readdir(GIT_INSOMNIA_DIR);
    ...
    if (!(await containsInsomniaWorkspaceDir(inMemoryFsClient))) {
      ...
      const workspaceJson =
        await inMemoryFsClient.promises.readFile(workspacePath);
      ...
      if (existingWorkspace) {
        const project = await models.project.getById(
          existingWorkspace.parentId,
        );
        if (!project) {
          return {
            errors: [
...
export const updateGitRepoAction = async ({
  ...
  try {
    ...
    if (workspaceId) {
      ...
      const workspaceMeta =
        await models.workspaceMeta.getByParentId(workspaceId);
    ...
    async function setupGitRepository() {
      if (gitRepositoryId) {
        const gitRepository =
          await models.gitRepository.getById(gitRepositoryId);
...
export const resetGitRepoAction = async ({
  projectId,
  workspaceId,
}: {
  projectId: string;
...
export const continueMerge = async ({
  ...
  try {
    ...
    return {
      errors: [
        err instanceof Error ? err.message : "Error while continuing merge",
      ],
    };
  }
};
...
export const discardChangesAction = async ({
  ...
  try {
    ...
    const files = changes.unstaged.filter((change) =>
...
export const unstageChangesAction = async ({
  ...
  try {
    ...
    const files = changes.staged.filter((change) =>
      paths.includes(change.path),
    );
...
const getRepositoryDirectoryTree = async ({
  ...
  const recursivelyGetDirectoryTree = async (
      ...
      directoryContents.map(async (file: string) => {
        ...
        if (await stats.isDirectory()) {
          ...
          return {
            ...
            name: file,
            type: "directory",
            children: await recursivelyGetDirectoryTree(
  ...
  return {
    repositoryTree: {
      id: "",
      name:
        gitRepository.uri.split("/").pop()?.replace(".git", "").toUpperCase() ||
        "Repository",
      type: "root",
      children: tree,
    } satisfies FileTree,
...
async function completeSignInToGitHub({
  ...
  const existingGitHubCredentials =
    await models.gitCredentials.getByProvider("github");
  ...
  const emailsPromise = fetch(getGitHubRestApiUrl() + "/user/emails", {
    ...
    (response) =>
      response.json() as Promise<{ email: string; primary: boolean }[]>,
  );
...
async function getGitHubRepositories({
  ...
  if (!response.ok) {
    ...
    if (response.status === 401) {
      return {
        errors: [
          `User token not authorized to fetch repositories, please sign out and back in.\nResponse: ${raw}`,
        ],
        repos: [],
      };
    }
  ...
  let pullableRepos = data.filter(
    (repo: GitHubRepositoryApiResponse) => repo.permissions.pull,
  );
...
async function getGitHubRepository({ uri }: { uri: string }) {
  ...
  const response = await fetch(
    `${getGitHubRestApiUrl()}/repos/${owner}/${name}`,
    opts,
  );
  if (!response.ok) {
    const raw = await response.text();
    return {
      errors: [
        `Failed to fetch repository from GitHub: ${response.statusText}\nResponse: ${raw}`,
      ],
  ...
  return {
    repo: (await response.json()) as GitHubRepositoryApiResponse,
    errors: [],
    notFound: false,
  };
}
...
const getGitLabConfig = async () => {
  ...
  const configResponse = await fetch(
    getApiBaseURL() + "/v1/oauth/gitlab/config",
    {
      method: "GET",
    },
  );

  const { applicationId: clientId, redirectUri } =
...
export const getGitLabOauthApiURL = () =>
  INSOMNIA_GITLAB_API_URL || "https://gitlab.com";
...
async function completeSignInToGitLab({
  ...
  const gitLabResponse = await fetch(
    getGitLabOauthApiURL() + url.pathname + url.search,
    {
      method: "POST",
    },
  );
  ...
  const existingGitLabCredentials =
    await models.gitCredentials.getByProvider("gitlab");

  const gitLabUserResponse = await fetch(
    `${getGitLabOauthApiURL()}/api/v4/user`,
    {
      headers: {
...
export const registerGitServiceAPI = () => {
  ipcMainHandle(
    ...
    (_, options: Parameters<typeof loadGitRepository>[0]) =>
      loadGitRepository(options),
  );
  ipcMainHandle(
    "git.getGitBranches",
    (_, options: Parameters<typeof getGitBranches>[0]) =>
      getGitBranches(options),
  );
  ipcMainHandle(
    "git.gitFetchAction",
    (_, options: Parameters<typeof gitFetchAction>[0]) =>
      gitFetchAction(options),
  );
  ipcMainHandle(
    "git.gitLogLoader",
    (_, options: Parameters<typeof gitLogLoader>[0]) => gitLogLoader(options),
  );
  ipcMainHandle(
    "git.gitChangesLoader",
    (_, options: Parameters<typeof gitChangesLoader>[0]) =>
      gitChangesLoader(options),
  );
  ipcMainHandle(
    "git.canPushLoader",
    (_, options: Parameters<typeof canPushLoader>[0]) => canPushLoader(options),
  );
  ipcMainHandle(
    "git.cloneGitRepo",
    (_, options: Parameters<typeof cloneGitRepoAction>[0]) =>
      cloneGitRepoAction(options),
  );
  ipcMainHandle(
    "git.initGitRepoClone",
    (_, options: Parameters<typeof initGitRepoCloneAction>[0]) =>
      initGitRepoCloneAction(options),
  );
  ipcMainHandle(
    "git.updateGitRepo",
    (_, options: Parameters<typeof updateGitRepoAction>[0]) =>
      updateGitRepoAction(options),
  );
  ipcMainHandle(
    "git.resetGitRepo",
    (_, options: Parameters<typeof resetGitRepoAction>[0]) =>
      resetGitRepoAction(options),
  );
  ipcMainHandle(
    "git.commitToGitRepo",
    (_, options: Parameters<typeof commitToGitRepoAction>[0]) =>
      commitToGitRepoAction(options),
  );
  ipcMainHandle(
    "git.commitAndPushToGitRepo",
    (_, options: Parameters<typeof commitAndPushToGitRepoAction>[0]) =>
      commitAndPushToGitRepoAction(options),
  );
  ipcMainHandle(
    "git.createNewGitBranch",
    (_, options: Parameters<typeof createNewGitBranchAction>[0]) =>
      createNewGitBranchAction(options),
  );
  ipcMainHandle(
    "git.checkoutGitBranch",
    (_, options: Parameters<typeof checkoutGitBranchAction>[0]) =>
      checkoutGitBranchAction(options),
  );
  ipcMainHandle(
    "git.mergeGitBranch",
    (_, options: Parameters<typeof mergeGitBranch>[0]) =>
      mergeGitBranch(options),
  );
  ipcMainHandle(
    "git.deleteGitBranch",
    (_, options: Parameters<typeof deleteGitBranchAction>[0]) =>
      deleteGitBranchAction(options),
  );
  ipcMainHandle(
    "git.pushToGitRemote",
    (_, options: Parameters<typeof pushToGitRemoteAction>[0]) =>
      pushToGitRemoteAction(options),
  );
  ipcMainHandle(
    "git.pullFromGitRemote",
    (_, options: Parameters<typeof pullFromGitRemote>[0]) =>
      pullFromGitRemote(options),
  );
  ipcMainHandle(
    "git.continueMerge",
    (_, options: Parameters<typeof continueMerge>[0]) => continueMerge(options),
  );
  ipcMainHandle(
    "git.discardChanges",
    (_, options: Parameters<typeof discardChangesAction>[0]) =>
      discardChangesAction(options),
  );
  ipcMainHandle(
    "git.gitStatus",
    (_, options: Parameters<typeof gitStatusAction>[0]) =>
      gitStatusAction(options),
  );
  ipcMainHandle(
    "git.stageChanges",
    (_, options: Parameters<typeof stageChangesAction>[0]) =>
      stageChangesAction(options),
  );
  ipcMainHandle(
    "git.unstageChanges",
    (_, options: Parameters<typeof unstageChangesAction>[0]) =>
      unstageChangesAction(options),
  );
  ipcMainHandle(
    "git.diffFileLoader",
    (_, options: Parameters<typeof diffFileLoader>[0]) =>
      diffFileLoader(options),
  );
  ipcMainHandle(
    "git.getRepositoryDirectoryTree",
    (_, options: Parameters<typeof getRepositoryDirectoryTree>[0]) =>
  ...
  ipcMainHandle(
    ...
    (_, options: Parameters<typeof completeSignInToGitLab>[0]) =>

...
const parseHeadersAndBuildTimeline = (
  url: string,
  headersWithStatus: HeaderInfo,
) => {
...
const openCurlConnection = async (
  ...
  if (existingConnection) {
    console.warn(
      "Connection still open to " +
        existingConnection.getInfo(Curl.info.EFFECTIVE_URL),
    );
    return;
  }
  ...
  try {
    ...
    const { curl, debugTimeline } = createConfiguredCurlInstance({
      req: {
        ...
        cookies: [],
        suppressUserAgent: options.suppressUserAgent,
      },
    ...
    CurlConnections.get(options.requestId)?.setOpt(
    ...
    CurlConnections.get(options.requestId)?.setOpt(
      Curl.option.DEBUGFUNCTION,
      (infoType, buffer) => {
        const isSSLData =
          infoType === CurlInfoDebug.SslDataIn ||
          infoType === CurlInfoDebug.SslDataOut;
        const isEmpty = buffer.length === 0;
        // Don't show cookie setting because this will display every domain in the jar
        const isAddCookie =
          infoType === CurlInfoDebug.Text &&
          buffer.toString("utf8").indexOf("Added cookie") === 0;
        ...
        if (isRequestData) {
          ...
          const isLessThan10KB =
            buffer.length / 1024 < (settings.maxTimelineDataSizeKB || 1);
          timelineMessage = isLessThan10KB
            ? buffer.toString("utf8")
            : `(${describeByteSize(buffer.length)} hidden)`;
        }
    ...
    CurlConnections.get(options.requestId)?.on(
      ...
      async (
        ...
        const {
          ...
          statusMessage,
          httpVersion,
        } = parseHeadersAndBuildTimeline(options.url, headersWithStatus);
        ...
        if (request.settingStoreCookies) {
          ...
          if (totalSetCookies) {
            ...
            const hasCookiesToPersist =
              totalSetCookies > rejectedCookies.length;
        ...
        invariant(
          eventLogFileStreams.get(request._id),
          "writableStream should be defined",
        );
...
const deleteRequestMaps = async (
  requestId: string,
  message: string,
  event?: CurlCloseEvent | CurlErrorEvent,
) => {
  if (event) {
...
const getCurlReadyState = async (options: {
  requestId: string;
...
const closeCurlConnection = (
  ...
  const statusCode = +(
    CurlConnections.get(options.requestId)?.getInfo(
      Curl.info.HTTP_CONNECTCODE,
    ) || 0
  );
  const closeEvent: CurlCloseEvent = {
    _id: uuidV4(),
...
const closeAllCurlConnections = (): void =>
  CurlConnections.forEach((curl) => curl.isOpen && curl.close());
...
export const registerCurlHandlers = () => {
  ...
  ipcMainHandle(
    "curl.readyState",
    (_, options: Parameters<typeof getCurlReadyState>[0]) =>
      getCurlReadyState(options),
  );
  ipcMainHandle(
    "curl.event.findMany",
  ...
  ipcMainHandle(
    "readCurlResponse",
    (_, options: Parameters<typeof readCurlResponse>[0]) =>

...
const parseResponseAndBuildTimeline = (
  url: string,
  ...
  const responseHeaders = Object.entries(incomingMessage.headers).map(
    ([name, value]) => ({ name, value: value?.toString() || "" }),
  );
  const headersIn = responseHeaders
    .map(({ name, value }) => `${name}: ${value}`)
    .join("\n");
  const timeline = [
    {
      value: `Preparing request to ${url}`,
      name: "Text",
      timestamp: Date.now(),
    },
    {
    ...
    {
      value: `HTTP/${httpVersion} ${statusCode} ${statusMessage}`,
      name: "HeaderIn",
      timestamp: Date.now(),
    },
...
const openWebSocketConnection = async (
  ...
  eventLogFileStreams.set(
    options.requestId,
    fs.createWriteStream(responseBodyPath),
  );
  const timelinePath = path.join(responsesDir, responseId + ".timeline");
  timelineFileStreams.set(
    options.requestId,
    fs.createWriteStream(timelinePath),
  );
  ...
  const environment =
    activeEnvironment ||
    (await models.environment.getOrCreateForParentId(options.workspaceId));
  ...
  const caCertificate =
    (caCertficatePath &&
      (await fs.promises.readFile(caCertficatePath)).toString()) ||
  ...
  try {
    ...
    if (!options.authentication.disabled) {
      ...
      if (options.authentication.type === AUTH_API_KEY) {
        ...
        if (addTo === HEADER) {
          ...
          const qs = authQueryParam
            ? buildQueryStringFromParams([authQueryParam])
            : "";
          url = joinUrlAndQueryString(options.url, qs);
        }
      }
      if (
        options.authentication.type === AUTH_BEARER &&
    ...
    const filteredClientCertificates = filterClientCertificates(
      clientCertificates,
      options.url,
      "wss:",
    );
    ...
    filteredClientCertificates.forEach((clientCertificate) => {
      ...
      if (key) {
        ...
        pemCertificateKeys.push({
          pem: fs.readFileSync(key, "utf-8"),
      ...
      if (pfx) {
        ...
        pfxCertificates.push({
          buf: fs.readFileSync(pfx, "utf-8"),
          passphrase: passphrase ?? undefined,
        });
      }
    });
    ...
    if (request.settingSendCookies && options.cookieJar.cookies.length) {
      ...
      const cookieHeaderWithAuth = cookieHeader
    ...
    const ws = new WebSocket(url, protocols, {
      ...
      followRedirects,
      maxRedirects:
        settings.maxRedirects > 0 ? settings.maxRedirects : undefined,
    });
    ...
    ws.on("upgrade", async (incomingMessage) => {
      ...
      const settings = await models.settings.get();
      const res = await models.webSocketResponse.create(
        responsePatch,
        settings.maxHistoryResponses,
      );
      models.requestMeta.updateOrCreateByParentId(request._id, {
        activeResponseId: res._id,
      });

      if (request.settingStoreCookies) {
        const setCookieStrings: string[] = getSetCookieHeaders(
          responseHeaders,
        ).map((h) => h.value);
        const totalSetCookies = setCookieStrings.length;
        if (totalSetCookies) {
          const currentUrl = request.url;
          const { cookies, rejectedCookies } =
            await addSetCookiesToToughCookieJar({
              setCookieStrings,
              currentUrl,
    ...
    ws.on("unexpected-response", async (clientRequest, incomingMessage) => {
      ...
      deleteRequestMaps(
        request._id,
        `Unexpected response ${incomingMessage.statusCode}`,
      );
    });
    ...
    ws.addEventListener("open", () => {
      ...
      eventLogFileStreams
        .get(options.requestId)
        ?.write(JSON.stringify(openEvent) + "\n");
      timelineFileStreams
        .get(options.requestId)
        ?.write(
          JSON.stringify({
    ...
    ws.addEventListener("message", ({ data }: MessageEvent) => {
      ...
      eventLogFileStreams
        .get(options.requestId)
        ?.write(JSON.stringify(messageEvent) + "\n");
...
const handleGraphQLWsMessage = (
  ...
  if (graphqlServerDataType === MessageType.ConnectionAck) {
    ...
    graphqlServerDataType === MessageType.Error ||
    graphqlServerDataType === MessageType.Complete
  ) {
...
const deleteRequestMaps = async (
  ...
  timelineFileStreams
    .get(requestId)
    ?.write(
      JSON.stringify({ value: message, name: "Text", timestamp: Date.now() }) +
        "\n",
    );
...
const getWebSocketReadyState = async (options: {
  requestId: string;
}): Promise<boolean> => {
  return (
    WebSocketConnections.get(options.requestId)?.readyState === WebSocket.OPEN
  );
};
...
const sendPayload = async (
  ...
  eventLogFileStreams
    .get(options.requestId)
    ?.write(JSON.stringify(lastMessage) + "\n");
  const response = await models.webSocketResponse.getLatestByParentId(
    options.requestId,
  );
  if (!response) {
...
export const registerWebSocketHandlers = () => {
  ...
  ipcMainHandle(
    ...
    (_, options: Parameters<typeof sendWebSocketEvent>[0]) =>
      sendWebSocketEvent(options),
  );
  ipcMainOn(
    "webSocket.close",
    (_, options: Parameters<typeof closeWebSocketConnection>[0]) =>
      closeWebSocketConnection(options),
  );
  ipcMainOn("webSocket.closeAll", closeAllWebSocketConnections);
  ipcMainHandle(
    "webSocket.readyState",
    (_, options: Parameters<typeof getWebSocketReadyState>[0]) =>
      getWebSocketReadyState(options),
  );
  ipcMainHandle(
    "webSocket.event.findMany",
    (_, options: Parameters<typeof findMany>[0]) => findMany(options),
  );
};

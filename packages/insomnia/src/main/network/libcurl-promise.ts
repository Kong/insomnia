...
invariant(
  ...
  "Native abstractions for Nodejs module unavailable in renderer",
);
...
export const cancelCurlRequest = (id: string) =>
  cancelCurlRequestHandlers[id]();
export const curlRequest = (options: CurlRequestOptions) =>
  new Promise<CurlRequestOutput>(async (resolve) => {
    try {
      ...
      const isMultipart =
        body.mimeType === CONTENT_TYPE_FORM_DATA && requestBodyPath;
      let requestFileDescriptor: number | undefined;
      const { authentication } = req;
      if (requestBodyPath) {
        // AWS IAM file upload not supported
        invariant(
          authentication.type !== AUTH_AWS_IAM,
      ...
      if (process.env.PLAYWRIGHT) {
        req.headers = [
          ...req.headers,
          { name: "X-Mockbin-Test", value: "true" },
        ];
      }
      ...
      curl.on("error", async (err, code) => {
        const elapsedTime =
          (curl.getInfo(Curl.info.TOTAL_TIME) as number) * 1000;
        curl.isOpen && curl.close();
        await waitForStreamToFinish(responseBodyWriteStream);
...
export const createConfiguredCurlInstance = ({
  ...
  const userAgent: RequestHeader | null =
    headers.find((h: any) => h.name.toLowerCase() === "user-agent") || null;
  const userAgentOrFallback =
    typeof userAgent?.value === "string"
      ? userAgent?.value
      : "insomnia/" + version;
...
const parseRequestBody = ({ body, method }: { body: any; method: string }) => {
  ...
  const hasMimetypeAndUpdateMethod =
    typeof body.mimeType === "string" || expectsBody;
  if (isUrlEncodedForm) {
    const urlSearchParams = new URLSearchParams();
    (body.params || []).map((p: { name: string; value: any }) =>
...
export const getHttpVersion = (preferredHttpVersion: string) => {
  switch (preferredHttpVersion) {
    ...
    case "V2PriorKnowledge":
      return {
        log: "Using HTTP/2 PriorKnowledge",
        curlHttpVersion: CurlHttpVersion.V2PriorKnowledge,
      };
    case "V2_0":

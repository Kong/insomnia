...
export const getExecution = (requestId?: string) =>
  requestId ? executions.get(requestId) : [];

export const startExecution = (requestId: string) =>
  executions.set(requestId, []);

export function addExecutionStep(requestId: string, stepName: string) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send(`syncTimers.${requestId}`, {
      executions: executions.get(requestId),
    });
  }
}
...
export function updateLatestStepName(executionId: string, stepName: string) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send(`syncTimers.${executionId}`, {
      executions: executions.get(executionId),
    });
  }
}

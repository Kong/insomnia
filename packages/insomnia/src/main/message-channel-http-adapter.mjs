...
class PortEventManager {
  ...
  /** @param {Electron.MessageEvent | { data?: ProtocolMessage } | ProtocolMessage} event */
  handleMessage(event) {
      ...
      manager.registerStreamHandler(requestId, msg => {
        if (msg.type === 'body-chunk') {
          try {
            ...
            controller.error(new Error(`Failed to enqueue body chunk: ${message}`));
          ...
          try {
            ...
            const message = err instanceof Error ? err.message : String(err);
            controller.error(new Error(`Failed to close stream: ${message}`));
          }
          manager.cleanup(requestId);

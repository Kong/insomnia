...
async function vcs(branch) {
  ...
  await v.switchAndCreateBackendProjectIfNotExist(
    "workspace_1",
    "Test Workspace",
  );
...
describe("VCS", () => {
      ...
      expect(status).toEqual({
        ...
        unstaged: {
          foo: {
            ...
            key: "foo",
            blobContent:
              '{"_id":"bar","created":1234,"isPrivate":false,"name":"name","parentId":"","type":"base"}',
          ...
          baz: {
            ...
            key: "baz",
            blobContent:
              '{"_id":"qux","created":1234,"isPrivate":false,"name":"name","parentId":"","type":"base"}',
      ...
      expect(status).toEqual({
        ...
        unstaged: {
          ...
          d: {
            ...
            blobContent:
              '{"_id":"ddd","created":1234,"isPrivate":false,"name":"name","parentId":"","type":"base"}',
            previousBlobContent: "null",
          },
        },
      });
      ...
      expect(status2).toEqual({
        ...
        unstaged: {
          ...
          b: {
            ...
            key: "b",
            blobContent:
              '{"_id":"bbb","created":1234,"isPrivate":false,"name":"name","parentId":"","type":"base"}',
            previousBlobContent: "null",
          },
        },
      });
  ...
  describe("describeChanges()", () => {
    it("works with same object structure", async () => {
      const a = workspaceModelBuilder
        .reset()
        .name("foo")
        .certificates({ baz: 10 })
        .build();
      const b = workspaceModelBuilder
        .reset()
        .name("baz")
        .certificates({ baz: 11 })
        .build();
  ...
  describe("hasBackendProjectForRootDocument", () => {
    ...
    beforeEach(async () => {
      ...
      driver.setItem(
        "/projects/",
        Buffer.from(JSON.stringify([backendProject])),
      );
    ...
    it("should return true if has project", async () => {
      const hasProject = await vcs.hasBackendProjectForRootDocument(
        backendProject.rootDocumentId,
      );
    ...
    it("should return false if has no project", async () => {
      const hasProject =
        await vcs.hasBackendProjectForRootDocument("some other id");

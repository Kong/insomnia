...
import {
  branchSchema,
  mergeConflictSchema,
  statusCandidateSchema,
} from "../../__schemas__/type-schemas";
...
describe("util", () => {
  ...
  describe("hashDocument()", () => {
    it("fails on undefined", () => {
      expect(() => hashDocument(undefined)).toThrowError(
        "Cannot hash undefined value",
      );
    ...
    it("shouldnt change the hash of a workspace after a parent id is added and ignored", () => {
      ...
      const originalSyncedWorkspace = workspaceModelBuilder
        .parentId(null)
        .build();
...
const newCandidate = (key: string, n: number) =>
  statusCandidateBuilder
    ...
    .document(
      baseModelBuilder
        .reset()
        .name(`Content for candidate ${key}.${n}`)
        .build(),
    )
    .build();

const newBranch = (snapshots: string[]) =>
  branchBuilder.snapshots(snapshots).build();
  ...
  it("intercepts an error", async () => {
    ...
    const action = async () =>
      (await interceptAccessError({
        ...
        callback: () => {
          throw new Error(
            "DANGER! invalid access to the fifth dimensional nebulo 9.",
          );
        },

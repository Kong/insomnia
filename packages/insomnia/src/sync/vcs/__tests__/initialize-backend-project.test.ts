...
describe("initialize-backend-project", () => {
  describe("pushSnapshotOnInitialize()", () => {
    ...
    it("should not push if no active project", async () => {
      ...
      await expect(
        models.workspaceMeta.getByParentId(workspace._id),
      ).resolves.toStrictEqual(workspaceMeta);
    });
    ...
    it("should not push snapshot if not remote project", async () => {
      ...
      vcs.switchAndCreateBackendProjectIfNotExist(
        workspace._id,
        workspace.name,
      );
    ...
    it("should push snapshot if conditions are met", async () => {
      ...
      const updatedMeta = await models.workspaceMeta.getByParentId(
        workspace._id,
      );

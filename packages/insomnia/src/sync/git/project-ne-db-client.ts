...
export class GitProjectNeDBClient {
  ...
  async readFile(
    ...
    try {
      ...
      const workspaceFile = await getInsomniaV5DataExport({
        workspaceId,
  ...
  async writeFile(filePath: string, data: Buffer | string) {
    ...
    const doesFileContainInsomniaV5FormatTypeString = dataStr
      .split("\n")[0]
      .trim()
      .includes("insomnia.rest");
    ...
    for (const doc of dataToImport) {
      if (isWorkspace(doc)) {
        console.log(
          "[git] setting workspace parent to be that of the active project",
          { original: doc.parentId, new: this._projectId },
        );
  ...
  async readdir(filePath: string) {
    ...
    const workspaceMetas = await db.find<WorkspaceMeta>(
    ...
    const hasDirectoryInsomniaFiles = workspaceMetas.some(
      ({ gitFilePath }) =>
        gitFilePath && path.dirname(gitFilePath) === filePath,
    );
      ...
      const workspacePaths = workspaceMetas
        ...
        .filter(
          (workspaceMeta) =>
            ...
            path.dirname(workspaceMeta.gitFilePath) === filePath,
        )
  ...
  async getWorkspaceIdFromFilePath(filePath: string) {
    ...
    const workspaceMeta = await db.find<WorkspaceMeta>(
      models.workspaceMeta.type,
      {
        gitFilePath: filePath,
      },

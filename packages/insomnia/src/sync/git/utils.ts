...
export const addDotGit = (url: string): string =>
...
const onAuthFailure =
  ...
  async (message, auth) => {
    ...
    if (
      ...
      try {
        const providerCredentials = await gitCredentials.getByProvider(
...
export const getAuthorFromGitRepository = async (
  ...
  if (
    "oauth2format" in gitRepo.credentials &&
    gitRepo.credentials.oauth2format
  ) {
    console.log("[git-event] Using OAuth2 credentials");
    const providerCredentials = await gitCredentials.getByProvider(
      gitRepo.credentials.oauth2format,
...
export const getOauth2FormatName = (

...
describe("NeDBClient", () => {
  ...
  describe("readdir()", () => {
    it("reads model IDs from model type folders", async () => {
      ...
      expect(await neDbClient.readdir(GIT_CLONE_DIR)).toEqual([
        GIT_INSOMNIA_DIR_NAME,
      ]);
      ...
      expect(await neDbClient.readdir(reqDir)).toEqual([
        "req_1.yml",
        "req_2.yml",
      ]);
  ...
  describe("readFile()", () => {
    it("reads file from model/id folders", async () => {
      const wrk1Yml = path.join(
        GIT_INSOMNIA_DIR,
        models.workspace.type,
        "wrk_1.yml",
      );
      const req1Yml = path.join(
        GIT_INSOMNIA_DIR,
        models.request.type,
        "req_1.yml",
      );
      const reqXYml = path.join(
        GIT_INSOMNIA_DIR,
        models.request.type,
        "req_x.yml",
      );
      ...
      expect(
        YAML.parse((await pNeDB.readFile(wrk1Yml, "utf8")).toString()),
      ).toEqual(
      ...
      expect(
        YAML.parse((await pNeDB.readFile(req1Yml, "utf8")).toString()),
      ).toEqual(
        expect.objectContaining({
          _id: "req_1",
          parentId: "wrk_1",
  ...
  describe("stat()", () => {
    it("stats a dir", async () => {
      ...
      expect(await neDbClient.stat(path.join(wrkDir, "wrk_1.yml"))).toEqual(
        fileType,
      );
      expect(await neDbClient.stat(path.join(reqDir, "req_2.yml"))).toEqual(
        fileType,
      );
    });
  });

...
describe("Git-VCS", () => {
  ...
  describe("common operations", () => {
    it("stage and unstage file", async () => {
      ...
      await fsClient.promises.writeFile(
        path.join(GIT_INSOMNIA_DIR, fooTxt),
        "foo",
      );
      await fsClient.promises.writeFile(
        path.join(GIT_INSOMNIA_DIR, barTxt),
        "bar",
      );
...
`,
  ...
  describe("undoPendingChanges()", () => {
    it("should remove pending changes from all tracked files", async () => {
      ...
      expect((await fsClient.promises.readFile(folderBarTxt)).toString()).toBe(
        originalContent,
      );
    });
    ...
    it("should remove pending changes from select tracked files", async () => {
      ...
      // Write to all files
      await Promise.all(
        files.map((f) => fsClient.promises.writeFile(f, originalContent)),
      );
      // Commit all files
      await GitVCS.setAuthor({
        name: "Karen Brown",
      ...
      // Change all files
      await Promise.all(
        files.map((f) => fsClient.promises.writeFile(f, changedContent)),
      );

      const status2 = await GitVCS.status();
      // Undo foo1 and foo2, but not foo3
      const changesToUndo = status2.unstaged.filter(
        (change) => !change.path.includes(foo3Txt),
      );
      ...
      expect((await fsClient.promises.readFile(foo1Txt)).toString()).toBe(
        originalContent,
      );
      expect((await fsClient.promises.readFile(foo2Txt)).toString()).toBe(
        originalContent,
      );
      expect((await fsClient.promises.readFile(foo3Txt)).toString()).toBe(
        changedContent,
      );
    });
  });

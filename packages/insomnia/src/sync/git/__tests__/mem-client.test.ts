...
describe("MemClient", () => {
  ...
  describe("writeFile()", () => {
    it("fails to write over directory", async () => {
      ...
      await assertAsyncError(
        fsClient.writeFile(dirName, "Hello World 2!"),
        "EISDIR",
      );
    });
    ...
    it("overwrites file", async () => {
      ...
      expect((await fsClient.readFile(fooTxt)).toString()).toBe(
        "Hello World 2!",
      );
    });
  ...
  describe("unlink()", () => {
    ...
    it("fails to unlinks missing file", async () => {
      const fsClient = new MemClient();
      await assertAsyncError(
        fsClient.unlink(path.join("not", "exist.txt")),
        "ENOENT",
      );
    });
  });
  ...
  describe("readdir()", () => {
    ...
    it("errors on missing directory", async () => {
      const fsClient = new MemClient();
      await assertAsyncError(
        fsClient.readdir(path.join("/", "invalid")),
        "ENOENT",
      );
    });
  });

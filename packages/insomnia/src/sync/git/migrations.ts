...
async function migrateGitHubConnectedRepositories(repositories: GitRepository[]) {
  ...
  if (githubCredentials && models.gitCredentials.isGitCredentialsV1(githubCredentials)) {
    ...
    for (const repo of repositories) {
      if (
        repo.credentials &&
        models.gitRepository.isGitCredentialsOAuth(repo.credentials) &&
...
async function migrateGitLabConnectedRepositories(repositories: GitRepository[]) {
  ...
  if (gitlabCredentials && models.gitCredentials.isGitCredentialsV1(gitlabCredentials)) {
    const newCredential = await services.gitCredentials.create({
...
async function migrateCustomCredentialsRepositories(repositories: GitRepository[]) {
  for (const repo of repositories) {
    if (!repo.credentials || models.gitRepository.isGitCredentialsOAuth(repo.credentials)) {
      continue;
    }
...
export async function runGitCredentialsMigration(): Promise<void> {
  try {
    ...
    const githubConnectedRepositories = allRepositories.filter(({ credentials }) => {
      ...
      return models.gitRepository.isGitCredentialsOAuth(credentials) && credentials.oauth2format === 'github';
    });
    ...
    const gitlabConnectedRepositories = allRepositories.filter(({ credentials }) => {
      ...
      return models.gitRepository.isGitCredentialsOAuth(credentials) && credentials.oauth2format === 'gitlab';
    });
    ...
    const customCredentialsRepositories = allRepositories.filter(({ credentials }) => {
      return credentials && !models.gitRepository.isGitCredentialsOAuth(credentials);
    });

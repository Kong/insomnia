...
const Debug = () => {
  ...
  const collectionDragAndDrop = useDragAndDrop({
    ...
    renderDropIndicator(target) {
      if (target.type === 'item') {
        ...
        if (item) {
          return (
            <DropIndicator
              target={target}
              className="absolute left-0 top-0 z-10 w-full outline-solid outline-1 outline-(--color-surprise)"
              style={{
      ...
      return (
        <DropIndicator target={target} className="absolute left-0 top-0 outline-solid outline-1 outline-(--color-surprise)" />
      );
    },
  });
          ...
          <div className="flex flex-col items-start divide-y divide-solid divide-(--hl-md)">
            <div className={`flex w-full h-[${INSOMNIA_TAB_HEIGHT}px]`}>
              <Breadcrumbs className="m-0 flex h-full w-full list-none items-center gap-2 px-(--padding-sm) font-bold">
                ...
                <Breadcrumb className="ml-auto mr-2.5 flex h-full select-none items-center gap-2 justify-self-end truncate text-sm text-(--color-font) outline-hidden data-focused:outline-hidden">
                  <NavLink
                    data-testid="run-collection-btn-quick"
                    className="flex h-7 shrink-0 items-center justify-center gap-2 rounded-xs px-2 py-1 text-sm text-(--color-font) outline-hidden ring-1 ring-transparent transition-all hover:bg-(--hl-xs) focus:ring-inset focus:ring-(--hl-md) aria-pressed:bg-(--hl-sm) aria-[current]:hidden data-focused:outline-hidden"
  ...
  return (
    <>
      ...
      >
        {item => {
          return (
            <GridListItem textValue={item.title} className="group select-none outline-hidden">
...
const CollectionGridListItem = ({
  ...
  return (
    <GridListItem
      id={item.doc._id}
      className={`group absolute left-0 top-0 w-full select-none outline-hidden ${isRequestGroup(item.doc) ? 'data-drop-target:bg-(--hl-md)' : 'border-solid data-drop-target:border-b data-drop-target:border-(--color-surprise)'}`}
      textValue={label}
      data-testid={item.doc.name}
      style={style}
      ref={triggerRef}

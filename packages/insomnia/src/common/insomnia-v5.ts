...
export function insomniaSchemaTypeToScope(
  type: InsomniaFile["type"],
): WorkspaceScope {
...
function getEnvironments(file: InsomniaFile): Environment[] {
  if ("environments" in file && file.environments) {
    const baseEnvironment: WithExportType<Environment> = {
      ...
      dataPropertyOrder:
        (file.environments.dataPropertyOrder as Record<string, any>) ||
        undefined,
    ...
    const subEnvironments: WithExportType<Environment>[] =
      file.environments.subEnvironments?.map((environment, index) => ({
        ...
        dataPropertyOrder:
          (environment.dataPropertyOrder as Record<string, any>) || undefined,
    ...
    return [
      {
        ...
        contentType: "json",
        contents:
          "contents" in file.spec && file.spec.contents
            ? stringify(file.spec.contents)
            : "",
        parentId: file.meta?.id || "__WORKSPACE_ID__",
      },
    ];
...
function getCollection(
  file: InsomniaFile,
): (Request | WebSocketRequest | GrpcRequest | RequestGroup)[] {
  if (
    file.type === "collection.insomnia.rest/5.0" ||
    file.type === "spec.insomnia.rest/5.0"
  ) {
    const resources: (
      | Request
      | WebSocketRequest
        ...
        if ("children" in item && item.children) {
          const requestGroup: WithExportType<RequestGroup> = {
            ...
            environmentPropertyOrder:
              (item.environmentPropertyOrder as Record<string, any>) ||
              undefined,
          };
...
export async function getInsomniaV5DataExport({
  ...
  try {
    ...
    const exportableTypes = Object.values(models.MODELS_BY_EXPORT_TYPE).map(
      (model) => model.type,
    );
    ...
    function getEnvironmentsFromResources(
      ...
      { type: "collection.insomnia.rest/5.0" }
    >["environments"] {
      const baseEnvironment = resources.find((environment) =>
        environment.parentId.startsWith("wrk_"),
      );
      if (!baseEnvironment) {
      ...
      const subEnvironments = resources
        ...
        .filter(
          (environment) => includePrivateEnvironments || !environment.isPrivate,
        );
    ...
    function getTestSuitesFromResources(
      ...
      const testSuites: Extract<
        InsomniaFile,
        { type: "spec.insomnia.rest/5.0" }
      >["testSuites"] = [];

      resources
        .filter(models.unitTestSuite.isUnitTestSuite)
        .forEach((testSuite) => {
          const tests = resources
            .filter(models.unitTest.isUnitTest)
            .filter((test) => test.parentId === testSuite._id);
    ...
    if (workspace.scope === "collection") {
      const collection: InsomniaFile = {
        ...
        cookieJar: getCookieJarFromResources(
          exportableResources.filter(models.cookieJar.isCookieJar),
        ),
      ...
      const spec: InsomniaFile = {
        ...
        spec: getSpecFromResources(
          exportableResources.filter(models.apiSpec.isApiSpec),
        ),
        testSuites: getTestSuitesFromResources(
          exportableResources.filter(
            (resource) =>
              models.unitTestSuite.isUnitTestSuite(resource) ||
              models.unitTest.isUnitTest(resource),
          ),
        ),
      };
      ...
      const server = exportableResources.filter(
        models.mockServer.isMockServer,
      )[0];
      ...
      const mockServer: InsomniaFile = {
        ...
        routes: getRoutesFromResources(
          exportableResources.filter(models.mockRoute.isMockRoute),
        ),
      };

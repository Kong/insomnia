...
export async function getFilesFromPostmanExportedDataDump(
  filePath: string,
): Promise<PostmanDataDumpRawData> {
  let res;
  try {
...
export async function scanResources(
  contentList: string[] | ImportFileDetail[],
): Promise<ScanResult[]> {
  resourceCacheList = [];
  const results = await Promise.allSettled(
    contentList.map(async (content) => {
      const contentStr =
        typeof content === "string" ? content : content.contentStr;
      const oriFileName =
        typeof content === "string" ? "" : content.oriFileName;
...
export async function importResourcesToProject({
  ...
  for (const resourceCacheItem of resourceCacheList) {
    ...
    const postmanTopLevelFolder = resources.find(
      (resource) =>
        isRequestGroup(resource) && resource.parentId === "__WORKSPACE_ID__",
    ...
    await Promise.all(
      workspaceResources.map((workspace) => {
        if (
          workspaceResources.filter(({ _id }) => _id === "__WORKSPACE_ID__")
            .length > 1
        ) {
          console.warn(
        ...
        if (workspaceResources.length > 1) {
          resourcesInCurrentWorkspace = filterResourcesInWorkspace(
            resources,
            workspace,
          );
        }
...
function filterResourcesInWorkspace(
  ...
  return resources.filter(
    (resource) => findRootId(resource._id, new Set()) === workspaceId,
  );
}
...
const isTeamOrAbove = async () => {
  ...
  return ["team", "enterprise", "enterprise-member"].includes(
    currentPlan?.type,
  );
};
...
const importRequestWithNewIds = (
  ...
  if (canTransform) {
    ...
    transformedRequest = JSON.parse(
      updateIdsInString(JSON.stringify(request), ResourceIdMap),
    );
  }
  return {
    ...transformedRequest,
...
export const importResourcesToWorkspace = async ({
  ...
  for (const resourceCacheItem of resourceCacheList) {
    ...
    const baseEnvironmentFromResources = resources
      ...
      .find(
        (env) => env.parentId && env.parentId.startsWith("__WORKSPACE_ID__"),
      );
      ...
      if (model) {
        ...
        if (isGrpcRequest(resource)) {
          ...
          await models.request.create(
            importRequestWithNewIds(resource, ResourceIdMap, canTransform),
          );
        } else {
...
const importResourcesToNewWorkspace = async ({
  ...
  if (
    resourceCacheItem?.importer?.id &&
    isApiSpecImport(resourceCacheItem.importer)
  ) {
    ...
    const apiSpec = resources.find(
      (r) => r.type === "ApiSpec" && r.parentId === workspaceToImport?._id,
    ) as ApiSpec;
    ...
    for (const resource of resourcesWithoutWorkspaceAndApiSpec) {
      ...
      if (model) {
        ...
        if (!newParentId) {
          console.warn(
            `Could not find new parent id for ${resource.name} ${resource._id}`,
          );
          continue;
        }
        if (isGrpcRequest(resource)) {

...
export async function getSendRequestCallbackMemDb(
  ...
  const fetchInsoRequestData = async (
    requestId: string,
    ...
    invariant(
      environment,
      "failed to find environment " + overrideEnvironmentId,
    );
  ...
  return async function sendRequest(requestId: string, iteration?: number) {
    ...
    const getCurrentRowOfIterationData = wrapAroundIterationOverIterationData(
      iterationData,
      iteration,
    );
    ...
    const res = await responseTransform(
      response,
      environmentId,
      renderedRequest,
    ...
    const postMutatedContext = await tryToExecuteAfterResponseScript({
      ...
      transientVariables:
        mutatedContext.transientVariables || transientVariables,
      response,
    });

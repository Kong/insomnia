...
export const getSegmentWriteKey = () =>
  appConfig.segmentWriteKeys[
    isDevelopment() || env.PLAYWRIGHT ? "development" : "production"
  ];
...
export const getClientString = () =>
  `${getAppEnvironment()}::${getAppPlatform()}::${getAppVersion()}`;
...
export const displayModifierKey = (
...
export const getApiBaseURL = () =>
  env.INSOMNIA_API_URL || "https://api.insomnia.rest";
export const getMockServiceURL = () =>
  env.INSOMNIA_MOCK_API_URL || "https://mock.insomnia.rest";
...
export const getAIServiceURL = () =>
  env.INSOMNIA_AI_URL || "https://ai-helper.insomnia.rest";

export const getUpdatesBaseURL = () =>
  env.INSOMNIA_UPDATES_URL || "https://updates.insomnia.rest";

// App website
export const getAppWebsiteBaseURL = () =>
  env.INSOMNIA_APP_WEBSITE_URL || "https://app.insomnia.rest";

// GitHub API
export const getGitHubRestApiUrl = () =>
  env.INSOMNIA_GITHUB_REST_API_URL || "https://api.github.com";
export const getGitHubGraphQLApiURL = () =>
  env.INSOMNIA_GITHUB_API_URL || `${getGitHubRestApiUrl()}/graphql`;
...
export const isWorkspaceActivity = (
  activity?: string,
): activity is GlobalActivity =>
  isDesignActivity(activity) || isCollectionActivity(activity);

export const isDesignActivity = (
  activity?: string,
): activity is GlobalActivity => {
  switch (activity) {
    case ACTIVITY_SPEC:
...
export const isCollectionActivity = (
  activity?: string,
): activity is GlobalActivity => {
  switch (activity) {
    case ACTIVITY_DEBUG:
...
export const PREVIEW_MODES = Object.keys(
  previewModeMap,
) as (keyof typeof previewModeMap)[];
...
export function getContentTypeName(
  ...
  for (const contentTypeKey in contentTypesMap) {
    if (contentType.includes(contentTypeKey) && contentTypeKey.length > 0) {
      return useLong
        ? contentTypesMap[contentTypeKey][1]
        : contentTypesMap[contentTypeKey][0];
    }
  }

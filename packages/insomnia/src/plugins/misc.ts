...
export const validateThemeName = (name: string) => {
  ...
  if (!isValid) {
    ...
    console.error(
      `[theme] found an invalid theme name "${name}".  Try using ${validName}`,
    );
  }
  return validName;
};
...
export const validateTheme = (pluginTheme: PluginTheme) => {
  const checkIfContainsNunjucks =
    (pluginTheme: PluginTheme) => (keyPath: string[]) => {
...
export const generateThemeCSS = (pluginTheme: PluginTheme) => {
  ...
  if (theme.styles) {
    ...
    css += wrapStyles(
      name,
      ".theme--tooltip",
      getThemeBlockCSS(styles.tooltip || styles.dialog),
    );
    // Overlay
    css += wrapStyles(
      name,
    ...
    css += wrapStyles(
      name,
      ".theme--app-header",
      getThemeBlockCSS(styles.appHeader),
    );
    // Sidebar Styles
    css += wrapStyles(
      name,
      ".theme--sidebar",
      getThemeBlockCSS(styles.sidebar),
    );
    css += wrapStyles(
      name,
      ".theme--sidebar__list",

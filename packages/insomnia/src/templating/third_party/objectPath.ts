...
const ObjectPath = {
            ...
            if (dot === -1 && bracket === -1) {
                ...
                if (quote !== '"' && quote !== "'") {
                    ...
                    */
                    ...
                    parts.push(str.slice(i + 2, closing).replace(regex[quote], quote).replace(/\\+/g, backslash => {
    ...
    stringify: (arr: (string | number)[], quote?: Quote, forceQuote?: boolean) => {
        ...
        return arr.map((value: string | number, key: number) => {
    ...
    normalize: (data: string, quote?: Quote, forceQuote?: boolean) => {

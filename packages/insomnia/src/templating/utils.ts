...
export function getKeys(obj: any, prefix = ""): { name: string; value: any }[] {
  ...
  if (typeOfObj === "[object Array]") {
    for (let i = 0; i < obj.length; i++) {
      allKeys = [
        ...allKeys,
        ...getKeys(obj[i], forceBracketNotation(prefix, i)),
      ];
    }
    ...
    for (const key of keys) {
      allKeys = [
        ...allKeys,
        ...getKeys(obj[key], forceBracketNotation(prefix, key)),
      ];
    }
  ...
  const stringArgs: string[] = (args || []).map((argDefinition) => {
    switch (argDefinition.type) {
      case "enum":
        ...
        const value =
          defaultValue !== undefined ? String(defaultValue) : String(fallback);
        return `'${value}'`;
  ...
  if (typeof vaultEnvironmentData === "object") {
    if (shouldDecrypt) {
      ...
      if (isVaultEnabled && vaultKey) {
        const symmetricKey = (await decryptVaultKeyFromSession(
          vaultKey,
          true,
        )) as JsonWebKey;
        ...
        Object.keys(vaultEnvironmentData).forEach((vaultContextKey) => {
          ...
          vaultEnvironmentData[vaultContextKey] = decryptSecretValue(
            encryptedValue,
            symmetricKey,
          );
        });

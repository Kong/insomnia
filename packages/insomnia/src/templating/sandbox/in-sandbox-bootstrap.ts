...
export const IN_SANDBOX_BOOTSTRAP = [
  ...
  // Snapshot the workspace id into a primitive now, before any plugin code runs, and have every
  // `models.read` bridge call below close over this local rather than re-reading env.meta/ctx.meta
  // live — ctx.meta = env.meta (below) is a live reference to the same mutable object, so a live read
  // would let a plugin overwrite ctx.meta.workspaceId right before a bridge call.
  '    var __callerWorkspaceId = (env.meta && env.meta.workspaceId) || null;',
  '    var ctx = {',
  '      app: {',
  ...
  '        models: {',
  '          request: {',
  '            getById: function (id) { return __bridge("request.getById", { id: id, callerWorkspaceId: __callerWorkspaceId }); },',
  '            getAncestors: function (request) { return __bridge("request.getAncestors", { request: request, types: ["RequestGroup", "Workspace"], callerWorkspaceId: __callerWorkspaceId }).then(function (anc) { return (anc || []).filter(function (doc) { return doc._id !== request._id; }); }); }',
  '          },',
  '          cloudCredential: {',
  ...
  '          },',
  '          workspace: { getById: function (id) { return __bridge("workspace.getById", { id: id, callerWorkspaceId: __callerWorkspaceId }); } },',
  '          oAuth2Token: { getByRequestId: function (parentId) { return __bridge("oAuth2Token.getByRequestId", { parentId: parentId, callerWorkspaceId: __callerWorkspaceId }); } },',
  '          cookieJar: {',
  '            getOrCreateForParentId: function (parentId) { return __bridge("cookieJar.getOrCreateForParentId", { parentId: parentId, callerWorkspaceId: __callerWorkspaceId }); },',
  '            getCookiesForUrl: function (parentId, url) { return __bridge("cookieJar.getCookiesForUrl", { parentId: parentId, url: url, callerWorkspaceId: __callerWorkspaceId }); }',
  '          },',
  '          response: {',
  '            getLatestForRequestId: function (requestId, environmentId) { return __bridge("response.getLatestForRequestId", { requestId: requestId, environmentId: environmentId, callerWorkspaceId: __callerWorkspaceId }); },',

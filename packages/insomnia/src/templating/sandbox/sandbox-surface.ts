 ...
 * `require('url').URL` is *meant* to be `=== globalThis.URL`, matching real Node's own
 * `require('url').URL === global.URL` identity, because `URL` is already an ungated ambient global
 * with or without the module grant) — callers must pass the exact expected message, so a genuinely
 * new leak on an unrelated path still fails loudly instead of being masked.
 */

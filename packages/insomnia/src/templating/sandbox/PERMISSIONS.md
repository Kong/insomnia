...
- **Grantable:** any other module in the sandbox registry. Declaring one adds it to your grant.
  - Pure-JS reimplementations: `events`, `string_decoder` (and more via M2).
    - `string_decoder` is a full reimplementation of `StringDecoder`, buffering incomplete
      ...
      against `node:string_decoder`, byte-boundary-for-byte-boundary, for `utf8`/`utf16le`/`ucs2`/
      `base64`/`latin1`/`binary`/`ascii`/`hex`). `write()` accepts a `Uint8Array` or a plain byte
      array. No omissions — the module's full public surface (`new StringDecoder(encoding)`,
      `.write()`, `.end()`) is implemented.

...
describe('runTagInSandbox — PoC milestone 1', () => {
  ...
  describe('require("crypto") parity', () => {
    ...
    it.each(['sha256', 'md5', 'sha1', 'sha512'])('createHash %s hex', async algo => {
      ...
      const actual = await runTagInSandbox({
        pluginSource: source, tagName: 'c', envelope: envelope(['hello world']), bridge: noBridge, hostCrypto: nodeHostCrypto,
      });
      expect(actual).toBe(expected);
    });
    ...
    it('createHash sha256 base64 + chained updates', async () => {
      ...
      const actual = await runTagInSandbox({
        pluginSource: source, tagName: 'c', envelope: envelope(['bar']), bridge: noBridge, hostCrypto: nodeHostCrypto,
      });
      expect(actual).toBe(expected);
    });
    ...
    it('createHmac sha256 hex', async () => {
      ...
      const actual = await runTagInSandbox({
        pluginSource: source, tagName: 'c', envelope: envelope(['payload']), bridge: noBridge, hostCrypto: nodeHostCrypto,
      });
      expect(actual).toBe(expected);
    });
    ...
    it('randomUUID has v4 shape', async () => {
      ...
      const actual = await runTagInSandbox({
        pluginSource: source, tagName: 'c', envelope: envelope([]), bridge: noBridge, hostCrypto: nodeHostCrypto,
      });
  ...
  it('surfaces a host bridge failure inside the tag', async () => {
    const source = 'module.exports.templateTags = [{ name: "needsOs", run: async function (ctx) { return await ctx.util.nodeOS(); } }];';
    const failing = createMapBridge({ nodeOS: async () => { throw new Error('bridge exploded'); } });
    await expect(

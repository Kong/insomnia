...
describe("queryAllWorkspaceUrls", () => {
  it("should return empty array when no requests exist", async () => {
    ...
    await expect(
      queryAllWorkspaceUrls(w._id, models.request.type),
    ).resolves.toHaveLength(0);
    await expect(
      queryAllWorkspaceUrls(w._id, models.grpcRequest.type),
    ).resolves.toHaveLength(0);
  });
  ...
  it("should return urls and exclude that of the selected request", async () => {
    ...
    await expect(
      queryAllWorkspaceUrls(w._id, models.request.type),
    ).resolves.toStrictEqual(expect.arrayContaining([r1.url, r2.url]));
    await expect(
      queryAllWorkspaceUrls(w._id, models.grpcRequest.type),
    ).resolves.toStrictEqual(expect.arrayContaining([gr1.url, gr2.url]));
    // Ignore url of the selected request id
    await expect(
      queryAllWorkspaceUrls(w._id, models.request.type, r1._id),
    ).resolves.toStrictEqual(expect.arrayContaining([r2.url]));
    await expect(
      queryAllWorkspaceUrls(w._id, models.grpcRequest.type, gr1._id),
    ).resolves.toStrictEqual(expect.arrayContaining([gr2.url]));
  });
});

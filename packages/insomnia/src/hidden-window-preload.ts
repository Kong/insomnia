...
export interface HiddenBrowserWindowToMainBridgeAPI {
  ...
  onmessage: (
    listener: (data: any, callback: (result: any) => void) => void,
  ) => void;
...
const bridge: HiddenBrowserWindowToMainBridgeAPI = {
  onmessage: (listener) => {
    ...
    return () =>
      ipcRenderer.removeListener("renderer-listener", rendererListener);
  },
  requireInterceptor,
  curlRequest: (options) => ipcRenderer.invoke("curlRequest", options),
  readCurlResponse: (options) =>
    ipcRenderer.invoke("readCurlResponse", options),

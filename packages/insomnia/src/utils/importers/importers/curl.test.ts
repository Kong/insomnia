...
describe("curl", () => {
  describe("cURL --data flags", () => {
    it.each([
      ...
      },
      {
        flag: "-d",
        inputs: ["@filename"],
        expected: [{ name: "", fileName: "filename", type: "file" }],
      },
      {
        flag: "-d",
      ...
      },
      {
        flag: "--data",
        inputs: ["@filename"],
        expected: [{ name: "", fileName: "filename" }],
      },
      {
        flag: "--data",
        inputs: ["first=1", "second=2", "third"],
      ...
      async ({
        ...
        const flaggedInputs = inputs
          .map((input) => `${flag} ${quote([input])}`)
          .join(" ");
  ...
  describe("cURL -H flags", () => {
    it.each([
      {
        flag: "-H",
        inputs: ["X-Host: example.com"],
        expected: [{ name: "X-Host", value: "example.com" }],
      },
      {
        flag: "-H",
        inputs: ["X-Host:example.com"],

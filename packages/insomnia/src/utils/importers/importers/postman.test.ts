...
describe("postman", () => {
  ...
  describe("transformPostmanToNunjucksString", () => {
    it("should transform to nunjucks syntax", () => {
      ...
      const output =
        "{% faker 'guid' %}abc{% faker 'randomStreetAddress' %}def{% faker 'guid' %}";
  ...
  describe("headers", () => {
    ...
    describe("awsv4", () => {
      it("get auth info from header", () => {
          ...
          header: [
            {
              key: "Authorization",
              value:
                "AWS4-HMAC-SHA256 Credential=<accessKeyId>/20220110/<region>/<service>/aws4_request, SignedHeaders=accept;content-type;host;x-amz-date;x-amz-security-token, Signature=ed270ed6ad1cad3513f6edad9692e4496e321e44954c70a86504eea5e0ef1ff5",
            },
            {
        ...
        const schema = postmanSchema({ requests: [request] });
        const postman = new ImportPostman(schema);
        const { authentication } = postman.importRequestItem(
          { request },
          "n/a",
        );
    ...
    describe("oauth1", () => {
      it("returns simple oauth1 authentication", () => {
        const oauth1 =
          'OAuth realm="Realm",oauth_consumer_key="Consumer%20Key",oauth_token="Access%20Token",oauth_signature_method="HMAC-SHA1",oauth_timestamp="Timestamp",oauth_nonce="Nonce",oauth_version="Version",oauth_callback="Callback%20URL",oauth_verifier="Verifier",oauth_signature="TwJvZVasVWTL6X%2Bz3lmuiyvaX2Q%3D"';
        const request: Request1 = {
          header: [
            {
              key: "Authorization",
  ...
  describe("oauth2", () => {
    ...
    it("returns oauth2 for Postman v2.1.0 with PKCE", () => {
      ...
      const schema = postmanSchema({
        requests: [requestWithPCKE],
        version: "v2.1.0",
      });
      const postman = new ImportPostman(schema);
      const { authentication } = postman.importRequestItem(
        { request: requestWithPCKE },
        "n/a",
      );

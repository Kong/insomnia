...
type ProcessDeconstructFuncReturnType<T> = T extends {
...
export const deconstructQueryStringToParams = <T extends IQueryStringOptions>(
  ...
  for (const stringPair of stringPairs) {
    ...
    const encodedValue: ValueType =
      encodedValues.length === 0 && strictNullHandling
        ? null
        : encodedValues.join("=");
...
export const smartEncodeUrl = (
  ...
  if (!encode) {
    ...
    if (parsedUrl.query) {
      const qsParams = deconstructQueryStringToParams(parsedUrl.query, true, {
        strictNullHandling,
      });
      ...
      for (const { name, value } of qsParams) {
        encodedQsParams.push({
          ...
          value:
            strictNullHandling && value === null
              ? null
              : flexibleEncodeComponent(value as string),
        });
      }

      parsedUrl.query = buildQueryStringFromParams(encodedQsParams, true, {
        strictNullHandling,
      });

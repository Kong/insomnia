  ...
  try {
    ...
    if (vaultSalt) {
      ...
      if (localVaultKey) {
        ...
        const validateResult = await validateVaultKey(
          userSession,
          localVaultKey,
          vaultSalt,
        );
        if (validateResult) {
          ...
          const encryptedVaultKey =
            await window.main.secretStorage.encryptString(localVaultKey);
...
const Authorize = () => {
  ...
  const authFetchers = allFetchers.filter(
    (f) => f.formAction === "/auth/authorize",
  );
  ...
  return (
    <div className="flex flex-col gap-[--padding-md] text-[--color-font]">
      <Heading className="px-3 text-center text-2xl font-bold">
        Authorizing Insomnia
      </Heading>
      {
        <Fragment>
          <p>
            A new page should have opened in your default web browser. Please
            log in. If you choose to login with SSO and it uses a different
            email to your previous login your teams will not be migrated.
          </p>
          <div className="flex flex-col gap-3 rounded-md bg-[--hl-sm] p-[--padding-md]">
            <p className="text-[rgba(var(--color-font-rgb),0.8))] text-start">
              If you were not redirected back here after creating an account,
              please copy and paste the following URL into your browser to
              complete login.
            </p>
            ...
            >
              ...
              {authorizeFetcher.data?.errors?.message && (
                <p>{authorizeFetcher.data.errors.message}</p>
              )}
            </form>
          </div>
        </Fragment>

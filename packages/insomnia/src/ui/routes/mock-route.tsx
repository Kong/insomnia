...
export const loader: LoaderFunction = async ({
  ...
  const reqIds = (await models.request.findByParentId(mockRouteId)).map(
    (r) => r._id,
  );
...
export const MockRouteRoute = () => {
  ...
  const { mockRoutes } = useRouteLoaderData(
    "mock-server",
  ) as MockServerLoaderData;
  ...
  const upsertBinOnRemoteFromResponse = async (
    ...
    try {
      ...
      console.log("[mock] Error: invalid response from remote", {
        res,
        mockbinUrl,
      });
  ...
  const upsertMockbinHar = async (pathInput?: string) => {
    const hasRouteInServer = mockRoutes
      ...
      .find(
        (m) =>
          m.name === pathInput &&
          m.method.toUpperCase() === mockRoute.method.toUpperCase(),
      );
    if (hasRouteInServer) {
    ...
    <Pane type="request">
      <PaneHeader>
        <MockUrlBar
          ...
          onSend={onSend}
          onPathUpdate={upsertMockbinHar}
        />
      </PaneHeader>
      <PaneBody>
        <Tabs
          aria-label="Mock response config"
            ...
            <Toolbar className="flex h-[--line-height-sm] w-full flex-shrink-0 items-center border-b border-solid border-[--hl-md] px-2">
              <Dropdown
                ...
                triggerButton={
                  <Button>
                    {mockRoute.mimeType
                      ? "Mock " + contentTypesMap[mockRoute.mimeType]?.[0]
                      : "Mock Body"}
                ...
                {mockContentTypes.map((contentType) => (
                  <DropdownItem key={contentType}>
                    <ItemContent
                      ...
                      onClick={() =>
                        patchMockRoute(mockRoute._id, { mimeType: contentType })
                      }
                    />
                  </DropdownItem>
                ))}
                ...
                <div className="form-control form-control--outlined">
                  <label htmlFor="mock-response-status-code-editor">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        patchMockRoute(mockRoute._id, {
                          statusCode: parseInt(e.currentTarget.value, 10),
                        })
                      }
                      onBlur={onBlurTriggerUpsert}
                      placeholder="200"
                ...
                <div className="form-control form-control--outlined">
                  <label htmlFor="mock-response-status-text-editor">
                    ...
                    <input
                      ...
                      onBlur={onBlurTriggerUpsert}
                      placeholder={
                        RESPONSE_CODE_REASONS[mockRoute.statusCode || 200] ||
                        "Unknown"
                      }
                    />
                  </label>

...
import type {
  ...
  RunnerTestResult,
} from "../../models/runner-test-result";
...
import {
  UploadDataModal,
  type UploadDataType,
} from "../components/modals/upload-runner-data-modal";
...
async function aggregateAllTimelines(
  ...
  for (let i = 0; i < responsesInfo.length; i++) {
    ...
    if (resp) {
      const timeline = models.response.getTimeline(
        resp,
        true,
      ) as unknown as ResponseTimelineEntry[];
      timelines = [
        ...timelines,
        {
...
export const repositionInArray = (
  allItems: string[],
  itemsToMove: string[],
  targetIndex: number,
) => {
...
export const Runner: FC<{}> = () => {
  ...
  const { reqList, requestRows, entityMap } = useRunnerRequestList(
    organizationId,
    targetFolderId,
    runnerId,
  );
  ...
  const isConsistencyChanged = useMemo(() => {
    if (requestRows.length !== reqList.length) {
      ...
      selectedKeys !== "all" &&
      Array.from(selectedKeys).length !== requestRows.length
    ) {
      return true;
    }

    return requestRows.some(
      (row: RequestRow, index: number) => row.id !== reqList[index].id,
    );
  ...
  const onRun = () => {
    ...
    window.main.trackSegmentEvent({
      ...
      properties: {
        plan: currentPlan?.type || "scratchpad",
  ...
  useEffect(() => {
    const readResults = async () => {
      const results =
        (await models.runnerTestResult.findByParentId(runnerId)) || [];
  ...
  const [executionResult, setExecutionResult] =
    useState<RunnerTestResult | null>(null);
  ...
  useEffect(() => {
    const refreshTimeline = async () => {
      if (executionResult) {
        const mergedTimelines = await aggregateAllTimelines(
          errorMsg,
          executionResult,
        );
  ...
  const refreshPanes = useCallback(async () => {
    const latestTimingSteps = await window.main.getExecution({
      requestId: runnerId,
    });
    let isRunning = false;
    if (latestTimingSteps) {
      ...
      isRunning =
        latestTimingSteps.length > 0 &&
        latestTimingSteps[latestTimingSteps.length - 1].stepName !== "Done";
    }
    setIsRunning(isRunning);

    if (isRunning) {
      const duration =
        Date.now() - latestTimingSteps[latestTimingSteps.length - 1].startedAt;
      const { number: durationNumber, unit: durationUnit } =
        getTimeAndUnit(duration);
  ...
  const { passedTestCount, totalTestCount, testResultCountTagColor } =
    useMemo(() => {
      ...
      if (!isRunning) {
        if (executionResult?.iterationResults) {
          for (let i = 0; i < executionResult.iterationResults.length; i++) {
            // iterations
            for (
              let j = 0;
              j < executionResult.iterationResults[i].length;
              j++
            ) {
              // requests
              for (
                let k = 0;
                k < executionResult.iterationResults[i][j].results.length;
                k++
              ) {
                // test cases
                if (
                  executionResult.iterationResults[i][j].results[k].status ===
                  "passed"
                ) {
                  passedTestCount++;
                }
    ...
    <>
              ...
              <Heading className="flex h-[--line-height-sm] w-full items-center pl-[--padding-md]">
                <div className="h-full w-full overflow-hidden text-left">
                  <div className="h-full min-w-[500px]">
                    ...
                    >
                      <Icon icon={file ? "eye" : "upload"} />{" "}
                      {file ? "View Data" : "Upload Data"}
                    </Button>
                  </div>
                </div>
            ...
            >
              ...
              >
                  ...
                  <span className="mr-2">
                    ...
                    Array.from(selectedKeys).length ===
                      ...
                      <span onClick={onToggleSelection}>
                        <i
                          style={{ color: "rgb(74 222 128)" }}
                          className="fa fa-square-minus fa-1x mr-2 h-4"
                        />{" "}
                        <span className="cursor-pointer">Select All</span>
                      </span>
                    )}
                ...
                <PaneBody placeholder className="p-0">
                  ...
                  >
                    {(item) => {
                      const parentFolders = item.ancestorNames.map(
                        (parentFolderName: string, i: number) => {
                          // eslint-disable-next-line react/no-array-index-key
                          return (
                            <TooltipTrigger
                              key={`parent-folder-${i}=${parentFolderName}`}
                            >
                      ...
                      const parentFolderContainer =
                        parentFolders.length > 0 ? (
                          <span className="ml-2">{parentFolders}</span>
                        ) : null;
                      ...
                      return (
                        ...
                        >
                          ...
                          <span
                            className={`ml-2 text-xs uppercase http-method-${item.method}`}
                          >
                            {item.method}
                          </span>
                          <span
              ...
              <TabPanel
                className="align-center flex w-full flex-1 overflow-y-auto"
                id="advanced"
              >
                <div className="w-full p-4">
                  <div>
                    <label className="flex items-center gap-2">
                      ...
                      <HelpTooltip className="space-left">
                        Enabling this will impact performance while responses
                        are saved for other purposes.
                      </HelpTooltip>
                    </label>
                  </div>
                  <div>
                    <label className="flex items-center gap-2">
                      ...
                      Keep logs after run
                      <HelpTooltip className="space-left">
                        Disabling this will improve the performance while logs
                        are not saved.
                      </HelpTooltip>
                    </label>
                  </div>
                  ...
                  <div>
                    <label className="flex items-center gap-2">
                      ...
                      <HelpTooltip className="space-left">
                        Enabling this will persist generated values.
                      </HelpTooltip>
                    </label>
                  </div>
                  <div>
                  ...
                  <div>
                    <label className="flex items-center gap-2">
                      ...
                      <HelpTooltip className="space-left">
                        Cookies in the running will be saved to the cookie
                        manager.
                      </HelpTooltip>
                    </label>
                  </div>
      ...
      <Panel id="pane-two" className="pane-two theme--pane">
        <PaneHeader className="row-spaced">
          <Heading className="scro flex h-[--line-height-sm] w-full items-center border-b border-solid border-b-[--hl-md] pl-3">
...
export const runCollectionAction: ActionFunction = async ({
  request,
  params,
}) => {
      ...
      while (j < requests.length) {
        ...
        const isNextRequest = (
          ...
          const matchLastIndex =
            ...
            requests.findLastIndex(
              (req) => req.name.trim() === nextRequestIdOrName.trim(),
            );
        ...
        try {
          ...
          const activeRequestMeta =
            await models.requestMeta.updateOrCreateByParentId(
              targetRequest.id,
              { lastActive: Date.now() },
            );
          ...
          const mutatedContext = (await sendActionImplementation({
            ...
            iteration: i + 1,
            iterationCount,
            userUploadEnvironment: wrapAroundIterationOverIterationData(
              userUploadEnvs,
              i,
            ),
          ...
          if (mutatedContext?.execution?.nextRequestIdOrName) {
            nextRequestIdOrName =
              mutatedContext.execution.nextRequestIdOrName || "";
          }
...
export const collectionRunnerStatusLoader: LoaderFunction = async ({
  params,
}) => {

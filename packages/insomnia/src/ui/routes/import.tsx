...
export const scanForResourcesAction: ActionFunction = async ({
  request,
}): Promise<ScanResult[]> => {
  try {
    ...
    invariant(
      ["file", "uri", "clipboard"].includes(source),
    ...
    if (source === "uri") {
      ...
      const zipFilePaths = filePaths.filter(
        (filePath) => path.extname(filePath) === ".zip",
      );
      const nonZipFilePaths = filePaths.filter(
        (filePath) => path.extname(filePath) !== ".zip",
      );
      ...
      for (const zipFilePath of zipFilePaths) {
        ...
        try {
          postmanDataDumpRawData =
            await getFilesFromPostmanExportedDataDump(zipFilePath);
        } catch (err) {
          return [
            {
              errors: [err.message],
...
export const importResourcesAction: ActionFunction = async ({
  request,
}): Promise<ImportResourcesActionResult> => {
  const formData = await request.formData();
...
async function syncNewWorkspaceIfNeeded(newWorkspace: Workspace) {
  ...
  if (
    ...
    [ORG_STORAGE_RULE.CLOUD_ONLY, ORG_STORAGE_RULE.CLOUD_PLUS_LOCAL].includes(
      await fetchAndCacheOrganizationStorageRule(project.parentId),
    )
  ) {

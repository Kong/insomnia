...
const Environments = () => {
  ...
  const selectedEnvironment = allEnvironment.find(
  ...
  const containsSecret = allEnvironment.some(
    (env) =>
      env.isPrivate &&
      env.kvPairData?.some(
        (pairData) => pairData.type === EnvironmentKvPairDataType.SECRET,
      ),
  );
  const shouldShowVaultKeyModal = containsSecret && !vaultKeyInSession;
  const [showInputVaultKeyModal, setShowModal] = useState(
    shouldShowVaultKeyModal,
  );
      ...
      <Panel id="pane-one" className="pane-one theme--pane flex flex-col">
        ...
        <div className="flex flex-1 flex-col divide-y divide-solid divide-[--hl-md] overflow-hidden">
          ...
          {showInputVaultKeyModal && (
            <InputVaultKeyModal
              onClose={handleInputVaultKeyModalClose}
              allowClose={false}
            />
          )}
        </div>

...
export const loader: LoaderFunction = async ({ params, request }) => {
  if (!params.requestId && !params.requestGroupId) {
    ...
    const activeRequest = activeRequestId
      ? await models.request.getById(activeRequestId)
      : null;
    // TODO(george): we should remove this after enabling the sidebar for the runner
    const startOfQuery = request.url.indexOf("?");
    const urlWithoutQuery =
      startOfQuery > 0 ? request.url.slice(0, startOfQuery) : request.url;
    const isDisplayingRunner = urlWithoutQuery.includes("/runner");
    const doNotSkipToActiveRequest = request.url.includes(
      "doNotSkipToActiveRequest=true",
    );
...
const getRequestNameOrFallback = (
  doc: Request | RequestGroup | GrpcRequest | WebSocketRequest,
): string => {
...
export const Debug: FC = () => {
  ...
  const { organizationId, projectId, workspaceId, requestId, requestGroupId } =
    useParams() as {
      ...
      window.main.on("grpc.error", (_, id, error) => {
        if (isGrpcConnectionError(error)) {
          showModal(ErrorModal, {
            error,
            ...getGrpcConnectionErrorDetails(error),
          });
        }
        setGrpcStates((state) =>
  ...
  const sortOrder =
    (searchParams.get("sortOrder") as SortOrder) || "type-manual";
  ...
  const collectionDragAndDrop = useDragAndDrop({
    ...
    onReorder(event) {
      ...
      const isMovingFolderInsideItsChildren =
        isRequestGroup(dropItem.doc) &&
        targetItem.ancestors?.includes(dropItem.doc._id);
      ...
      const isMovingItemInsideFolder =
        isRequestGroup(targetItem.doc) && event.target.dropPosition === "after";
      if (isMovingItemInsideFolder) {
        ...
        const targetSiblingsCollections = collection.filter(
          (r) => r.doc.parentId === targetItem.doc.parentId,
        );
        const targetIndexInSiblingsCollection =
          targetSiblingsCollections.findIndex((r) => r.doc._id === targetId);
        if (event.target.dropPosition === "after") {
          const beforeItem = targetItem;
          const afterItem =
            targetSiblingsCollections[targetIndexInSiblingsCollection + 1];

          if (beforeItem && afterItem) {
            metaSortKey =
              beforeItem.doc.metaSortKey -
    ...
    renderDropIndicator(target) {
      ...
      return (
        <DropIndicator
          target={target}
          className="absolute left-0 top-0 outline outline-1 outline-[--color-surprise]"
        />
      );
    },
  });
  ...
  useLayoutEffect(() => {
    if (
      expandAllForRequestFetcher.state !== "idle" &&
      expandAllForRequestFetcher.data &&
      requestId
    ) {
      setTimeout(() => {
        const activeIndex = collection.findIndex(
          (item) => item.doc._id === requestId,
        );
    ...
    collection,
    expandAllForRequestFetcher.data,
    expandAllForRequestFetcher.state,
    requestId,
    virtualizer,
  ]);
                        ...
                        <Collection items={section.items}>
                          {(item) => (
                            ...
                            >
                              ...
                              <span>{item.name}</span>
                              {item.hint && (
                                <DropdownHint keyBindings={item.hint} />
                              )}
                            </MenuItem>
                          )}
                        </Collection>
                          ...
                          className={`flex w-10 flex-shrink-0 items-center justify-center rounded-sm border border-solid border-[--hl-sm] text-[0.65rem] ${
                            {
                              ...
                              OPTIONS:
                                "bg-[rgba(var(--color-info-rgb),0.5)] text-[--color-font-info]",
                              DELETE:
                                "bg-[rgba(var(--color-danger-rgb),0.5)] text-[--color-font-danger]",
                              PUT: "bg-[rgba(var(--color-warning-rgb),0.5)] text-[--color-font-warning]",
                              PATCH:
                                "bg-[rgba(var(--color-notice-rgb),0.5)] text-[--color-font-notice]",
                            }[item.doc.method] ||
                            "bg-[--hl-md] text-[--color-font]"
                          }`}
                        >
                    ...
                    {workspaceId ? (
                      <ErrorBoundary showAlert>
                        ...
                        {isRequestId(requestId) && (
                          <RequestPane
                            environmentId={
                              activeEnvironment ? activeEnvironment._id : ""
                            }
                            settings={settings}
                            onPaste={(text) => {
                              setPastedCurl(text);
...
const CollectionGridListItem = ({
  ...
  patchRequest: (
    requestId: string,
    patch: Partial<GrpcRequest> | Partial<Request> | Partial<WebSocketRequest>,
  ) => void;
  ...
  const isSelected =
    item.doc._id === params.requestId || item.doc._id === params.requestGroupId;
  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        {isWebSocketRequest(item.doc) && (
          <WebSocketSpinner requestId={item.doc._id} />
        )}
        {isGraphqlSubscriptionRequest(item.doc) && (
          <WebSocketSpinner requestId={item.doc._id} />
        )}
        {isRequest(item.doc) && <RequestTiming requestId={item.doc._id} />}
        {isEventStreamRequest(item.doc) && (
          <EventStreamSpinner requestId={item.doc._id} />
        )}
        {item.pinned && (

...
export const gitRepoLoader: ActionFunction = async ({
  ...
  try {
    ...
    if (!isGitProject(project)) {
      throw new Error('Project is not a git project');
    }
...
export const gitBranchesLoader: LoaderFunction = async ({
  ...
  if (!project.gitRepositoryId) {
    return {
      errors: ['Project is not linked to a git repository'],
    };
  }
...
export const gitChangesLoader: LoaderFunction = async ({
  ...
  try {
    const {
      changes,
      // @TODO - Project uncomiitedChanges
      // hasUncommittedChanges
    } = await getGitChanges(GitVCS);
...
export const updateGitRepoAction: ActionFunction = async ({
  ...
  let gitRepositoryId = project.gitRepositoryId;
...
export const resetGitRepoAction: ActionFunction = async ({ params }) => {
  ...
  invariant(project.gitRepositoryId, 'Workspace is not linked to a git repository');

  const repo = await models.gitRepository.getById(project.gitRepositoryId);
...
export const checkoutGitBranchAction: ActionFunction = async ({
  ...
  // @TODO Project Git hasUncommittedChanges and hasUnpushedChanges and lastCommitTime and activeBranch
...
async function getGitRepoInfo(projectId: string) {
  invariant(projectId, 'Project ID is required');
  const project = await models.project.getById(projectId);
  invariant(project, 'Project not found');
...
export const pullFromGitRemote = async (projectId: string) => {
  const {
    gitRepository,
    providerName,
  } = await getGitRepoInfo(projectId);
...
export const gitStatusAction: ActionFunction = async ({ }): Promise<GitStatusResult> => {
  try {
    const {
      // @TODO Project Git hasUncommittedChanges
      // hasUncommittedChanges,
      changes,
    } = await getGitChanges(GitVCS);

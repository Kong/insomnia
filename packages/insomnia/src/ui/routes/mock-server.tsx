...
export const loader: LoaderFunction = async ({
  ...
  const mockRoutes = await models.mockRoute.findByParentId(
    activeMockServer._id,
  );
...
const MockServerRoute = () => {
  const { organizationId, projectId, workspaceId, mockRouteId } =
    useParams() as {
              ...
              .find(
                (m) =>
                  ...
                  m.method.toUpperCase() ===
                    mockRoutes.find((m) => m._id !== id)?.method.toUpperCase(),
              );
            if (hasRouteInServer) {
    ...
    >
      ...
      >
        <div className="flex flex-1 flex-col divide-y divide-solid divide-[--hl-md] overflow-hidden">
          <div
            className={`flex items-center gap-2 h-[${INSOMNIA_TAB_HEIGHT}px] px-[--padding-sm]`}
          >
            ...
            <Button
              ...
              onPress={() => {
                showPrompt({
                  ...
                  onComplete: (name) => {
                    const hasRouteInServer = mockRoutes.find(
                      (m) =>
                        m.name === name && m.method.toUpperCase() === "GET",
                    );
                    if (hasRouteInServer) {
                      showModal(AlertModal, {
                        title: "Error",

...
const TestRoute: FC = () => {
  ...
  const { unitTestSuite } =
    (useRouteLoaderData(":testSuiteId") as { unitTestSuite: UnitTestSuite }) ||
    {};
  ...
  const testSuitesDragAndDrop = useDragAndDrop({
    ...
    onReorder(e) {
      ...
      const sourceTestSuite = unitTestSuites.find(
        (testSuite) => testSuite._id === source,
      );
      const targetTestSuite = unitTestSuites.find(
        (testSuite) => testSuite._id === e.target.key,
      );
      ...
      if (dropPosition === "before") {
        const currentTestSuiteIndex = unitTestSuites.findIndex(
          (testSuite) => testSuite._id === targetTestSuite._id,
        );
      ...
      <Panel className="flex flex-col">
        ...
        <PanelGroup autoSaveId="insomnia-panels" direction={direction}>
          <Panel
            id="pane-one"
            minSize={10}
            className="pane-one theme--pane relative overflow-hidden"
          >
            <Routes>
              <Route

...
export const indexLoader: LoaderFunction = async ({ params }) => {
  ...
  const testResult =
    await models.unitTestResult.getLatestByParentId(workspaceId);
  if (testResult) {
    return redirect(
...
export const loader: LoaderFunction = async ({
  ...
  const testResult = await database.getWhere<UnitTestResult>(
    models.unitTestResult.type,
    {
      _id: testResultId,
    },
  );
...
export const TestRunStatus: FC = () => {
  ...
  return (
    <div
      key={testResult._id}
      className="flex h-full w-full flex-1 flex-col divide-y divide-solid divide-[--hl-md]"
    >
      <Heading

...
export const loader: LoaderFunction = async (args) => {
  ...
  const allOrganizations = JSON.parse(
  ...
  const allOrganizationWorkspaces = await database.find<Workspace>(
    workspace.type,
    {
  ...
  const workspaceIds = allOrganizationWorkspaces.map(
  ...
  allOrganizationWorkspaces.forEach((workspaceId) => {
    parentReferences.set(workspaceId._id, {
      type: "Workspace",
      organizationId: parentReferences.get(workspaceId.parentId)!
        .organizationId,
  ...
  const getRequestGroups = async ({
    ...
    for (const requestGroup of requestGroups) {
      parentReferences.set(requestGroup._id, {
        type: "RequestGroup",
        organizationId: parentReferences.get(requestGroup.parentId)!
          .organizationId,
    ...
    const requestGroupIds = requestGroups.map(
  ...
  for (const grpcRequest of grpcRequests) {
    parentReferences.set(grpcRequest._id, {
      type: "GrpcRequest",
      organizationId: parentReferences.get(grpcRequest.parentId)!
        .organizationId,
  ...
  const webSocketRequests = await database.find<WebSocketRequest>(
  ...
  for (const webSocketRequest of webSocketRequests) {
    parentReferences.set(webSocketRequest._id, {
      ...
      organizationId: parentReferences.get(webSocketRequest.parentId)!
        .organizationId,
        ...
        .map((item) => ({
          ...
          organizationName:
            allOrganizations.find(
              (org) =>
                org.id === parentReferences.get(item.parentId)?.organizationId,
            )?.display_name || "",
          projectName:
            allProjects.find(
              (project) =>
                project._id === parentReferences.get(item.parentId)?.projectId,
            )?.name || "",
          workspaceName:
            allOrganizationWorkspaces.find(
              (workspace) =>
                workspace._id ===
                parentReferences.get(item.parentId)?.workspaceId,
            )?.name || "",
        })),
      files: currentFiles.map((workspace) => {
        const parentProject = allProjects.find(
          (project) => project._id === workspace.parentId,
        );
        return {
          id: workspace._id,
          ...
          item: {
            ...workspace,
            teamProjectId:
              parentProject && isRemoteProject(parentProject)
                ? parentProject.remoteId
                : "",
          },
          organizationName:
            allOrganizations.find(
              (org) =>
                org.id ===
                parentReferences.get(workspace.parentId)?.organizationId,
            )?.display_name || "",
          projectName:
            allProjects.find(
              (project) =>
                project._id ===
                parentReferences.get(workspace.parentId)?.projectId,
            )?.name || "",
        };
      }),
      environments,
...
export interface RemoteFilesLoaderResult {
  files: CommandRemoteItem<
    RemoteFile & { teamProjectLocalId: string; scope: "unsynced" }
  >[];
}

export const remoteFilesLoader: LoaderFunction =
  async (): Promise<RemoteFilesLoaderResult> => {
    ...
    try {
      ...
      const allRemoteFilesOrganizationIds = remoteFiles.map(
        (file) => file.organizationId,
      );
      const allRemoteFilesProjectIds = remoteFiles.map(
        (file) => file.teamProjectId,
      );

      const organizations = allOrganizations.filter((org) =>
        allRemoteFilesOrganizationIds.includes(org.id),
      );
      ...
      const files = remoteFiles.map((file) => {
        const parentProject = projects.find(
          (project) => project.remoteId === file.teamProjectId,
        );
        return {
          id: file.id,
          ...
          item: {
            ...file,
            teamProjectLocalId: parentProject?._id || "",
            scope: "unsynced" as const,
          },
          organizationName:
            organizations.find((org) => org.id === file.organizationId)
              ?.display_name || "",

...
const { Buffer, Client, generateAES256Key, getRandomHex, params, srpGenKey } =
  srp;
...
const createVaultKeyRequest = async (
  sessionId: string,
  salt: string,
  verifier: string,
) =>
...
const resetVaultKeyRequest = async (
  sessionId: string,
  salt: string,
  verifier: string,
) =>
...
export const saveVaultKey = async (accountId: string, vaultKey: string) => {
  ...
  const encryptedVaultKey =
    await window.main.secretStorage.encryptString(vaultKey);
...
const createVaultKey = async (type: "create" | "reset" = "create") => {
  ...
  try {
    ...
    if (type === "create") {
      const response = await createVaultKeyRequest(
        sessionId,
        vaultSalt,
        verifier,
      );
      ...
      const response = await resetVaultKeyRequest(
        sessionId,
        vaultSalt,
        verifier,
      );
...
export const validateVaultKey = async (
  session: UserSession,
  vaultKey: string,
  vaultSalt: string,
) => {
...
export const clearVaultKeyAction: ActionFunction = async ({ request }) => {
  const { organizations = [], sessionId: resetVaultClientSessionId } =
    await request.json();
  ...
  if (resetVaultClientSessionId !== sessionId) {
    ...
    const notification: ToastNotification = {
      key: "Vault key reset",
      message:
        "Your vault key has been reset, all you local secrets have been deleted.",
    };
...
export const validateVaultKeyAction: ActionFunction = async ({ request }) => {
  const { vaultKey, saveVaultKey: saveVaultKeyLocally = false } =
    await request.json();
  const userSession = await sessionModel.getOrCreate();

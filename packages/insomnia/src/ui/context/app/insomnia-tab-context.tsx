...
export const InsomniaTabProvider: FC<PropsWithChildren> = ({ children }) => {
  ...
  const [appTabs, setAppTabs] = useLocalStorage<InsomniaTabs>(
    "insomnia-tabs",
    {},
  );
  ...
  const addTab = useCallback(
    (tab: BaseTab) => {
      ...
      if (tab.temporary) {
        ...
        const temporaryIndex = currentTabs.tabList.findIndex(
          (t) => t.temporary,
        );
        if (temporaryIndex !== -1) {
  ...
  const batchCloseTabs = useCallback(
    (deleteIds: string[]) => {
      ...
      const index = currentTabs.tabList.findIndex((tab) =>
        deleteIds.includes(tab.id),
      );
      const newTabList = currentTabs.tabList.filter(
        (tab) => !deleteIds.includes(tab.id),
      );
  ...
  const closeAllTabsUnderWorkspace = useCallback(
    (workspaceId: string) => {
      ...
      const closeIds = currentTabs.tabList
  ...
  return (
    <InsomniaTabContext.Provider
      value={{
        currentOrgTabs: appTabs?.[organizationId] || {
          tabList: [],
          activeTabId: "",
        },
        closeTabById,

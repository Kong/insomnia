...
} from "../components/tags/method-tag";
...
export const useInsomniaTab = ({
  ...
  const { appTabsRef, addTab, changeActiveTab, closeTabById } =
  ...
  const getCurrentTab = useCallback(
    (type: TabType | null) => {
      ...
      if (type === "request") {
        return currentOrgTabs?.tabList.find(
          (tab) => tab.id === activeRequest?._id,
        );
      }

      if (type === "folder") {
        return currentOrgTabs?.tabList.find(
          (tab) => tab.id === activeRequestGroup?._id,
        );
      }
      ...
      if (type === "mockRoute") {
        return currentOrgTabs?.tabList.find(
          (tab) => tab.id === activeMockRoute?._id,
        );
      }

      if (type === "testSuite") {
        return currentOrgTabs?.tabList.find(
          (tab) => tab.id === unitTestSuite?._id,
        );
      }

      const collectionTabTypes: TabType[] = [
        "collection",
        "document",
        "environment",
        "mockServer",
  ...
  useEffect(() => {
    ...
    if (!currentTab && type) {
      ...
      if (tabInfo) {
        ...
        if (
          (type === "request" || type === "folder" || type === "collection") &&
    ...
    if (currentTab) {
      const currentActiveTabId =
        appTabsRef?.current?.[organizationId]?.activeTabId;

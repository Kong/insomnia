...
export const useThemes = () => {
  ...
  const { lightTheme, darkTheme, autoDetectColorScheme, theme, pluginConfig } =
    settings;
  ...
  const isActiveDark = useCallback(
    ({ name }: PluginTheme) => name === darkTheme,
    [darkTheme],
  );
  const isActiveLight = useCallback(
    ({ name }: PluginTheme) => name === lightTheme,
    [lightTheme],
  );
  ...
  const changeAutoDetect = useCallback(
    (autoDetectColorScheme: boolean) => apply({ autoDetectColorScheme }),
    [apply],
  );

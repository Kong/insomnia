  ...
  const { dragAndDropHooks } = useDragAndDrop({
    ...
    renderDropIndicator(target) {
      return (
        <DropIndicator
          target={target}
          className="z-10 outline-solid outline-1 data-drop-target:outline-(--color-surprise)"
        />
      );
    },
  });
            ...
            if (isFile) {
              valueEditor = (
                <FileInputButton
                  ...
                  className="flex h-full w-full flex-1 shrink-0 items-center justify-center gap-2 overflow-hidden rounded-xs px-2 py-1 text-sm text-(--color-font) ring-1 ring-transparent transition-all hover:bg-(--hl-xs) focus:ring-inset focus:ring-(--hl-md) aria-pressed:bg-(--hl-sm)"
                  path={pair.fileName || ''}
            ...
            if (isMultiline) {
              valueEditor = (
                <Button
                  ...
                  className="flex h-full w-full flex-1 items-center justify-center gap-2 overflow-hidden rounded-xs px-2 py-1 text-sm text-(--color-font) ring-1 ring-transparent transition-all hover:bg-(--hl-xs) focus:ring-inset focus:ring-(--hl-md) aria-pressed:bg-(--hl-sm)"
                  onPress={() =>
                ...
                <Toolbar className="flex items-center gap-1">
                  <MenuTrigger>
                    <Button
                      ...
                      className="flex aspect-square h-7 items-center justify-center rounded-xs text-sm text-(--color-font) ring-1 ring-transparent transition-all hover:bg-(--hl-xs) focus:ring-inset focus:ring-(--hl-md) aria-pressed:bg-(--hl-sm)"
                    >

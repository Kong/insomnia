...
export const KeyValueEditor: FC<Props> = ({
  ...
  const initialReadOnlyItems =
    readOnlyPairs?.map((pair) => ({
      ...pair,
      id: pair.id || generateId("pair"),
    })) || [];
  ...
  const upsertPair = useCallback(
    function upsertPair(pairsListItems: Pair[], pair: Pair) {
      if (pairsListItems.find((item) => item.id === pair.id)) {
        onChange(
          pairsListItems.map((item) => (item.id === pair.id ? pair : item)),
        );
      } else {
  ...
  const { dragAndDropHooks } = useDragAndDrop({
    getItems: (keys) =>
      [...keys].map((key) => ({
        "text/plain": `${pairsListItems.find((item) => item.id === key.toString())?.id}`,
      })),
    onReorder(e) {
      onChange(
        repositionInArray(
          ...
          pairsListItems.findIndex(
            (item) => item.id === e.target.key.toString(),
          ),
        ),
      );
    },
    renderDragPreview(items) {
      const pair =
        pairsListItems.find((item) => item.id === items[0]["text/plain"]) ||
        createEmptyPair();
      ...
      const isMultiline =
        "type" in pair && pair.type === "text" && pair.multiline;
      ...
      {initialReadOnlyItems.length > 0 && (
        ...
        >
          {(pair) => {
            ...
            const bytes = isMultiline
              ? Buffer.from(pair.value, "utf8").length
              : 0;
        ...
        {(pair) => {
          ...
          let valueEditor = (
            <OneLineEditor
              ...
              onChange={(value) =>
                upsertPair(pairsListItems, { ...pair, value })
              }
            />
          );
          ...
          if (isFile) {
            valueEditor = (
              <FileInputButton
                ...
                path={pair.fileName || ""}
                onChange={(fileName) =>
                  upsertPair(pairsListItems, { ...pair, fileName })
                }
              />
            );
          }
                ...
                onPress={() =>
                  showModal(CodePromptModal, {
                    ...
                    onModeChange: (mode: string) =>
                      upsertPair(pairsListItems, { ...pair, multiline: mode }),
                  })
                }
              >
                <i className="fa fa-pencil-square-o space-right" />
            ...
            >
              ...
              {showDescription && (
                <OneLineEditor
                  id={"key-value-editor__description" + pair.id}
                  key={
                    "key-value-editor__description" + pair.id + pair.disabled
                  }
                  placeholder={descriptionPlaceholder || "Description"}
                  defaultValue={pair.description || ""}
                  readOnly={pair.disabled || isDisabled}
                  onChange={(description) =>
                    upsertPair(pairsListItems, { ...pair, description })
                  }
                />
              )}
              <Toolbar className="flex items-center gap-1">
                ...
                <PromptButton
                  ...
                  onClick={() => {
                    if (pairsListItems.find((item) => item.id === pair.id)) {
                      pairsListItems = pairsListItems.filter(

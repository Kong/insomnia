...
export const OneLineEditor = forwardRef<
  OneLineEditorHandle,
  OneLineEditorProps
>(
  (
    ...
    const getKeyMap = useCallback(() => {
      if (
        !readOnly &&
        settings.enableKeyMapForInlineTextEditors &&
    ...
    const initEditor = useCallback(() => {
      ...
      const canAutocomplete = !!(
        handleGetRenderContext || getAutocompleteConstants
      );
      ...
      codeMirror.current.on(
        "beforeChange",
        (_: CodeMirror.Editor, change: CodeMirror.EditorChangeCancellable) => {
      ...
      codeMirror.current.on("blur", () =>
        codeMirror.current
          ?.getTextArea()
          .parentElement?.removeAttribute("data-focused"),
      );
      codeMirror.current.on("focus", () =>
        codeMirror.current
          ?.getTextArea()
          .parentElement?.setAttribute("data-focused", "on"),
      );
      codeMirror.current.on(
        "keyHandled",

...
import {
  ...
  type nunjucksTagContextMenuOptions,
...
export const shouldIndentWithTabs = ({
  mode,
  indentWithTabs,
}: {
  mode?: string;
...
const widget = (
  ...
  try {
    const squareBraceIsOutsideCurlyBrace =
      prevLine.lastIndexOf("[") > prevLine.lastIndexOf("{");
    const startToken = squareBraceIsOutsideCurlyBrace ? "[" : "{";
    const endToken = squareBraceIsOutsideCurlyBrace ? "]" : "}";
    const keys = Object.keys(
      JSON.parse(startToken + cm?.getRange(from, to) + endToken),
    );
...
export const CodeEditor = memo(
  forwardRef<CodeEditorHandle, CodeEditorProps>(
    (
      ...
      const indentWithTabs = shouldIndentWithTabs({
        mode,
        indentWithTabs: settings.editorIndentWithTabs,
      });
      const indentChars = indentWithTabs
        ? "\t"
        : new Array((indentSize || TAB_SIZE) + 1).join(" ");
      ...
      const initEditor = useCallback(() => {
          ...
          if (tagDef.args[0]?.type === "enum") {
            return (
              tagDef.args[0].options?.map((option) => {
                const optionName = misc.fnOrString(
                  option.displayName,
                  tagDef.args,
                );
        ...
        const initialOptions: EditorConfiguration = {
          ...
          foldOptions: {
            widget: (from: CodeMirror.Position, to: CodeMirror.Position) =>
              widget(codeMirror.current, from, to),
          },
          mode: !handleRender
          ...
          environmentAutocomplete: {
            getVariables: async () =>
              !handleGetRenderContext
                ? []
                : (await handleGetRenderContext())?.keys || [],
            getTags: async () =>
              !handleGetRenderContext
                ? []
                : (await getTagDefinitions()).map(transformEnums).flat(),
        ...
        codeMirror.current.on(
          ...
          (
            ...
            const isGraphqlWithChange =
              doc.getOption("mode") === "graphql" && change.text.length > 0;
            if (isGraphqlWithChange) {
              // Don't allow non-breaking spaces because they break the GraphQL syntax
              change.update?.(
                change.from,
                change.to,
                change.text.map(normalizeIrregularWhitespace),
              );
            }
            if (pinToBottom) {
        ...
        codeMirror.current.on(
          "keydown",
          (doc: CodeMirror.Editor, event: KeyboardEvent) => {
            ...
            const isUserDefinedKeyboardShortcut = isKeyCombinationInRegistry(
              pressedKeyComb,
              settings.hotKeyRegistry,
            );
            const isAutoCompleteBinding = isKeyCombinationInRegistry(
              pressedKeyComb,
              {
            ...
            const isShortcutButNotAutocomplete =
              isUserDefinedKeyboardShortcut && !isAutoCompleteBinding;
            ...
            if (isShortcutButNotAutocomplete) {
              ...
              if (doc.getOption("mode") === "graphql") {
                ...
                const isDropdownActive =
                  codeMirror.current?.isHintDropdownActive();
                if (
                  (isAutoCompleteBinding || !isModifier) &&
                  !isDropdownActive
                ) {
        ...
        codeMirror.current.on(
          "keyHandled",
          (_: CodeMirror.Editor, _keyName: string, event: Event) =>
            event.stopPropagation(),
        );
        ...
        if (uniquenessKey && editorStates[uniquenessKey]) {
          const { scroll, selections, cursor, history, marks } =
            editorStates[uniquenessKey];
          ...
          codeMirror.current.setCursor(cursor.line, cursor.ch, {
            scroll: false,
          });
          codeMirror.current.setSelections(selections, undefined, {
            scroll: false,
          });
      ...
      useEffect(() => {
        const fn = misc.debounce((doc: CodeMirror.Editor) => {
          if (onChange) {
            ...
            const withinLintingThresholds =
              value.length > 0 && value.length < MAX_SIZE_FOR_LINTING;
            ...
            try {
              ...
              console.log(
                "[codemirror] Failed to set CodeMirror option",
                err.message,
              );
            }
      ...
      useEffect(() => {
        const handleOnBlur = (_: CodeMirror.Editor, e: FocusEvent) =>
          onBlur?.(e);
      ...
      useEffect(() => {
        const handleOnFocus = (_: CodeMirror.Editor, e: FocusEvent) =>
          onFocus?.(e, _);
        ...
        const unsubscribe = window.main.on(
          ...
          (_, { key, tag, nunjucksTag, needsEnterprisePlan, displayName }) => {
            if (id === key) {
              ...
              if (nunjucksTag) {
                const { type, template, range } =
                  nunjucksTag as nunjucksTagContextMenuOptions;
                switch (type) {
                  case "edit":
      ...
      useEffect(
        () => tryToSetOption("hintOptions", hintOptions),
        [hintOptions],
      );
        ...
        () => ({
          ...
          selectAll: () =>
            codeMirror.current?.setSelection(
              { line: 0, ch: 0 },
              { line: codeMirror.current.lineCount(), ch: 0 },
            ),
          ...
          scrollToSelection: (
            ...
            codeMirror.current?.scrollIntoView(
              { line: lineStart, ch: chStart },
          ...
          focusEnd: () => {
            ...
            codeMirror.current
              ?.getDoc()
              ?.setCursor(codeMirror.current.getDoc().lineCount(), 0);
          },
          getCursor: () => {
          ...
          indexFromPos: (pos?: CodeMirror.Position) =>
            pos ? codeMirror.current?.indexFromPos(pos) || 0 : 0,
      ...
      const showFilter =
        readOnly && (mode?.includes("json") || mode?.includes("xml"));
      const showPrettify =
        (showPrettifyButton && mode?.includes("json")) || mode?.includes("xml");
        ...
        <div
          ...
          onContextMenu={(event) => {
            ...
            if (target?.classList?.contains("nunjucks-tag")) {
              ...
              const nunjucksTag = extractNunjucksTagFromCoords(
              ...
              <Toolbar className="flex h-full items-center">
                ...
                {showFilter ? (
                  <Button
                    ...
                    onPress={() =>
                      showModal(FilterHelpModal, {
                        isJSON: Boolean(mode?.includes("json")),
                      })
                    }
                  >

      ...
      args: [
        {
          ...
          type: "enum",
          options: Object.keys(fakerFunctions).map((key) => ({
            displayName: key,
            value: key,
          })),
        },
  ...
  },
  {
    templateTag: {
      ...
      async run(context, url, name) {
        ...
        const cookieJar =
          await context.util.models.cookieJar.getOrCreateForWorkspace(
            workspace,
          );
  ...
  },
  {
    templateTag: {
      ...
      async run(
        ...
        const storageKey =
          explicitStorageKey || `${context.meta.requestId}.${titleHash}`;
  ...
  },
  {
    templateTag: {
        ...
        },
        {
          ...
          options: [
            ...
            },
            {
              ...
              description:
                "resend the request if current response is older than Max age seconds",
              value: "when-expired",
            },
            {
              displayName: "Always",
      ...
      async run(context, field, id, filter, resendBehavior, maxAgeSeconds) {
        ...
        const globalEnvironmentId =
          context.context.getGlobalEnvironmentId?.() || null;
        ...
        if (!response.statusCode) {
          console.log(
            "[response tag] Invalid status code " + response.statusCode,
          );
        ...
        const bodyBuffer = await context.util.models.response.getBodyBuffer(
          response,
          "",
        );
        const match =
          response.contentType &&
          response.contentType.match(/charset=([\w-]+)/);
        ...
        if (field === "header") {
          ...
          const header = response.headers.find(
            (h) => h.name.toLowerCase() === sanitizedFilter.toLowerCase(),
          );
          if (!header) {
            const names = response.headers
              ...
              .join(",\n\t");
            throw new Error(
              `No header with name "${sanitizedFilter}".\nChoices are [\n\t${names}\n]`,
            );
          }
          return header.value;
        }
        ...
        if (field === "body" && bodyBuffer !== null) {
          ...
          if (sanitizedFilter.indexOf("$") === 0) {
            ...
            try {
              const selectedValues = (await import("xpath")).select(
                sanitizedFilter,
                dom,
              );
              ...
              if (results.length === 0) {
                ...
                throw new Error(
                  `Returned more than one result: ${sanitizedFilter}`,
                );
              }
      ...
      async run(context, attribute, name, folderIndex) {
        ...
        if (attribute === "parameter") {
          ...
          for (const queryParameter of request.parameters) {
            const queryParameterName = await context.util.render(
              queryParameter.name,
            );
          ...
          const parameterNamesStr = parameterNames
            .map((n) => `"${n}"`)
            .join(",\n\t");
          throw new Error(
        ...
        if (attribute === "folder") {
          const ancestors =
            await context.util.models.request.getAncestors(request);
...
export const localTemplateTags: TemplateTag[] = localTemplatePlugins.map(
  (t) => ({
    plugin: {

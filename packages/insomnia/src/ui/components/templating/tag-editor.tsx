...
export const TagEditor: FC<Props> = (props) => {
  ...
  const refreshModels = useCallback(async () => {
    ...
    allDocs[models.request.type] = sortRequests(
      (allDocs[models.request.type] || []).concat(
        allDocs[models.requestGroup.type] || [],
      ),
      props.workspace._id,
    );
  ...
  function handleChange(
    ...
    if (event.currentTarget.parentNode instanceof HTMLElement) {
      const index =
        event.currentTarget.parentNode?.getAttribute("data-arg-index");
  ...
  async function update(
    ...
    if (!activeTagData && tagDefinition) {
      activeTagData = templateUtils.tokenizeTag(
        templateUtils.getDefaultFill(tagDefinition.name, tagDefinition.args),
      );
  ...
  function resolveRequestGroupPrefix(
  ...
  const { error, preview, activeTagDefinition, activeTagData, rendering } =
    state;
  if (!activeTagData) {
    return null;
  }
  ...
  if (
    activeTagDefinition?.disablePreview &&
    activeTagDefinition.disablePreview(activeTagData.args)
  ) {
  ...
  if (error) {
    ...
    previewElement = (
      <textarea value={finalPreview || "error"} readOnly rows={5} />
    );
  }
  ...
  return (
    <div>
      <div className="form-control form-control--outlined">
        <label>
          ...
          <select
            onChange={async (event) => {
              ...
              const tagDefinitions = await templating.getTagDefinitions();
              const tagDefinition =
                tagDefinitions.find((d) => d.name === name) || null;
        ...
        {activeTagDefinition?.name === "response" &&
          props.editorId?.includes("environment") && (
            <p className="warning mt-2 text-sm">
              <Icon icon="exclamation-circle" />
              <a href={docsAfterResponseScript}>
                {" "}
                We suggest to save your response into an environment variable
                using after-response script.
              </a>
            </p>
          )}
      </div>
      {activeTagDefinition?.args.map(
        (argDefinition: NunjucksParsedTagArg, index) => {
          ...
          if (
            ...
            argDefinition.hide(activeTagData.args)
          ) {
            return null;
          }
          ...
          const strValue = templateUtils.decodeEncoding(
            argData.value?.toString() || "",
          );
          ...
          if (!isVariable) {
            if (argDefinition.type === "string") {
                ...
                >
                  ...
                  {state.allDocs[
                    ...
                    if (isRequest(doc)) {
                      ...
                      const request: any = requests.find(
                        (r) => r._id === doc._id,
                      );
                      const method =
                      ...
                      const allRequestGroups =
                        state.allDocs[models.requestGroup.type] || [];
              ...
              argInput = (
                <input
                  type="checkbox"
                  checked={strValue.toLowerCase() === "true"}
                  onChange={handleChange}
                />
              );
            ...
            <div key={uniqueKey} className="form-row">
              <div className={formControlClasses}>
                <label data-arg-index={index}>
                  ...
                  {validationError && (
                    <span className="font-error space-left">
                      {validationError}
                    </span>
                  )}
                  {isVariable ? (
                        ...
                        <ItemContent
                          ...
                          onClick={() => {
                            ...
                            const variable = variables.find(
                              (v) => v.name === existingValue,
                            );
                            const value = variable ? variable.value : "";
                            return updateArg(value, index, initialType, {
                              quotedBy: "'",
                            });
                          }}
                        />
                      ...
                      <DropdownItem aria-label="Environment Variable">
                        <ItemContent
                          ...
                          onClick={() => {
                            ...
                            const existingValue = argData ? argData.value : "";
                            const variable = variables.find(
                              (v) => v.value === existingValue,
                            );
                            const firstVariable = variables.length
                              ? variables[0].name
                              : "";
      ...
      {activeTagDefinition?.actions &&
      activeTagDefinition?.actions?.length > 0 ? (
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    const templateTags = [
                      ...pluginTemplateTags,
                      ...localTemplateTags,
                    ] as plugins.TemplateTag[];
                    const activeTemplateTag = templateTags.find(
                      ({ templateTag }) => {
        ...
        <div className="form-control form-control--outlined">
          <label>
            ...
            <input
              ...
              onChange={(event) => {
                const { tagDefinitions, activeTagData, activeTagDefinition } =
                  state;

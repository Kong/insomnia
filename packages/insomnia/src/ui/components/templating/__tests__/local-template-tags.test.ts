...
describe("base64 tag", () => {
  describe("encoder", () => {
    const base64EncoderTag = localTemplateTags.find(
      (p) => p.templateTag.name === "base64",
    )?.templateTag;
    invariant(base64EncoderTag, "missing tag in localTemplateTags");
    it("encodes from normal", () => {
      const encoded = base64EncoderTag.run(
        {} as PluginTemplateTagContext,
        "encode",
        "normal",
        "hello",
      );

...
export const GrpcRequestPane: FunctionComponent<Props> = ({
  ...
  useMount(async () => {
    if (activeRequest.protoFileId) {
      console.log(
        `[gRPC] loading proto file methods pf=${activeRequest.protoFileId}`,
      );
      const methods = await window.main.grpc.loadMethods(
        activeRequest.protoFileId,
      );
      setGrpcState({ ...grpcState, methods });
    } else if (activeRequest.url && activeRequest.reflectionApi) {
      const requestGroups = (
        await db.withAncestors<GrpcRequest | RequestGroup>(activeRequest, [
      ...
      const rendered = await tryToInterpolateRequestOrShowRenderErrorModal({
        ...
        payload: {
          ...
          metadata: getOrInheritHeaders({
            request: { headers: activeRequest.metadata },
            requestGroups,
          }),
      ...
      const workspaceClientCertificates =
        await models.clientCertificate.findByParentId(workspaceId);
      const clientCertificate = workspaceClientCertificates.find(
        (c) =>
          !c.disabled &&
          urlMatchesCertHost(
            setDefaultProtocol(c.host, "grpc:"),
            rendered.url,
            false,
          ),
      );
      const caCertificate =
        await models.caCertificate.findByParentId(workspaceId);
      const caCertificatePath =
        caCertificate && !caCertificate.disabled
          ? caCertificate.path
          : undefined;
      ...
      const methods = await window.main.grpc.loadMethodsFromReflection(
  ...
  const method = methods.find(
    (c) => c.fullPath === activeRequest.protoMethodName,
  );
  ...
  return (
    <>
      <Pane type="request">
        <PaneHeader>
          <div className="flex h-full w-full flex-row items-stretch justify-between">
            ...
            <div className="flex flex-1 items-center gap-[--padding-xs] pr-[--padding-sm]">
              ...
              >
                <Tooltip
                  message="Click to replace body with an example"
                  position="bottom"
                  delay={500}
                >
              ...
              <Button
                ...
                onClick={async () => {
                  try {
                    const requestGroups = (
                      await db.withAncestors<GrpcRequest | RequestGroup>(
                        activeRequest,
                        [models.requestGroup.type],
                      )
                    ).filter(isRequestGroup);
                    ...
                    const workspaceClientCertificates =
                      await models.clientCertificate.findByParentId(
                        workspaceId,
                      );
                    const clientCertificate = workspaceClientCertificates.find(
                      (c) =>
                    ...
                    const methods =
                      await window.main.grpc.loadMethodsFromReflection(
                        rendered,
                      );
                    setGrpcState({ ...grpcState, methods });
              ...
              >
                <Tooltip
                  message="Click to use server reflection"
                  position="bottom"
                  delay={500}
                >
              ...
              >
                <Tooltip
                  message="Click to change proto file"
                  position="bottom"
                  delay={500}
                >
        ...
        <PaneBody>
          <Tabs
            aria-label="Grpc request pane tabs"
                  ...
                  {running && canClientStream(methodType) && (
                    <div className="box-border flex h-[var(--line-height-sm)] flex-row justify-end border-b border-[var(--hl-lg)] p-1">
                      <button
                        ...
                        onClick={async () => {
                          const requestBody =
                            await getRenderedGrpcRequestMessage({
                              request: activeRequest,
                              environment: environmentId,
                              purpose: "send",
                            });
                  ...
                  >
                    ...
                    >
                      <CodeEditor
                        ...
                        onChange={(text) =>
                          patchRequest(requestId, { body: { text } })
                        }
            ...
            <TabPanel className={"h-full w-full overflow-y-auto"} id="headers">
              ...
              >
                <KeyValueEditor
                  ...
                  onChange={(metadata: GrpcRequestHeader[]) =>

...
} from "../editors/request-headers-editor";
...
export const RequestPane: FC<Props> = ({
  ...
  const [dismissPathParameterTip, setDismissPathParameterTip] = useLocalStorage(
  ...
  const pathParameters = getCombinedPathParametersFromUrl(
    activeRequest.url,
  ...
  const parametersCount =
    ...
    activeRequest.parameters.filter((p) => !p.disabled).length;
  const headersCount =
    activeRequest.headers.filter((h) => !h.disabled).length +
    readOnlyHttpPairs.length;
  ...
  const isBodyEmpty = Boolean(
    typeof activeRequest.body.mimeType !== "string" && !activeRequest.body.text,
  );
  const requestAuth = getAuthObjectOrNull(activeRequest.authentication);
  const isNoneOrInherited =
    requestAuth?.type === "none" || requestAuth === null;
  ...
  return (
    <Pane type="request">
      ...
      >
        <TabList
          className="scrollbar-thin scro flex h-[--line-height-sm] w-full flex-shrink-0 items-center overflow-x-auto border-b border-solid border-b-[--hl-md] bg-[--color-bg]"
          aria-label="Request pane tabs"
        >
          <Tab
        ...
        >
          ...
          <PanelGroup className="flex-1 overflow-hidden" direction={"vertical"}>
            <Panel minSize={20}>
              <div className="flex h-full flex-col">
                <div className="flex h-4 w-full items-center justify-between p-4">
                  <Heading className="text-xs font-bold uppercase text-[--hl]">
                    Query parameters
                  </Heading>
            ...
            <Panel minSize={20}>
              <div className="flex h-full flex-col">
                <Heading className="p-4 text-xs font-bold uppercase text-[--hl]">
                  Path parameters
                </Heading>
                ...
                {pathParameters.length === 0 && !dismissPathParameterTip && (
                  <div className="flex items-center gap-2 rounded-sm border border-solid border-[--hl-md] p-2 text-sm text-[--hl]">
                    ...
                    <span>
                      Path parameters are url path segments that start with a
                      colon ':' e.g. ':id'{" "}
                    </span>
                    <Button
        ...
        <TabPanel className="w-full flex-1 overflow-y-auto" id="docs">
          <MarkdownEditor
            ...
            onChange={(description: string) =>
              patchRequest(requestId, { description })
            }
          />
        </TabPanel>
      </Tabs>

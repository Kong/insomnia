...
export const RunnerTestResultPane: FC<Props> = ({ result }) => {
  ...
  const resultsByIteration = result.iterationResults.map(
    (iterationResults: RunnerResultPerRequest[], i: number) => {
      const key = `runner-test-result-iteration-${i + 1}`;

      if (Array.isArray(iterationResults)) {
        const resultByRequest = iterationResults.map(
          (requestTestResult: RunnerResultPerRequest, i: number) => {
        ...
        return (
          <div
            key={key}
            data-testid={key}
            className="border-b border-dashed border-b-[--hl-md] pb-6 pt-6"
          >
            <div className="mb-3 pl-3 font-bold uppercase leading-10">
              {" "}
              Iteration {i + 1}{" "}
            </div>

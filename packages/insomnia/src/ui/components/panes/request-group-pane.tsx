...
} from "../editors/environment-editor";
...
export const RequestGroupPane: FC<{ settings: Settings }> = ({ settings }) => {
  ...
  const [isRequestGroupSettingsModalOpen, setIsRequestGroupSettingsModalOpen] =
    useState(false);
  ...
  const requestGroupAuth = getAuthObjectOrNull(
    activeRequestGroup.authentication,
  );
  const isNoneOrInherited =
    requestGroupAuth?.type === "none" || requestGroupAuth === null;
          ...
          >
            ...
            {Boolean(
              activeRequestGroup.preRequestScript ||
                activeRequestGroup.afterResponseScript,
            ) && (
              <span className="flex h-6 min-w-6 items-center justify-center rounded-lg border border-solid border-[--hl] p-1 text-xs">
                <span className="h-2 w-2 rounded-full bg-green-500" />
              </span>
              ...
              >
                <RequestScriptEditor
                  ...
                  onChange={(preRequestScript) =>
                    patchRequestGroup(activeRequestGroup._id, {
                      preRequestScript,
                    })
                  }
                  settings={settings}
              ...
              <ToggleButton
                onChange={(isSelected) => {
                  if (activeRequestGroup) {
                    ...
                    const isValidJSON =
                      !!environmentEditorRef.current?.isValid();
                    handleToggleEnvironmentType(
                      isSelected,
                      {
                        data: environment,
                        dataPropertyOrder: environmentPropertyOrder,
                        kvPairData,
                      },
                      isValidJSON,
                ...
                isSelected={
                  activeRequestGroup?.environmentType !== EnvironmentType.KVPAIR
                }
            ...
            <div className="flex h-[calc(100%-var(--line-height-md))] flex-col">
              ...
              activeRequestGroup.environmentType === EnvironmentType.KVPAIR ? (
                ...
                <EnvironmentEditor
                  ...
                  key={activeRequestGroup ? activeRequestGroup._id : "n/a"}
                  environmentInfo={{
                    object: activeRequestGroup
                      ? activeRequestGroup.environment
                      : {},
                    propertyOrder:
                      activeRequestGroup &&
                      activeRequestGroup.environmentPropertyOrder,
                  }}
                  onBlur={saveChanges}
                />
              )}
        ...
        <TabPanel className="w-full flex-1 overflow-y-auto" id="docs">
          <MarkdownEditor
            ...
            onChange={(description: string) =>
              patchRequestGroup(activeRequestGroup._id, { description })
            }
          />
        </TabPanel>
      </Tabs>

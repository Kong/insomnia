...
export const RunnerResultHistoryPane: FC<Props> = ({
  ...
  const rows = history.map((runnerResult: RunnerTestResult) => {
    ...
    for (let i = 0; i < runnerResult.iterationResults.length; i++) {
      // iterations
      for (let j = 0; j < runnerResult.iterationResults[i].length; j++) {
        // requests
        for (
          let k = 0;
          k < runnerResult.iterationResults[i][j].results.length;
          k++
        ) {
          // test cases
    ...
    const { number: durationNumber, unit: durationUnit } = getTimeAndUnit(
      runnerResult.duration,
    );

...
export const ResponsePane: FC<Props> = ({ activeRequestId }) => {
  const { activeRequest, activeRequestMeta, activeResponse } =
  ...
  const { isExecuting, steps } = useExecutionState({
      ...
      >
        ...
        >
          ...
          >
            <div>
              <span>Tests</span>
              <span
                className={`ml-1 rounded-sm px-1 ${testResultCountTagColor}`}
                style={{ color: "white" }}
              >
                {`${passedTestCount} / ${totalTestCount}`}
              </span>
        ...
        >
          <Toolbar className="flex h-[--line-height-sm] w-full flex-shrink-0 items-center border-b border-solid border-[--hl-md] px-2">
            <PreviewModeDropdown
              ...
              copyToClipboard={async () => {
                const bodyBuffer = activeResponse
          ...
          <ResponseViewer
            ...
            previewMode={
              activeResponse.error ? PREVIEW_MODE_SOURCE : previewMode
            }
        ...
        <TabPanel
          ...
          id="test-results"
        >
          <RequestTestResultPane
            requestTestResults={activeResponse.requestTestResults}
          />
        </TabPanel>
        <TabPanel
          className="flex w-full flex-1 flex-col overflow-y-auto"

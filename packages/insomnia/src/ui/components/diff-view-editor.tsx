...
export const DiffEditor = ({
  ...
  useEffect(() => {
    ...
    function getColorVariableAsHex(
      colorVariable: string,
      lightnessLimit = 100,
    ) {
      try {
        const color = parseColor(
          computedStyles.getPropertyValue(colorVariable),
        );
        if (
          color.toFormat("hsl").getChannelValue("lightness") > lightnessLimit
        ) {
          const newColor = color
            .toFormat("hsl")
            .withChannelValue("lightness", lightnessLimit);
    ...
    monaco.editor.defineTheme("insomnia", {
      ...
      colors: {
        ...
        "diffEditor.insertedTextBackground": getColorVariableAsHex(
          "--color-success",
          20,
        ),
        "diffEditor.removedTextBackground": getColorVariableAsHex(
          "--color-danger",
          20,
        ),
        "diffEditor.insertedLineBackground": getColorVariableAsHex(
          "--color-success",
          40,
        ),
        "diffEditor.removedLineBackground": getColorVariableAsHex(
          "--color-danger",
          40,
        ),
        "diffEditorGutter.insertedLineBackground": getColorVariableAsHex(
          "--color-success",
          40,
        ),
        "diffEditorGutter.removedLineBackground": getColorVariableAsHex(
          "--color-danger",
          40,
        ),
        "diffEditorOverview.insertedForeground": getColorVariableAsHex(
          "--color-success",
          20,
        ),
        "diffEditorOverview.removedForeground": getColorVariableAsHex(
          "--color-danger",
          20,
        ),
        "diffEditor.unchangedRegionBackground":
          getColorVariableAsHex("--color-bg"),
        "diffEditor.unchangedRegionForeground":
          getColorVariableAsHex("--color-font"),

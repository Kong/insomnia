...
export const EnvironmentPicker = ({
  ...
  const isUsingInsomniaCloudSync = Boolean(
    isRemoteProject(activeProject) && !activeWorkspaceMeta?.gitRepositoryId,
  );
  const isUsingGitSync = Boolean(
  ...
  const collectionEnvironmentList = [baseEnvironment, ...subEnvironments].map(
    ({ type, ...environment }) => ({
      ...environment,
      id: environment._id,
  ...
  const selectedGlobalBaseEnvironmentId =
    activeGlobalEnvironment?.parentId.startsWith("wrk")
      ? activeGlobalEnvironment._id
      : activeGlobalEnvironment?.parentId;
  const selectedGlobalBaseEnvironment = globalBaseEnvironments.find(
    (e) => e._id === selectedGlobalBaseEnvironmentId,
  );

  const globalEnvironmentList = selectedGlobalBaseEnvironment
    ? [
        selectedGlobalBaseEnvironment,
        ...globalSubEnvironments.filter(
          (e) => e.parentId === selectedGlobalBaseEnvironment._id,
        ),
  ...
  const activeSubEnvironment = subEnvironments.find(
    (e) => e._id === activeEnvironment._id,
  );
        ...
        <div className="flex w-full flex-1 items-center gap-2">
          ...
          <span className="truncate">
            {activeSubEnvironment
              ? activeSubEnvironment.name
              : activeBaseEnvironment.name}
          </span>
        </div>
      </Button>
      <Popover
        className="!z-10 flex max-h-[90vh] min-w-max flex-col"
        ...
        <Dialog className="grid h-full w-full select-none grid-flow-col divide-x divide-solid divide-[--hl-md] overflow-hidden rounded-md border border-solid border-[--hl-sm] bg-[--color-bg] text-sm shadow-lg [grid-auto-columns:min(260px,calc(40vw))_min(260px,calc(40vw))] focus:outline-none">
          <div className="relative flex h-full w-full flex-1 flex-col overflow-hidden">
            ...
            <ListBox
              ...
              selectedKeys={[
                activeGlobalEnvironment?._id ||
                  activeGlobalBaseEnvironment?._id ||
                  "",
              ]}
                  ...
                  {({ isSelected }) => (
                    <Fragment>
                      ...
                      {isSelected && (
                        <Icon
                          icon="check"
                          className="justify-self-end px-2 text-[--color-success]"
                        />
                      )}
                    </Fragment>
                  )}
            ...
            <div className="relative flex h-full w-full flex-1 flex-col overflow-hidden">
              ...
              <ListBox
                ...
                selectedKeys={[
                  activeEnvironment._id || baseEnvironment._id || "",
                ]}
                disallowEmptySelection
                onSelectionChange={(keys) => {
                  if (keys === "all" || !keys) {

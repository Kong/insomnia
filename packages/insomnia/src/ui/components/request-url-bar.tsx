...
export const RequestUrlBar = forwardRef<RequestUrlBarHandle, Props>(
  ({ handleAutocompleteUrls, uniquenessKey, onPaste }, ref) => {
    ...
    const [showEnvVariableMissingModal, setShowEnvVariableMissingModal] =
      useState(false);
    const [showInputVaultKeyModal, setShowInputVaultKeyModal] = useState(false);
    const [undefinedEnvironmentVariables, setUndefinedEnvironmentVariables] =
      useState("");
    const undefinedEnvironmentVariableList =
      undefinedEnvironmentVariables?.split(",");
    if (searchParams.has("error")) {
      if (
        searchParams.has("envVariableMissing") &&
        searchParams.get("undefinedEnvironmentVariables")
      ) {
        setShowEnvVariableMissingModal(true);
        setUndefinedEnvironmentVariables(
          searchParams.get("undefinedEnvironmentVariables")!,
        );
      } else {
    ...
    const { organizationId, projectId, workspaceId, requestId } =
      useParams() as {
      ...
      async (
        ...
        if (
          ...
          const startListening = async () => {
            ...
            const ancestors = await db.withAncestors<Request | RequestGroup>(
              activeRequest,
            ...
            activeRequest.authentication = getOrInheritAuthentication({
              request: activeRequest,
              requestGroups,
            });
            activeRequest.headers = getOrInheritHeaders({
              request: activeRequest,
              requestGroups,
            });
            const rendered =
              await tryToInterpolateRequestOrShowRenderErrorModal({
                ...
                payload: {
                  ...
                  parameters: activeRequest.parameters.filter(
                    (p) => !p.disabled,
                  ),
                  workspaceCookieJar,
                },
    ...
    useInterval(
      sendOrConnect,
      currentInterval && fetcher.state === "idle" ? currentInterval : null,
    );
    ...
    const buttonText = isRealtimeRequest
      ? "Connect"
      : downloadPath
        ? "Download"
        : "Send";
    ...
    const isEventStreamOpen = useReadyState({
      requestId: activeRequest._id,
      protocol: "curl",
    });
    const isGraphQLSubscriptionOpen = useReadyState({
      requestId: activeRequest._id,
      protocol: "webSocket",
    });
    const isCancellable =
    ...
    return (
      <div className="flex w-full items-stretch justify-between self-stretch">
        ...
        >
          <div>
            These environment variables have been defined, but have not been
            valued with in the currently active environment:
          ...
          {!vaultKey &&
            undefinedEnvironmentVariableList.some((variableName) =>
              ...
              <div className="mt-4">
                <p>
                  These are secret environment variables. However, the required
                  vault key has not been provided yet.
                </p>
                <Button
        ...
        {showInputVaultKeyModal && (
          <InputVaultKeyModal
            onClose={() => setShowInputVaultKeyModal(false)}
          />
        )}
      </div>
    );

...
const addApiKeyToParams = (requestAuth: RequestAuthentication) => {
  const shouldAddAuthParamsToQuery =
    requestAuth.type === "apikey" && requestAuth.addTo === "queryParams";
...
async function getQueryParamsFromAuth(
  ...
  const hasAuthSetOnRequest =
    requestAuth !== null && isAuthEnabled(request.authentication);
  if (hasAuthSetOnRequest) {
    return addApiKeyToParams(requestAuth);
  }

  const ancestors = await db.withAncestors<
    Request | WebSocketRequest | RequestGroup
  >(request, [models.requestGroup.type]);
...
export const RenderedQueryString: FC<Props> = ({ request }) => {
  ...
  useEffect(() => {
    const fn = async () => {
      const enabledParameters = request.parameters.filter(
        ({ disabled }) => !disabled,
      );
      ...
      try {
        ...
        const {
          parameters,
          pathParameters,
          authQueryParams: renderedAuthQueryParams,
        } = result;
        let { url } = result;
        ...
        const qs = buildQueryStringFromParams(mergedParams, false, {
          encodeParams: request.settingEncodeUrl,
        });
  ...
  const className =
    previewString === defaultPreview
      ? "super-duper-faint"
      : "selectable force-wrap";

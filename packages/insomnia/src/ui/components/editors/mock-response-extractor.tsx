...
If you want to create a self-hosted mock server route, you need to upgrade to an enterprise plan.`;
      ...
      mockServerAndRoutes = mockServerAndRoutes.filter(
        ({ useInsomniaCloud }) => !useInsomniaCloud,
      );
...
If you want to create a self-hosted mock server route from a request response in a local project, please create a self-hosted mock server in project panel manually first.`;
  ...
  const maybeMimeType =
    activeResponse && getMimeTypeFromContentType(activeResponse.contentType);
  const mimeType =
    maybeMimeType && isInMockContentTypeList(maybeMimeType)
      ? maybeMimeType
      : "text/plain";
  return (
        ...
        <>
          ...
          <form
            onSubmit={async (e) => {
              ...
              try {
                path = activeResponse
                  ? new URL(activeResponse.url).pathname
                  : "/new-route";
              } catch (e) {
                console.log(e);
              }
              ...
              if (!selectedMockServer) {
                showPrompt({
                  ...
                  onComplete: async (name) => {
                    invariant(
                      activeResponse,
                      "Active response must be defined",
                    );
                    const body = await fs.readFile(activeResponse.bodyPath);
                    // auth mechanism is too sensitive to allow content length checks
                    const headersWithoutContentLength: ResponseHeader[] =
                      activeResponse.headers.filter(
                ...
                showPrompt({
                  ...
                  onComplete: async (name) => {
                    ...
                    const hasRouteInServer = mockServerAndRoutes
                      ...
                      ?.routes.find(
                        (r) =>
                          r.name === name && r.method.toUpperCase() === "GET",
                      );
                    if (hasRouteInServer) {
          ...
          >
            ...
            <div className="form-row">
              <div className="form-control form-control--outlined">
                <label>
                  Choose Mock Route
                  <HelpTooltip position="top" className="space-left">
                    Select from created mock routes to overwrite with this
                    response
                  </HelpTooltip>
                  <select
            ...
            <div className="mt-2 flex">
              ...
              <Button
                ...
                onPress={() => {
                  const mockWorkspaceId = mockServerAndRoutes.find(
                    (s) => s._id === selectedMockServer,
                  )?.parentId;
                  navigate(

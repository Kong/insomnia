...
const setCollectionVar =
  'insomnia.collectionVariables.set("variable_name", "variable_value");';
const unsetEnvVar = 'insomnia.environment.unset("variable_name");';
const unsetGlbVar = 'insomnia.globals.unset("variable_name");';
const unsetCollectionVar =
  'insomnia.collectionVariables.unset("variable_name");';
...
function getRequestScriptSnippets(
  insomniaObject: InsomniaObject,
  path: string,
): Snippet[] {
    ...
    if (
      ...
      for (const item of value) {
        snippets = snippets.concat(
          getRequestScriptSnippets(item, `${path}.${key}`),
        );
      }
    } else {
      snippets = snippets.concat(
        getRequestScriptSnippets(value, `${path}.${key}`),
      );
    }
  }
...
export const RequestScriptEditor: FC<Props> = ({
  ...
  const requestScriptSnippets = getRequestScriptSnippets(
    new InsomniaObject({
      ...
      vault: settings.enableVaultInScripts
        ? new Vault("vault", {}, settings.enableVaultInScripts)
        : undefined,
              ...
              >
                {(section) => {
                  if ("items" in section) {
                    return (
                      <Section>
                        <Header className="py-1 pl-2 text-xs uppercase text-[--hl]">
                          {section.name}
                        </Header>

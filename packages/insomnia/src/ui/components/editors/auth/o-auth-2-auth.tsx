...
const getFields = (
  authentication: Extract<RequestAuthentication, { type: typeof AUTH_OAUTH_2 }>,
) => {
  const clientId = (
    <AuthInputRow label="Client ID" property="clientId" key="clientId" />
  );
  const clientSecret = (
    <AuthInputRow
      label="Client Secret"
  ...
  const usePkce = (
    <AuthToggleRow
      ...
      property="usePkce"
      key="usePkce"
      onTitle="Disable PKCE"
      offTitle="Enable PKCE"
    />
  );
  const pkceMethod = (
    <AuthSelectRow
  ...
  const username = (
    <AuthInputRow label="Username" property="username" key="username" />
  );
  const password = (
    <AuthInputRow label="Password" property="password" key="password" mask />
  );
  const tokenPrefix = (
    <AuthInputRow
      label="Header Prefix"
  ...
  const audience = (
    <AuthInputRow
      ...
      property="audience"
      key="audience"
      help="Indicate what resource server to access"
    />
  );
  const resource = (
    <AuthInputRow
      ...
      property="resource"
      key="resource"
      help="Indicate what resource to access"
    />
  );
  const origin = (
    <AuthInputRow
...
export const OAuth2Auth: FC = () => {
  ...
  const { basic, advanced } = getFieldsForGrantType(
  ...
  return (
    <>
      <AuthTableBody>
        ...
        {basic}
        <AuthAccordion
          accordionKey="OAuth2AdvancedOptions"
...
const renderIdentityTokenExpiry = (
  token?: Pick<OAuth2Token, "identityToken">,
) => {
...
const renderAccessTokenExpiry = (
  token?: Pick<OAuth2Token, "accessToken" | "expiresAt">,
) => {
      ...
      <div className="pad-top text-right">
        ...
        <button
          ...
          onClick={async () => {
            ...
            try {
              const renderedAuthentication = (await handleRender(
                authentication,
              )) as AuthTypeOAuth2;

...
import {
  CodePromptModal,
  type CodePromptModalHandle,
} from "../../modals/code-prompt-modal";
...
export const EnvironmentKVEditor = ({
  ...
  const [kvPairError, setKvPairError] = useState<
  ...
  const secretItemType = [
    { id: EnvironmentKvPairDataType.SECRET, name: "Secret" },
  ];
  // Use private environment to store vault secrets if vault key is available
  const kvPairItemTypes =
    isPrivate && !!vaultKey
      ? commonItemTypes.concat(secretItemType)
      : commonItemTypes;
  ...
  const { dragAndDropHooks } = useDragAndDrop({
    ...
    onReorder(e) {
      ...
      const targetIndex = kvPairs.findIndex(
        (pair) => pair.id === e.target.key.toString(),
      );
      onChange(repositionInArray(moveItems, targetIndex));
    },
    renderDragPreview(items) {
      const pair =
        kvPairs.find((pair) => pair.id === items[0]["text/plain"]) ||
        createNewPair();
    ...
    renderDropIndicator(target) {
      return (
        <DropIndicator
          target={target}
          className="!border-none outline outline-1 outline-[--color-surprise]"
        />
      );
    },
  });
  ...
  const handleItemChange = <K extends keyof EnvironmentKvPairData>(
    ...
    if (changedItemIdx !== -1) {
      ...
      if (
        newValue === EnvironmentKvPairDataType.JSON &&
        changedItem.value.trim() === ""
      ) {
  ...
  const handleItemTypeChange = (
    id: string,
    newType: EnvironmentKvPairDataType,
  ) => {
    ...
    if (targetItem) {
      ...
      if (originType === EnvironmentKvPairDataType.SECRET) {
        const newTypeDisplayText = kvPairItemTypes.find(
          ...
          onDone: async (yes: boolean) => {
            if (yes) {
              ...
              handleItemChange(
                id,
                "value",
                decryptSecretValue(originValue, symmetricKey),
              );
            }
          },
        });
        ...
        handleItemChange(
          id,
          "value",
          encryptSecretValue(originValue, symmetricKey),
        );
        handleItemChange(id, "type", newType);
      } else {
        handleItemChange(id, "type", newType);
  ...
  const renderPairItem = (kvPair: EnvironmentKvPairData) => {
    ...
    const hasItemWithSameNameAfter =
      ...
      kvPairs
        .slice(itemIndex + 1)
        .some((pair) => pair.name.trim() === name.trim() && pair.enabled);
    ...
    return (
      <>
        ...
        <div
          className={`${cellCommonStyle} relative flex h-full w-[30%] flex-grow pl-1`}
        >
          <OneLineEditor
            ...
            onChange={(newName) => {
              ...
              if (error) {
                if (itemError) {
                  setKvPairError(
                    kvPairError.map((p) => (p.id === id ? { id, error } : p)),
                  );
                } else {
          ...
          {hasItemWithSameNameAfter && (
            <Tooltip
              message={`Duplicate name: ${name}. Only the last item with same name will be used.`}
              delay={200}
            >
        ...
        <div className={`${cellCommonStyle} relative w-[50%]`}>
          ...
          {type === EnvironmentKvPairDataType.JSON && (
            ...
            >
              <i
                className="fa fa-pencil-square-o space-right"

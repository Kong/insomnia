...
export function handleToggleEnvironmentType(
  ...
  const kvPairData = isSelected
    ? []
    : getKVPairFromData(environment.data, environment.dataPropertyOrder);
  const foundDisabledItem =
    isSelected && environment.kvPairData?.some((pair) => !pair.enabled);
  const foundDuplicateNameItem =
    ...
    environment.kvPairData?.some((pair, idx) =>
      environment.kvPairData
        ...
        .some(
          (newPair) =>
            pair.name.trim() === newPair.name.trim() && newPair.enabled,
        ),
    );
  if (!isValidJSON && newEnvironmentType === EnvironmentType.KVPAIR) {
    showModal(AlertModal, {
      title: "Error",
      message:
        "Please modify and fix the JSON string error before switch to Table view",
    });
    ...
    showModal(AskModal, {
      ...
      message: (
        <>
          ...
          {foundDuplicateNameItem && (
            <p>Items with same name will be lost except the last one.</p>
          )}

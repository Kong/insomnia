...
describe("ensureKeyIsValid()", () => {
  it.each(["$", "$a", "$ab"])(
    '"%s" should be invalid when key begins with $',
    (key) => {
      expect(ensureKeyIsValid(key, false)).toBe(
        `"${key}" cannot begin with '$' or contain a '.'`,
      );
    },
  );

  it.each([".", "a.", ".a", "a.b"])(
    '"%s" should be invalid when key contains .',
    (key) => {
      expect(ensureKeyIsValid(key, false)).toBe(
        `"${key}" cannot begin with '$' or contain a '.'`,
      );
  ...
  it.each(["$a.b", "$."])(
    '"%s" should be invalid when key starts with $ and contains .',
    (key) => {
      expect(ensureKeyIsValid(key, false)).toBe(
        `"${key}" cannot begin with '$' or contain a '.'`,
      );
...
describe("checkNestedKeys()", () => {
  ...
  it("should check nested property and error", () => {
    ...
    expect(err).toBe(
      "\"$path-wi.th-hyphens\" cannot begin with '$' or contain a '.'",
    );
  });

      ...
      {({ isSelected, isHovered }) => (
        <Tooltip delay={1000} message={`${tab.projectName} / ${tab.workspaceName}`} className="h-full">
          ...
          >
            ...
            <span
              className={`absolute bottom-[0px] left-0 right-0 block h-px bg-(--color-bg) ${isSelected ? 'opacity-100' : 'opacity-0'}`}
            />
            <span
              className={`absolute bottom-[0px] left-0 right-0 block h-px bg-(--hl-sm) ${!isSelected ? 'opacity-100' : 'opacity-0'}`}
            />
          </div>

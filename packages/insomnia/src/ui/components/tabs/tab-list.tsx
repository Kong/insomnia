...
export const TAB_ROUTER_PATH: Record<TabType, string> = {
  ...
  testSuite:
    "/organization/:organizationId/project/:projectId/workspace/:workspaceId/test/test-suite/*",
};
...
export const OrganizationTabList = ({
  ...
  const handleUpdate = useCallback(
    async (
      doc: models.BaseModel,
      patches: Partial<models.BaseModel>[] = [],
    ) => {
      ...
      if (
        patchObj.parentId &&
        !patchObj.metaSortKey &&
        (patchObj.parentId as string).startsWith("wrk_")
      ) {
        ...
        if (workspace) {
          if (isRequest(doc)) {
            ...
            const folderEntities = await database.withDescendants(
              doc,
              models.request.type,
  ...
  return (
    <div
      className="box-content flex bg-[--color-bg]"
    ...
    >
      ...
      >
        <Icon
          icon="chevron-left"
          className={`w-[30px] ${isOverFlow ? "block" : "hidden"}`}
        />
      </Button>
      <div

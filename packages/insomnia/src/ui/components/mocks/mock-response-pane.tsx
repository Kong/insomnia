...
export const MockResponsePane = () => {
  ...
  useEffect(() => {
    const fn = async () => {
      if (activeResponse) {
        const timeline = await models.response.getTimeline(
          activeResponse,
          true,
        );
      ...
      <PlaceholderResponsePane>
        {
          <ResponseTimer
            handleCancel={() =>
              activeResponse && cancelRequestById(activeResponse.parentId)
            }
    ...
    <Pane type="response">
      {!activeResponse ? null : (
        <PaneHeader className="row-spaced">
          <div
            aria-atomic="true"
            aria-live="polite"
            className="no-wrap scrollable scrollable--no-bars pad-left"
          >
            <StatusTag
      ...
      >
        ...
        <TabPanel
          ...
          id="history"
        >
          <HistoryViewWrapperComponentFactory
...
const HistoryViewWrapperComponentFactory = ({
  ...
  const fetchLogs = useCallback(async () => {
    ...
    const mockbinUrl = mockServer.useInsomniaCloud
      ? getMockServiceURL()
      : mockServer.url;
    try {
      ...
      console.log("[mock] Error: fetching logs from remote", {
        mockbinUrl,
        res,
      });
    } catch (e) {
    ...
    <div className="grid h-full w-full grid-rows-[repeat(auto-fit,minmax(0,1fr))]">
      <div className="box-border w-full flex-1 overflow-hidden overflow-y-scroll">
        <div className="grid grid-cols-[repeat(5,auto)] divide-y divide-solid divide-[--hl-sm]">
          <div className="bg-[--hl-sm] p-2 text-left text-xs font-semibold uppercase focus:outline-none">
            Method
          </div>
          <div className="bg-[--hl-sm] p-2 text-left text-xs font-semibold uppercase focus:outline-none">
            Size
      ...
      <DropdownSection aria-label="Action Section" title="Action">
        <DropdownItem aria-label="Copy raw response">
          <ItemContent
            ...
            onClick={async () => {
              const bodyBuffer =
                await models.response.getBodyBuffer(activeResponse);
              bodyBuffer &&
            ...
            <ItemContent
              ...
              onClick={async () => {
                ...
                fs.promises.writeFile(
                  filePath,
                  jsonPrettify(bodyBuffer.toString("utf8")),
                );
              }}
            />
          )}
        ...
        <DropdownItem aria-label="Export as HAR">
          <ItemContent
            ...
            label="Export as HAR"
            onClick={async () => {
              const activeRequest = await models.request.getById(
                activeResponse.parentId,
              );
              const { canceled, filePath } = await window.dialog.showSaveDialog(
                {
                  title: "Save Full Response",
              ...
              const data = await exportHarCurrentRequest(
                activeRequest,
                activeResponse,
              );

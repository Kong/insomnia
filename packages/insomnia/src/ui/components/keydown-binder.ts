...
const keyCombinationToTinyKeyString = ({
  ctrl,
  alt,
  shift,
  meta,
  ...
  Object.entries(keyboardKeys).find(
    ([, { keyCode: kc }]) => kc === keyCode,
  )?.[1].code;

export function useKeyboardShortcuts(
  getTarget: () => HTMLElement,
  listeners: { [key in KeyboardShortcut]?: (event: KeyboardEvent) => any },
) {
  ...
  useEffect(() => {
    ...
    const keyboardShortcuts = Object.entries(listeners) as [
      KeyboardShortcut,
    ...
    const keyBindingMap: KeyBindingMap = keyboardShortcuts
      .map(([keyboardShortcut, action]) =>
        getPlatformKeyCombinations(hotKeyRegistry[keyboardShortcut]).map(
          (combo) => ({
      ...
      .flat()
      .reduce(
        (acc, { tinyKeyString, action }) => ({
          ...acc,
          [tinyKeyString]: action,
        }),
        {},
      );
...
export function useDocBodyKeyboardShortcuts(listeners: {
  [key in KeyboardShortcut]?: (event: KeyboardEvent) => any;
}) {

...
export const SyncDropdown: FC<Props> = () => {
  ...
  const error =
    ...
    pullFetcher.data?.error ||
    pushFetcher.data?.error ||
    rollbackFetcher.data?.error;
  ...
  const syncError =
    syncDataLoaderFetcher.data && "error" in syncDataLoaderFetcher.data
      ? syncDataLoaderFetcher.data.error
      : null;
  return (
    <Fragment>
      <MenuTrigger>
        <div className="flex h-[--line-height-sm] w-full items-center text-sm text-[--color-font] ring-1 ring-transparent transition-all hover:bg-[--hl-xs] focus:ring-inset focus:ring-[--hl-md] aria-pressed:bg-[--hl-sm]">
          ...
          >
            ...
            />
            <span
              className={`truncate ${syncError ? "text-[--color-warning]" : ""}`}
            >
                    ...
                    {(item) => (
                      ...
                      >
                        <Icon
                          icon={item.icon}
                          className={
                            item.isActive ? "text-[--color-success]" : ""
                          }
                        />
      ...
      {isSyncBranchesModalOpen && (
        <SyncBranchesModal
          ...
          remoteBranches={remoteBranches.filter(
            (remoteBranch) => !localBranches.includes(remoteBranch),
          )}
          onClose={() => {

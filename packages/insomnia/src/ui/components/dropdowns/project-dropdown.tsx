...
export const ProjectDropdown: FC<Props> = ({
  ...
  const isRemoteProjectInconsistent =
    isRemoteProject(project) && storage === ORG_STORAGE_RULE.LOCAL_ONLY;
  const isLocalProjectInconsistent =
    !isRemoteProject(project) && storage === ORG_STORAGE_RULE.CLOUD_ONLY;
  ...
  useEffect(() => {
    if (
      deleteProjectFetcher.data &&
      deleteProjectFetcher.data.error &&
      deleteProjectFetcher.state === "idle"
    ) {
      showAlert({
  ...
  useEffect(() => {
    if (
      updateProjectFetcher.data &&
      updateProjectFetcher.data.error &&

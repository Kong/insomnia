...
export const ContentTypeDropdown: FC = () => {
  ...
  const handleChangeMimeType = async (mimeType: string | null) => {
    ...
    const isFormUrlEncoded =
      hasMimeType && body.mimeType === CONTENT_TYPE_FORM_URLENCODED;
      ...
      showAlert({
        ...
        onConfirm: async () => {
          ...
          window.main.trackSegmentEvent({
            event: SegmentEvent.requestBodyTypeSelect,
  ...
  const numBodyParams = hasParams
    ? body.params?.filter(({ disabled }) => !disabled).length
    : 0;
...
export const updateMimeType = (
  ...
  const withoutContentType = request.headers.filter(
  ...
  if (
    ...
    const params =
      request.body.params || deconstructQueryStringToParams(request.body.text);
    return {

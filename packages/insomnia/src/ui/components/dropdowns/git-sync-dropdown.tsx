...
export const GitSyncDropdown: FC<Props> = ({
  gitRepository,
            ...
            try {
              ...
              const errorMessage =
                err instanceof Error
                  ? err.message
                  : "An error occurred while pulling";
              showAlert({
                title: "Pull Failed",
                message: (
                  <>
                    {errorMessage}
                    <ConfigLink
                      {...{ gitRepository, errors: [errorMessage] }}
                    />
                  </>
                ),
                bodyClassName: "whitespace-break-spaces",
  ...
  const commitToolTipMsg = status?.localChanges
    ? "Local changes made"
  ...
  return (
    <>
        ...
        <div className="p-[--padding-sm]">
          <div className="flex flex-wrap items-center justify-between gap-2 rounded border border-solid border-[--hl-md] bg-[rgba(var(--color-warning-rgb),var(--tw-bg-opacity))] bg-opacity-50 p-[--padding-xs] text-[--color-font-warning]">
            <p className="text-sm">
              ...
              You are using the legacy Git integration in this project, learn
              more about converting to the new Git Sync capability.{" "}
              <Button
                className="underline"
                onPress={() =>
                  window.main.openInBrowser(
                    "https://docs.insomnia.rest/insomnia/git-sync",
                  )
                }
              >
                Migration Guide
      ...
      <MenuTrigger>
        <div className="flex h-[--line-height-sm] w-full items-center text-sm text-[--color-font] ring-1 ring-transparent transition-all hover:bg-[--hl-xs] focus:ring-inset focus:ring-[--hl-md] aria-pressed:bg-[--hl-sm]">
          ...
          >
            ...
            <span className="truncate">
              {isSynced ? currentBranch : "Not synced"}
            </span>
          </Button>
          <TooltipTrigger>
            <Button
              className={`h-full px-[--padding-md] ${status?.localChanges ? "text-[--color-warning]" : ""}`}
            >
              <Icon
        ...
        <Popover
          className="min-w-max max-w-lg overflow-hidden"
          placement="top end"
          offset={8}
        >
          <Menu
            ...
            disabledKeys={allSyncMenuActionList
              .filter((item) => item?.isDisabled)
              .map((item) => item.id)}
            onAction={(key) => {
              const item = allSyncMenuActionList.find(
                (item) => item.id === key,
              );
              item?.action();
            }}
              ...
              <Collection items={branchesActionList}>
                {(item) => (
                  ...
                  >
                    <Icon
                      icon={item.icon}
                      className={item.isActive ? "text-[--color-success]" : ""}
                    />
      ...
      {isGitLogModalOpen && gitRepository && (
        <GitLogModal onClose={() => setIsGitLogModalOpen(false)} />
      )}

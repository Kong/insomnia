    ...
    <>
      {showDeprecatedWarning && (
        <div className="p-(--padding-sm)">
          <div className="flex flex-wrap items-center justify-between gap-2 rounded-sm border border-solid border-(--hl-md) bg-[rgba(var(--color-warning-rgb),var(--tw-bg-opacity))] bg-opacity-50 p-(--padding-xs) text-(--color-font-warning)">
            <p className="text-sm">
              ...
              <Collection items={switchToInsomniaSyncList}>
                {item => (
                  <MenuItem
                    ...
                    className={
                      'text-md group flex h-(--line-height-xs) w-full items-center gap-2 whitespace-nowrap bg-transparent px-(--padding-md) text-(--color-font) transition-colors focus:outline-hidden disabled:cursor-not-allowed aria-disabled:cursor-not-allowed aria-disabled:opacity-30 aria-selected:font-bold'
                    }
                    aria-label={item.label}
                  >
                    <div className="group-pressed:opacity-80 flex w-full items-center justify-center gap-2 rounded-xs border border-solid border-(--hl-md) bg-[rgba(var(--color-surprise-rgb),var(--tw-bg-opacity))] bg-opacity-100 px-4 py-1 text-sm font-semibold text-(--color-font-surprise) ring-1 ring-transparent transition-all hover:bg-opacity-80 focus:ring-inset focus:ring-(--hl-md) group-hover:bg-opacity-80 group-hover:ring-inset group-focus:bg-opacity-80 group-focus:ring-inset aria-pressed:opacity-80">
                      <Icon icon={item.icon} />
                      <div>{item.label}</div>
                    </div>
                  </MenuItem>
            ...
            <MenuSection className="border-b border-solid border-(--hl-sm) pb-2 empty:border-none empty:pb-0">
              <Collection items={gitSyncActions}>
                {item => (
                  <MenuItem
                    className={
                      'text-md flex h-(--line-height-xs) w-full items-center gap-2 whitespace-nowrap bg-transparent px-(--padding-md) text-(--color-font) transition-colors hover:bg-(--hl-sm) focus:bg-(--hl-xs) focus:outline-hidden disabled:cursor-not-allowed aria-disabled:cursor-not-allowed aria-disabled:opacity-30 aria-selected:font-bold'
                    }
                    aria-label={item.label}
                  >
                    <Icon icon={item.icon} />

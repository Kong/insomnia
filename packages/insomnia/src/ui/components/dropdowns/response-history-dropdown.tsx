...
export const ResponseHistoryDropdown = ({
  ...
  const handleSetActiveResponse = useCallback(
    async (requestId: string, activeResponse: Response | WebSocketResponse) => {
      ...
      await patchRequestMeta(requestId, {
        activeResponseId: activeResponse._id,
      });
    },
    [patchRequestMeta],
  );
  ...
  const renderResponseRow = (response: Response | WebSocketResponse) => {
    ...
    const requestVersion = requestVersions.find(
      ({ _id }) => _id === response.requestVersionId,
    );
    ...
    <Dropdown
      ...
      triggerButton={
        <Button className="btn btn--super-compact tall">
          {activeResponse && (
            <TimeFromNow timestamp={activeResponse.created} titleCase />
          )}

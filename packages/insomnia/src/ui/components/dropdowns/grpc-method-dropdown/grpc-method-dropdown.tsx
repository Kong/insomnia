...
const PROTO_PATH_REGEX =
  /^\/(?:(?<package>[\w.]+)\.)?(?<service>\w+)\/(?<method>\w+)$/;
...
export const groupGrpcMethodsByPackage = (
  methodInfoList: GrpcMethodInfo[],
): Record<string, GrpcMethodInfo[]> => {
  return groupBy(
    methodInfoList,
    ({ fullPath }) =>
      PROTO_PATH_REGEX.exec(fullPath)?.groups?.package || NO_PACKAGE_KEY,
  );
};

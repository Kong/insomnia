...
export const DirectoryProjectBar: FC<Props> = ({ directoryPath }) => {
  return (
    <div className="flex h-(--line-height-sm) w-full items-center justify-between gap-2 px-(--padding-md) text-sm text-(--color-font) ring-1 ring-transparent transition-all">
      ...
      <span className="flex-1 truncate" title={directoryPath}>
        {directoryPath}
      </span>
      <TooltipTrigger delay={0}>
        ...
        >
          Project data is backed by local Insomnia files in this folder.
        </Tooltip>
      </TooltipTrigger>
    </div>
  );

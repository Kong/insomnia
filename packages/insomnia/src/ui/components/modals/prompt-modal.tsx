...
export const PromptModal = forwardRef<PromptModalHandle, ModalProps>(
  (_, ref) => {
      ...
      if (inputRef.current) {
        const result =
          inputRef.current.type === "checkbox"
            ? inputRef.current.checked.toString()
    ...
    if (Array.isArray(hints)) {
      sanitizedHints = hints.slice(0, 15).map((hint) => (
        <div
          key={hint}
          className="btn btn--outlined btn--super-super-compact margin-right-sm margin-top-sm inline-block"
        >
          <button
            className="tall"
            onClick={() => {
              if (hint) {
                state.onComplete?.(
                  state.upperCase ? hint?.toUpperCase() : hint,
                );
              }
              modalRef.current?.hide();
            }}

...
function isSwitchingStorageType(
  project: Project,
  ...
  if (
    storageType === "local" &&
    (isRemoteProject(project) || isGitProject(project))
  ) {
    return true;
  }
...
export const ProjectModal = ({
  ...
  const [projectData, setProjectData] = useState<{
    ...
    password:
      gitRepository?.credentials && "password" in gitRepository.credentials
        ? gitRepository?.credentials?.password
        : "",
    token:
      gitRepository?.credentials && "token" in gitRepository.credentials
        ? gitRepository?.credentials?.token
        : "",
    oauth2format:
  ...
  const [activeView, setActiveView] = useState<
    "project" | "git-clone" | "git-results" | "switch-storage-type"
  >("project");
  const [selectedTab, setTab] = useState<OauthProviderName>("github");

  const showStorageRestrictionMessage =
    storageRule !== ORG_STORAGE_RULE.CLOUD_PLUS_LOCAL;
  ...
  const insomniaFiles =
    ...
    "files" in initCloneGitRepositoryFetcher.data
  ...
  const onUpsertProject = () => {
    if (
      ...
      isSwitchingStorageType(project, projectData.storageType)
    ) {
    ...
    upsertProjectFetcher.submit(projectData, {
      action,
      method: "POST",
    ...
    >
      <Modal className="flex max-h-[90dvh] min-h-[420px] w-full max-w-3xl flex-col overflow-hidden rounded-md border border-solid border-[--hl-sm] bg-[--color-bg] text-[--color-font]">
        <Dialog
          ...
          className="grid flex-1 gap-4 outline-none [grid-template-rows:min-content_1fr_min-content]"
        >
          {({ close }) => (
            <>
              ...
              {upsertProjectFetcher.data?.error && (
                <div className="px-10">
                  <div className="flex items-center gap-2 rounded-sm bg-[rgba(var(--color-danger-rgb),0.5)] px-2 py-1 text-sm text-[--color-font-danger]">
                    ...
                    <span>Error: {upsertProjectFetcher.data?.error}</span>
                  </div>
                </div>
              )}
                  ...
                  <div className="flex flex-col justify-start gap-2 overflow-y-auto px-10">
                    ...
                    <RadioGroup
                      ...
                      onChange={(value) =>
                        setProjectData({
                          ...projectData,
                          storageType: value as "local" | "remote" | "git",
                        })
                      }
                    ...
                    >
                      ...
                      <div className="flex gap-2">
                        <Radio
                          isDisabled={
                            storageRule === ORG_STORAGE_RULE.CLOUD_ONLY
                          }
                          value="local"
                        ...
                        >
                          ...
                          <p className="pt-2">
                            Stored locally only, with no cloud. Ideal when
                            collaboration is not needed.
                          </p>
                        </Radio>

                        <Radio
                          isDisabled={
                            storageRule === ORG_STORAGE_RULE.LOCAL_ONLY
                          }
                          value="remote"
                          className="flex-1 rounded border border-solid border-[--hl-md] p-4 transition-colors hover:bg-[--hl-xs] focus:bg-[--hl-sm] focus:outline-none data-[selected]:border-[--color-surprise] data-[disabled]:opacity-25 data-[selected]:ring-2 data-[selected]:ring-[--color-surprise]"
                        >
                          ...
                          <p className="pt-2">
                            Encrypted and synced securely to the cloud, ideal
                            for out of the box collaboration.
                          </p>
                        </Radio>
                        <Radio
                        ...
                        >
                          ...
                          <p className="pt-2">
                            Stored locally and synced to a Git repository. Ideal
                    ...
                    {showStorageRestrictionMessage && (
                      <div className="flex items-center gap-2 rounded-sm bg-[rgba(var(--color-warning-rgb),0.5)] px-2 py-1 text-sm text-[--color-font-warning]">
                        ...
                        <span>
                          ...
                          created and stored {storageRule.split("_").join(" ")}.
                        </span>
                      </div>
                    )}
                  </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {projectData.storageType === "git" && !gitRepository && (
                        <Button
                          ...
                          className="w-[10ch] rounded-sm border border-solid border-[--hl-md] bg-[--color-surprise] px-3 py-2 text-center text-[--color-font-surprise] transition-colors hover:bg-opacity-90 hover:no-underline"
                        >
                          Next
                        </Button>
                      )}
                ...
                <>
                  <ErrorBoundary>
                    <Tabs
                      ...
                      className="flex h-full w-full flex-1 flex-col px-10"
                    >
                      <TabList
                ...
                <>
                  {initCloneGitRepositoryFetcher.state !== "idle" && (
                    <div className="flex w-full flex-col items-center justify-center gap-2 p-4">
                      <InsomniaLogo loading className="h-12 w-12" />
                      Loading Insomnia files from repository
                    </div>
                  )}
                  {insomniaFiles.length === 0 &&
                    initCloneGitRepositoryFetcher.state === "idle" && (
                      <div className="flex w-full flex-col items-center justify-center gap-2 p-4 px-10">
                        <div className="flex w-full flex-col items-center gap-2 rounded-sm bg-[--hl-xs] p-4 text-sm text-[--color-font-success]">
                          ...
                          <p className="p-2 text-center font-bold text-[--color-font]">
                            We didn't find any Insomnia files in this
                            repository.
                          </p>
                          <p className="p-2 text-center font-bold text-[--color-font]">
                          ...
                          <p className="p-2 text-center text-[--color-font]">
                            Add your collections, documents, environments and
                            mock servers, and share them using Git.
                          </p>
                        </div>
                      </div>
                    )}
                  {insomniaFiles.length > 0 && (
                    <div className="flex flex-col gap-2 px-10">
                      <Heading className="text-base">
                        We found {insomniaFiles.length} Insomnia files in your
                        repository
                      </Heading>
                              ...
                              <Row className="group transition-colors focus-within:bg-[--hl-xxs] focus:outline-none">
                                <Cell className="whitespace-nowrap border-b border-solid border-[--hl-sm] text-sm font-medium focus:outline-none group-last-of-type:border-none">
                                  <div className="flex items-center gap-2 px-2 py-2">
                                    ...
                                    >
                                      <Icon
                                        icon={scopeToIconMap[file.scope]}
                                        className="w-4"
                                      />
                                    </span>
                                ...
                                <Cell className="whitespace-nowrap border-b border-solid border-[--hl-sm] text-sm font-medium focus:outline-none group-last-of-type:border-none">
                                  <span className="flex items-center gap-1 italic text-[--hl]">
                                    ...
                                    <span className="truncate px-2 text-[--hl]">
                                      {file.path}
                                    </span>
                                  </span>
                                </Cell>
                  ...
                  <div className="flex flex-col justify-start gap-2 overflow-y-auto px-10">
                      ...
                      <div className="flex flex-col gap-4 text-[--color-font]">
                        <div className="flex flex-col gap-4">
                          ...
                          <ul className="flex flex-col gap-2 text-left">
                            <li>
                              <i className="fa fa-check text-emerald-600" /> The
                              project will be 100% stored locally.
                            </li>
                            <li>
                              <i className="fa fa-check text-emerald-600" />{" "}
                              Your collaborators can synchronize files using
                              Git.
                            </li>
                            <li>
                              <i className="fa fa-check text-emerald-600" /> The
                              project will be stored locally also for every
                              existing collaborator.
                            </li>
                          </ul>
                          <p>
                            You can synchronize a local project back to the
                            cloud if you decide to do so.
                          </p>
                          {project && isRemoteProject(project) && (
                            <p className="flex items-center gap-2">
                              ...
                              />
                              Remember to pull your latest project updates
                        ...
                        <div className="flex flex-col gap-4">
                          ...
                          {project && isGitProject(project) && (
                            <ul className="flex flex-col gap-2 text-left">
                              <li>
                                <i className="fa fa-check text-emerald-600" />{" "}
                                The project will be 100% stored locally.
                              </li>
                              <li>
                                <i className="fa fa-check text-emerald-600" />{" "}
                                You will not be able to synchronize this project
                                using Git anymore.
                              </li>
                              <li>
                                <i className="fa fa-check text-emerald-600" />{" "}
                                This action will not delete your remote
                                repository.
                              </li>
                            </ul>
                          )}
                          {project && isRemoteProject(project) && (
                            <>
                              <ul className="flex flex-col gap-2 text-left">
                                ...
                                <li>
                                  <i className="fa fa-check text-emerald-600" />{" "}
                                  Your collaborators will not be able to push
                                  and pull files anymore.
                                </li>
                                <li>
                                  <i className="fa fa-check text-emerald-600" />{" "}
                                  The project will become local also for every
                                  existing collaborator.
                                </li>
                              </ul>
                              <p>
                                You can still use Git Sync for local projects
                                without using the cloud, and you can synchronize
                                a local project back to the cloud if you decide
                                to do so.
                              </p>
                            </>
                          )}
                          <p className="flex items-center gap-2">
                            <Icon
                              ...
                              className="text-[--color-warning]"
                            />
                            Remember to pull your latest project updates before
                            this operation
                          </p>
                        </div>
                      </div>
                    ...
                    {projectData.storageType === "remote" && (
                      <div className="flex flex-col gap-4 text-[--color-font]">
                        <div className="flex flex-col gap-4">
                          <p>
                            We will be synchronizing your local project to
                            Insomnia's Cloud in a secure encrypted format which
                          ...
                          <ul className="flex flex-col gap-2 text-left">
                            ...
                            <li>
                              <i className="fa fa-check text-emerald-600" /> You
                              can now collaborate with any amount of users and
                              use cloud features.
                            </li>
                            <li>
                              <i className="fa fa-check text-emerald-600" />{" "}
                              Your project will be always available on any

...
const Avatar = ({ src }: { src: string }) => {
  ...
  return imageSrc ? (
    <img src={imageSrc} className="h-8 w-8 rounded-md" />
  ) : (
...
const GitHubRepositoryForm = ({
  ...
  return (
    ...
    >
      <div className="mb-2 flex items-center justify-between rounded-md border border-solid border-[--hl-sm] p-2">
        <div className="flex items-center gap-2">
          <Avatar src={credentials.author.avatarUrl ?? ""} />
          <div className="flex flex-col">
            <span className="font-bold">{credentials.author.name}</span>
            <span>{credentials.author.email || "Signed in"}</span>
          </div>
        </div>
        <Button
          type="button"
            ...
            showAlert({
              ...
              onConfirm: () => {
                signOutFetcher.submit(
                  {},
                  {
                    action: "/git-credentials/github/sign-out",
                    method: "POST",
                  },
                );
              },
      ...
      <Button
        onPress={() => {
          ...
          initSignInFetcher.submit(
            {},
            { action: "/git-credentials/github/init-sign-in", method: "POST" },
          );
        }}
      >
      ...
      {isAuthenticating && (
        ...
        >
          <label className="form-control form-control--outlined">
            <div>
              If you aren't redirected to the app you can manually paste the
              authentication url here:
            </div>
            <div className="form-row">
              <input name="link" />
              <Button
                className="bold rounded bg-violet-400 p-2"
                type="submit"
                name="add-token"
              >
                Authenticate

...
type GitHubRepository = Awaited<
  ReturnType<typeof window.main.git.getGitHubRepositories>
>["repos"][number];
  ...
  useEffect(() => {
    ...
    if (!selectedRepository && token && isGitHubAppUserToken(token)) {
      (async function getRepository() {
        const { repo, errors, notFound } =
          await window.main.git.getGitHubRepository({ uri });
    ...
    <>
              ...
              <ComboBox
                ...
                onSelectionChange={(key) =>
                  setSelectedRepository(
                    repositories.find((r) => r.clone_url === key) || null,
                  )
                }
              >
                <div className="group my-2 flex items-center gap-2 rounded-sm border border-solid border-[--hl-sm] bg-[--color-bg] text-[--color-font] transition-colors focus:outline-none focus:ring-1 focus:ring-[--hl-md]">
                  <Input
                ...
                <Popover
                  className="grid min-w-max select-none grid-flow-col divide-x divide-solid divide-[--hl-md] overflow-y-auto rounded-md border border-solid border-[--hl-sm] bg-[--color-bg] text-sm shadow-lg focus:outline-none"
                ...
                <input
                  type="hidden"
                  name="uri"
                  value={selectedRepository?.clone_url || uri || ""}
                />
              </ComboBox>
              <Button
                type="button"
      ...
      {cannotFindRepository && (
        <div className="text-sm text-red-500">
          <Icon icon="warning" /> Repository information could not be retrieved.
          Please <code>Reset</code> and select a different repository.
        </div>
      )}
      {selectedRepository !== null && !selectedRepository.permissions.push && (
        <div className="mt-2 text-sm text-orange-500">
          <Icon icon="warning" /> You do not have write access to this
          repository
        </div>
      )}
    </>

...
export const WorkspaceSettingsModal = ({
  ...
  const [orgStorageRule, setOrgStorageRule] = useState<ORG_STORAGE_RULE>(
    ORG_STORAGE_RULE.CLOUD_PLUS_LOCAL,
  );
  const [description, setDescription] = useState<string>(workspace.description);
  useEffect(() => {
    fetchAndCacheOrganizationStorageRule(organizationId as string).then(
      setOrgStorageRule,
    );
  }, [organizationId]);
  ...
  useEffect(() => {
    if (
      ...
      gitRepoTreeFetcher.load(
        `/organization/${organizationId}/project/${project._id}/git/repository-tree`,
      );
    }
  ...
  const isSelfHostedDisabled =
    !isEnterprise || orgStorageRule === ORG_STORAGE_RULE.CLOUD_ONLY;
  const isCloudProjectDisabled =
    isLocalProject || orgStorageRule === ORG_STORAGE_RULE.LOCAL_ONLY;
  ...
  useEffect(() => {
    if (
      workspaceFetcher.state === "idle" &&
      workspaceFetcher.data &&
      workspaceFetcher.data.success
    ) {
      onClose();
    }
  ...
  const selectedFolderChildren =
    gitRepoTreeFetcher.data?.folderList[selectedFolder] || [];
                      ...
                      <Label className="group relative flex flex-col gap-2 overflow-hidden">
                        ...
                        <div className="grid w-full overflow-hidden rounded-sm border border-solid border-[--hl-sm] bg-[--color-bg] py-1 pl-2 pr-7 text-[--color-font] transition-colors [grid-template-areas:'input_extension'] [grid-template-columns:min-content_auto] focus:outline-none focus:ring-1 focus:ring-[--hl-md]">
                          <Input
                            placeholder={
                              workspace.name
                                ? safeToUseInsomniaFileName(workspace.name)
                                : "name"
                            }
                          ...
                          <span className="-z-10 w-min truncate opacity-0 [grid-area:input]">
                            {safeToUseInsomniaFileName(
                              fileName || workspace.name || "name",
                            )}
                          </span>
                          <span className="text-[--hl] [grid-area:extension]">
                            .yaml
                          </span>
                        </div>
                      </Label>
                ...
                {Boolean(isMockServer(workspace) && mockServer) && (
                  <>
                    <RadioGroup
                      name="mockServerType"
                      defaultValue={
                        mockServer?.useInsomniaCloud ? "cloud" : "self-hosted"
                      }
                      validate={(value) => {
                    ...
                    >
                      ...
                      <div className="flex gap-2">
                        ...
                        >
                          ...
                          <p className="pt-2">
                            Runs locally or on your infrastructure, ideal for
                            private usage and lower latency.
                          </p>
                        </Radio>
                      </div>
                      <FieldError className="text-xs text-red-500" />
                    ...
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span>
                        ...
                        <Link
                          href="https://docs.insomnia.rest/insomnia/api-mocking"
                          className="underline"
                        >
                          click here
                        </Link>

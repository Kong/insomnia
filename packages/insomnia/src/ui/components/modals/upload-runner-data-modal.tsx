...
export const UploadDataModal = ({ onUploadFile, onClose, userUploadData }: UploadDataModalProps) => {
  ...
  const handleFileSelect = async (fileList: FileList | null) => {
    ...
    try {
      const { content, encoding } = await window.main.secureReadFile({ path: filePath, overrideDataFolders: [filePath] });
  ...
  const handleEncodingChange = async (newEncoding: string) => {
    ...
    if (file) {
      ...
      const fileType = file.type;
      try {
        const { content } = await window.main.secureReadFile({ path: filePath, encoding: newEncoding, overrideDataFolders: [filePath]  });
        parseFileContent(content, fileType);
      } catch (error) {
        setInvalidFileReason(`Failed to read file ${error?.message}`);
      }

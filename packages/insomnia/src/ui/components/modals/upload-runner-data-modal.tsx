...
export const UploadDataModal = ({
          ...
          if (Array.isArray(jsonDataContent)) {
            ...
            if (headers.length > 0 && data.length > 0) {
              ...
              setInvalidFileReason(
                "Invalid JSON file uploaded, JSON file do not contain any key-value pair.",
              );
            }
          } else {
            setInvalidFileReason(
  ...
  const handleFileSelect = async (fileList: FileList | null) => {
    ...
    try {
      const { content, encoding } = await window.main.readFile({
        path: filePath,
      });
  ...
  const handleEncodingChange = async (newEncoding: string) => {
    ...
    if (file) {
      ...
      const fileType = file.type;
      try {
        const { content } = await window.main.readFile({
          path: filePath,
            ...
            <div className="flex flex-1 flex-col gap-4 overflow-hidden">
              ...
              {uploadData.length > 0 && (
                <div className="flex-1 overflow-auto py-2">
                  <Heading className="margin-bottom-sm text-xl">
                    Data Preview
                  </Heading>
                  <Table

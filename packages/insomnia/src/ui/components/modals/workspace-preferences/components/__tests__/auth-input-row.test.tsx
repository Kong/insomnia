...
describe('<AuthInputRow />', () => {
  ...
  it('should mask and toggle the input', async () => {
    ...
    const { store } = await createMockStoreWithWorkspace();
  ...
  it('should update the authentication property on typing', async () => {
    ...
    expect(workspace?.authentication.inputProperty).toEqual('inputValue');
  });

  it('should pre-fill existing authentication property value', async () => {
    // Arrange
    const { store } = await createMockStoreWithWorkspace({ workspacePatch: { authentication: { inputProperty: 'existing' } } });

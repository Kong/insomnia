...
export const GenerateCodeModal = forwardRef<GenerateCodeModalHandle, Props>(
  (props, ref) => {
    ...
    const generateCode = useCallback(
      async (
        ...
        const targetOrFallback =
          target ||
          (targets.find((t) => t.key === "shell") as HTTPSnippetTarget);
        const clientOrFallback =
          client ||
          (targetOrFallback.clients.find(
            (t) => t.key === "curl",
          ) as HTTPSnippetClient);
        ...
        window.localStorage.setItem(
          ...
          JSON.stringify(clientOrFallback),
        );
        window.localStorage.setItem(
          "insomnia::generateCode::target",
          JSON.stringify(targetOrFallback),
        );

        // Some clients need a content-length for the request to succeed
        const addContentLength = Boolean(
          (TO_ADD_CONTENT_LENGTH[targetOrFallback.key] || []).find(
            (c) => c === clientOrFallback.key,
          ),
        );
        const har = await exportHarWithRequest(
          request,
          props.environmentId,
          addContentLength,
        );
        if (har) {
          ...
          const cmd =
            snippet.convert(targetOrFallback.key, clientOrFallback.key) || "";
          setSnippet(cmd);
        }
      },
      [props.environmentId],
              ...
              {targets.map((target) => (
                <DropdownItem key={target.key} aria-label={target.title}>
                  <ItemContent
                    label={target.title}
                    onClick={() => {
                      const client = target.clients.find(
                        (c) => c.key === target.default,
                      );
                      if (request && client) {
              ...
              {clients.map((client) => (
                <DropdownItem key={client.key} aria-label={client.title}>
                  <ItemContent
                    label={client.title}
                    onClick={() =>
                      request && generateCode(request, state.target, client)
                    }
                  />
                </DropdownItem>
              ))}

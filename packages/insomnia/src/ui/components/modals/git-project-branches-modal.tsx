...
const LocalBranchItem = ({
  ...
  useEffect(() => {
    if (
      ...
      const error: string =
        mergeBranchFetcher.data.error ||
        "An unexpected error occurred while merging the branches.";
      showAlert({
  ...
  return (
    <div className="flex flex-col justify-start">
        ...
        <div className="flex items-center gap-2">
          {branch !== "master" && (
            ...
            >
              <Icon
                icon={
                  deleteBranchFetcher.state !== "idle" ? "spinner" : "trash"
                }
          ...
          >
            <Icon
              icon={
                checkoutBranchFetcher.state !== "idle" ? "spinner" : "turn-up"
              }
                    ...
                    showModal(SyncMergeModal, {
                      ...
                      handleDone: (conflicts?: MergeConflict[]) => {
                        if (Array.isArray(conflicts) && conflicts.length > 0) {
                          ...
                          reject(
                            new Error(
                              "You aborted the merge, no changes were made to working tree.",
                            ),
                          );
                        }
                      },
          ...
          >
            <Icon
              icon={
                mergeBranchFetcher.state !== "idle" ? "spinner" : "code-merge"
              }
      ...
      {errMsg && (
        <div className="whitespace-break-spaces text-right text-[--color-danger]">
          {errMsg}
        </div>
      )}
    </div>
...
const RemoteBranchItem = ({
  ...
  useEffect(() => {
    if (
      ...
      const error: string =
        pullBranchFetcher.data.error ||
        "An unexpected error occurred while pulling the branch.";
      showAlert({
...
export const GitProjectBranchesModal: FC<Props> = ({
  ...
  const errors =
    branchesFetcher.data && "errors" in branchesFetcher.data
      ? branchesFetcher.data.errors
      : [];
  ...
  useEffect(() => {
    if (branchesFetcher.state === "idle" && !branchesFetcher.data) {
      branchesFetcher.load(
        `/organization/${organizationId}/project/${projectId}/git/branches`,
      );
    }
  }, [branchesFetcher, organizationId, projectId, workspaceId]);

  const createNewBranchError =
    createBranchFetcher.data?.errors &&
    createBranchFetcher.data.errors.length > 0
  ...
  useEffect(() => {
    if (gitChangesFetcher.state === "idle" && !gitChangesFetcher.data) {
      ...
      gitChangesFetcher.load(
        `/organization/${organizationId}/project/${projectId}/git/changes`,
      );
    }
          ...
          {({ close }) => (
            <div className="flex flex-1 flex-col gap-4 overflow-hidden">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      <Icon
                        ...
                        icon={
                          createBranchFetcher.state !== "idle"
                            ? "spinner"
                            : "plus"
                        }
                      />{" "}
              ...
              <div className="flex max-h-96 flex-1 select-none flex-col divide-y divide-solid divide-[--hl-sm] overflow-hidden rounded border border-solid border-[--hl-sm]">
                <Heading className="p-2 text-sm font-semibold uppercase text-[--hl]">
                  Local Branches
                </Heading>
                <GridList

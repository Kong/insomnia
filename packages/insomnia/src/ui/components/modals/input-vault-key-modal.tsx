...
export const InputVaultKeyModal = (props: InputVaultKeyModalProps) => {
  ...
  const { organizations } = useRouteLoaderData(
    "/organization",
  ) as OrganizationLoaderData;
  const isLoading =
    resetVaultKeyFetcher.state !== "idle" ||
    validateVaultKeyFetcher.state !== "idle";

  useEffect(() => {
    // close modal and return new vault key after reset
    if (
      resetVaultKeyFetcher.data &&
      !resetVaultKeyFetcher.data.error &&
      resetVaultKeyFetcher.state === "idle"
    ) {
          ...
          {({ close }) => (
            <div className="flex flex-1 flex-col gap-4 overflow-hidden">
              ...
              {!resetDone ? (
                ...
                <>
                  <div>
                    Please save or download the vault key which will be needed

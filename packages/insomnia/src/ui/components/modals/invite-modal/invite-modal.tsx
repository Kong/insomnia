...
import {
  ...
  type Role,
  SELECTOR_TYPE,
} from "./organization-member-roles-selector";
...
const InviteModal: FC<{
  ...
  const total =
    (collaboratorsListLoader.data &&
      "total" in collaboratorsListLoader?.data &&
      collaboratorsListLoader.data.total) ||
    0;
  ...
  useEffect(() => {
    if (
      !collaboratorsListLoader.data &&
      collaboratorsListLoader.state === "idle"
    ) {
      collaboratorsListLoader.load(
        `/organization/${organizationId}/collaborators?page=0&per_page=${ItemsPerPage}`,
      );
    }
  ...
  const handleSearch = debounce((filter: string) => {
    if (filter.trim() === "") {
      ...
      setSearchParams(
        getSearchParamsString(searchParams, { page: 0, filter: "" }),
      );
    } else {
      collaboratorsListLoader.load(
  ...
  const resetCurrentPage = () => {
    collaboratorsListLoader.load(
      `/organization/${organizationId}/collaborators?page=${page}&per_page=${ItemsPerPage}`,
    );
    setSearchParams(
      getSearchParamsString(searchParams, { page, filter: queryInputString }),
    );
  };
        ...
        <Dialog className="relative outline-none">
          {({ close }) => (
            <>
              <Heading
                slot="title"
                className="mb-[24px] text-[22px] leading-[34px]"
              >
                Invite collaborators
              </Heading>
              <Button
                onPress={close}
                className="fa fa-times absolute right-0 top-0 text-xl"
              />
              ...
              {collaboratorListError && (
                <div className="flex h-[200px] items-center justify-center">
                  <p className="text-[12px] text-[--color-danger] first-letter:capitalize">
                    {collaboratorListError}
                  </p>
                </div>
              )}
                ...
                <>
                  ...
                  <PaginationBar
                    ...
                    onPrevPress={() => {
                      ...
                      setSearchParams(
                        getSearchParamsString(searchParams, { page: page - 1 }),
                      );
                    }}
                    onNextPress={() => {
                      collaboratorsListLoader.load(
...
const MemberListItem: FC<{
  ...
  const isCurrentUser =
    isAcceptedMember && currentUserAccountId === member.metadata.userId;

  const isPendingInvitationExpired =
    isPendingMember &&
    member.metadata.expiresAt &&
    isAfter(new Date(), new Date(member.metadata.expiresAt));
  const memberRoleName =
    allRoles.find((r: Role) => r.id === member.metadata.roleId)?.name ??
    "member";
      ...
      <div className="flex items-center gap-2">
        ...
        {member.type !== "group" && (
          <OrganizationMemberRolesSelector
            ...
            isDisabled={
              (isAcceptedMember && memberRoleName === "owner") ||
              invitationRoleUpdating ||
              memberRoleUpdating
            }
            ...
            hasPermissionToChangeRoles={
              permissionRef.current["update:membership"]
            }
    ...
    <div className="flex flex-col items-end">
      <div className="flex h-[50px] w-full flex-shrink-0 items-center justify-between">
        ...
        >
          <Icon
            icon="arrow-left"
            className="text h-[12px] w-[12px] text-[--color-font] disabled:text-[#00000080]"
          />
        ...
        >
          ...
          <Icon
            icon="arrow-right"
            className="h-[12px] w-[12px] text-[--color-font] disabled:text-[#00000080]"
          />
        </Button>
      </div>
    </div>
...
export const InviteModalContainer: FC<{
  ...
  const [currentUserRoleInOrg, setCurrentUserRoleInOrg] = useState<Role | null>(
    null,
  );
  ...
  const [currentOrgInfo, setCurrentOrgInfo] =
    useState<OrganizationAuth0 | null>(null);

  const isCurrentUserOrganizationOwner =
    currentUserAccountId === currentOrgInfo?.metadata?.ownerAccountId;
  ...
  function revalidateCurrentUserRoleAndPermissionsInOrg(
    organizationId: string,
  ) {
    return Promise.all([
      getCurrentUserRoleInOrg(organizationId).then(setCurrentUserRoleInOrg),

...
export const SyncMergeModal = forwardRef<SyncMergeModalHandle>((_, ref) => {
  const [state, setState] = useState<
    SyncMergeModalOptions & { isOpen: boolean }
  >({
    conflicts: [],
    isOpen: false,
    labels: { ours: "", theirs: "" },
  ...
  const [selectedConflict, setSelectedConflict] =
    useState<MergeConflict | null>(null);

  const selectedConflictDiff = selectedConflict
    ? getDiffFromConflict(selectedConflict)
    : null;
            ...
            <div className="flex flex-1 flex-col gap-4 overflow-hidden">
              ...
              <Form
                ...
                onSubmit={(event) => {
                  ...
                  if (
                    conflicts?.some(
                      (conflict) => conflict.choose === conflict.theirsBlob,
                    )
                  ) {
                    window.main.trackSegmentEvent({
                ...
                <div className="grid h-full gap-2 divide-x divide-solid divide-[--hl-md] overflow-hidden [grid-template-columns:300px_1fr]">
                  {conflicts && conflicts.length > 0 && (
                    <div className="flex flex-col gap-2 overflow-hidden">
                      ...
                      >
                        <Icon icon="code-merge" className="w-5" /> Resolve
                        conflicts
                      </Button>
                        ...
                        <GridList
                          ...
                          onSelectionChange={(keys) => {
                            if (keys !== "all") {
                              ...
                              setSelectedConflict(
                                conflicts.find((c) => c.key === selectedKey) ||
                                  null,
                              );
                            }
                          }}

...
function getDiff(previewDiffItem?: StageEntry) {
  ...
  if (
    previewDiffItem &&
    "previousBlobContent" in previewDiffItem &&
    previewDiffItem.previousBlobContent
  ) {
  ...
  if (
    previewDiffItem &&
    "blobContent" in previewDiffItem &&
    previewDiffItem.blobContent
  ) {
...
function getPreviewItemName(
  previewDiffItem?: StageEntry & { document?: { type: string } },
) {
  if (!previewDiffItem) {
    return "Diff view";
  }
  ...
  if (
    "document" in previewDiffItem &&
    previewDiffItem.document &&
    "type" in previewDiffItem.document
  ) {
...
export const SyncStagingModal = ({ onClose, status, syncItems }: Props) => {
  ...
  const unstagedChanges = Object.entries(status.unstaged).map(
    ([key, entry]) => ({
      ...entry,
      document:
        syncItems.find((item) => item.key === key)?.document ||
        "deleted" in entry
          ? { type: getModelTypeById(key) }
          : undefined,
      id: `unstaged-${key}`,
  ...
  const isPushing =
    state !== "idle" && formAction?.endsWith("create-snapshot-and-push");
  const isCreatingSnapshot =
    state !== "idle" && formAction?.endsWith("create-snapshot");
                    ...
                    <div className="flex max-h-96 w-full flex-col gap-2 overflow-hidden">
                      <Heading className="group flex w-full flex-shrink-0 items-center justify-between gap-2 py-1 font-semibold">
                        ...
                        <Button
                          ...
                          onPress={() => {
                            unstageChanges(
                              stagedChanges.map((item) => item.key),
                            );
                          }}
                        >
                          <Icon icon="minus" />
                            ...
                            return (
                              <GridListItem className="group flex w-full select-none items-center justify-between overflow-hidden px-2 py-1 text-[--hl] outline-none transition-colors hover:bg-[--hl-xs] focus:bg-[--hl-sm] aria-selected:bg-[--hl-sm] aria-selected:text-[--color-font]">
                                <span className="truncate">
                                  {item.entry.name || item.entry.document?.type}
                                </span>
                      ...
                      <Heading className="group flex w-full flex-shrink-0 items-center justify-between py-1 font-semibold">
                        ...
                        <div className="flex items-center gap-2">
                          <Button
                            ...
                            onPress={() => {
                              stageChanges(
                                unstagedChanges.map((item) => item.key),
                              );
                            }}
                          >
                            <Icon icon="plus" />

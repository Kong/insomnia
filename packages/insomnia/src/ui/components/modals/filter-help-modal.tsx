...
const HelpExamples: FC<{ helpExamples: HelpExample[] }> = ({
  helpExamples,
}) => (
...
const JSONPathHelp: FC = () => (
  <ModalBody className="pad">
    <p>
      Use <Link href="http://goessner.net/articles/JsonPath/">JSONPath</Link> to
      filter the response body. Here are some examples that you might use on a
      book store API:
    </p>
    <HelpExamples
      helpExamples={[
        {
          code: "$.store.books[*].title",
          description: "Get titles of all books in the store",
        },
        {
          code: "$.store.books[?(@.price < 10)].title",
    ...
    <p className="notice info">
      Note that there's{" "}
      <Link href="https://cburgmer.github.io/json-path-comparison/">
        no standard
      </Link>{" "}
      for JSONPath. Insomnia uses{" "}
      <Link href="https://www.npmjs.com/package/jsonpath-plus">
        jsonpath-plus
      </Link>
      .
    </p>
...
const XPathHelp: FC = () => (
  <ModalBody className="pad">
    <p>
      Use <Link href="https://www.w3.org/TR/xpath/">XPath</Link> to filter the
      response body. Here are some examples that you might use on a book store
      API:
    </p>
    <HelpExamples
      helpExamples={[
        ...
        {
          code: "/store/books[price < 10]",
          description: "Get books costing less than $10",
        },
        {
          code: "/store/books[last()]",
          description: "Get the last book in the store",
        },
        {
          code: "count(/store/books)",
          description: "Get the number of books in the store",
        },
      ]}
    />
...
export const FilterHelpModal = forwardRef<FilterHelpModalHandle, ModalProps>(
  (_, ref) => {

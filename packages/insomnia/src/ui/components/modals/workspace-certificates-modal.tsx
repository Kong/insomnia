...
const AddClientCertificateModal = ({ onClose }: { onClose: () => void }) => {
  ...
  useEffect(() => {
    if (
      createClientCertificateFetcher.data &&
      createClientCertificateFetcher.data.certificate
    ) {
      onClose();
    }
              ...
              <div className="flex w-full flex-1 basis-96 select-none flex-col gap-2 overflow-y-auto rounded px-2">
                ...
                >
                  ...
                  <Tabs className="rounded-sm border border-solid border-[--hl-md]">
                    ...
                    <TabPanel
                      className="flex w-full flex-col gap-2 overflow-hidden p-2"
                      id="certificate"
                    >
                      <Label
...
const ClientCertificateGridListItem = ({
  certificate,
}: {
      ...
      <div className="flex w-full items-center gap-2">
        {Boolean(certificate.pfx || certificate.cert) && (
          <Icon
            icon="file-contract"
            className="w-4"
            title={certificate.pfx || certificate.cert || ""}
          />
        )}
        {certificate.key && <Icon icon="key" title={certificate.key} />}
        <div className="flex-1 truncate text-sm text-[--color-font]">
          {certificate.host}
        </div>
...
export const CertificatesModal = ({ onClose }: { onClose: () => void }) => {
  ...
  const [isAddClientCertificateModalOpen, setIsAddClientCertificateModalOpen] =
    useState(false);
          ...
          {({ close }) => (
            <div className="flex h-full flex-1 flex-col gap-4 overflow-hidden">
              ...
              <div className="flex w-full flex-1 basis-96 select-none flex-col gap-6 overflow-hidden overflow-y-auto rounded">
                ...
                <div className="flex flex-col gap-2">
                  {caCertificate ? (
                    <div className="flex items-center justify-between gap-2 rounded-sm border border-solid border-[--hl-sm] p-4">
                      ...
                      <div
                        className="flex-1 truncate text-sm text-[--color-font]"
                        title={caCertificate.path || ""}
                      >
                        {caCertificate?.path
                          ?.split("\\")
                          ?.pop()
                  ...
                  <p className="max-w-[80ch] text-sm italic text-[--hl]">
                    ...
                    One or more PEM format certificates in a single file to pass
                    to curl. Overrides the root CA certificate. On MacOS please
                    upload your local Keychain certificates here.
                  </p>
                </div>
                <div className="flex items-center justify-between gap-2">
                ...
                >
                  {(item) => (
                    <ClientCertificateGridListItem certificate={item.cert} />
                  )}
                </GridList>
              </div>
              <div className="flex items-center justify-end gap-2">

...
export const GitProjectStagingModal: FC<{ onClose: () => void }> = ({
  ...
  const isCreatingSnapshot =
    state !== "idle" &&
    formAction ===
      `/organization/${organizationId}/project/${projectId}/git/commit`;
  const isPushing =
    state !== "idle" &&
    formAction ===
      `/organization/${organizationId}/project/${projectId}/git/commit-and-push`;
            ...
            <div className="flex flex-1 flex-col gap-4 overflow-hidden">
              <div className="flex flex-shrink-0 items-center justify-between gap-2">
                <Heading slot="title" className="text-2xl">
                  Commit changes{" "}
                  {gitChangesFetcher.state === "loading" && (

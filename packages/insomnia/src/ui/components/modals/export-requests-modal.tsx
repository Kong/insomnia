...
export const RequestGroupRow: FC<{
  ...
  const isIndeterminate =
    selectedRequests > 0 && selectedRequests < totalRequests;
    ...
    <li key={requestGroup._id} className="flex flex-col">
      <div className="flex items-center gap-2 p-2">
        <Checkbox
          ...
          onChange={(isSelected) =>
            handleSetItemSelected(requestGroup._id, isSelected)
          }
...
export const ExportRequestsModal = ({
  ...
  useEffect(() => {
    const isIdleAndUninitialized =
      workspaceFetcher.state === "idle" && !workspaceFetcher.data;
    if (isIdleAndUninitialized) {
      workspaceFetcher.load(
        `/organization/${organizationId}/project/${projectId}/workspace/${workspaceIdToExport}`,
      );
    }
  ...
  useEffect(() => {
    const createTreeNode = (child: Child): Node => {
      ...
      const children = child.children.map((child: Child) =>
        createTreeNode(child),
      );
      const totalRequests =
        +docIsRequest +
        children.reduce((acc, { totalRequests }) => acc + totalRequests, 0);
      return {
        doc: child.doc,
        collapsed: false,
        children,
  ...
  const getSelectedRequestIds = (node: Node): string[] => {
    ...
    return node.children
      .map((child) => getSelectedRequestIds(child))
      .reduce((acc, reqIds) => [...acc, ...reqIds], []);
  };
  ...
  const setRequestGroupCollapsed = (
    node: Node,
    isCollapsed: boolean,
    ...
    return !!node.children.find((child) =>
      setRequestGroupCollapsed(child, isCollapsed, requestGroupId),
    );
  };

  const isExportDisabled =
    (state?.treeRoot?.totalRequests &&
      state?.treeRoot?.totalRequests > 0 &&
      state?.treeRoot?.selectedRequests === 0) ||
    false;
              ...
              <div className="max-h-96 min-h-[20rem] w-full select-none overflow-y-auto rounded border border-solid border-[--hl-sm]">
                <Tree
                  ...
                  handleSetItemSelected={(
                    ...
                    if (
                      state?.treeRoot &&
                      setItemSelected(state?.treeRoot, isSelected, itemId)
                    ) {
                      setState({ treeRoot: state?.treeRoot });
                    }
                  }}
                  ...
                  onPress={() => {
                    state?.treeRoot &&
                      exportRequestsToFile(
                        workspaceIdToExport,
                        getSelectedRequestIds(state.treeRoot),
                      );
                    close();
                  }}

...
type GetRefHandleFromProps<Props> =
  Props extends React.RefAttributes<infer TModalHandle> ? TModalHandle : never;
...
>(
  modalComponent: ModalComponent<TModalProps>,
  config?: ModalHandleShowOptions<GetRefHandleFromProps<TModalProps>>,
) {
  ...
  const modalHandle = getModalComponentHandle(
    name,
  ) as unknown as GetRefHandleFromProps<TModalProps>;

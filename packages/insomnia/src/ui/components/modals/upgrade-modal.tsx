...
export const UpgradeModal = forwardRef<UpgradeModalHandle, ModalProps>(
  (_, ref) => {
    ...
    useImperativeHandle(
      ...
      () => ({
        ...
        show: (options: UpgradeModalOptions) => {
          ...
          const planDetail =
            newPlan === "team" ? "Pro plan or above" : "Enterprise plan";
          ...
          const onDone = async (isYes: boolean) => {
            if (isYes) {
              window.main.openInBrowser(
                `${getAppWebsiteBaseURL()}/app/subscription/update?plan=team`,
              );
            }
          };
          if (isOwner) {

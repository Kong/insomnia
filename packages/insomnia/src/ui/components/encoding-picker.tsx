  ...
  return (
    <ComboBox
      ...
      defaultFilter={(textValue, filter) => {
        const encodingKey =
          BUILT_IN_ENCODINGS.find((e) => e.label === textValue)?.key || "";
        return (
          Boolean(
            fuzzyMatch(filter, encodingKey, { splitSpace: false, loose: true })
              ?.indexes,
              ...
              {({ isSelected }) => (
                <>
                  ...
                  {isSelected && (
                    <Icon
                      icon="check"
                      className="ml-1 justify-self-end text-[--color-success]"
                    />
                  )}
                </>
              )}

...
export const CommandPalette = memo(function CommandPalette({
  ...
  const requestSwitchKeyCombination = getPlatformKeyCombinations(
...
const CommandPaletteCombobox = ({ close }: { close: () => void }) => {
      ...
      children: currentFiles.map((file) => ({
        ...
        description: (
          <span className="flex items-center gap-1">
            <span className="px-2 text-[--hl]">
              {scopeToLabelMap[file.item.scope]}
            </span>
          </span>
        ),
        ...
        presence: presence
          .filter(
            (p) => p.project === file.item.teamProjectId && p.file === file.id,
          )
  ...
  useEffect(() => {
    if (
      ...
      prevPullFetcherState.current !== "idle"
    ) {
  ...
  useEffect(() => {
    if (
      setActiveEnvironmentFetcher.state === "idle" &&
      prevEnvFetcherState.current !== "idle"
    ) {
      close();
    }
  ...
  const pullingFileBackedProjectId =
    pullFileFetcher.formData?.get("backendProjectId");
  const pullingFile = remoteFiles.find(
    (file) => file.item.projectId === pullingFileBackedProjectId,
  );
  ...
  return (
    <ComboBox
      ...
      defaultFilter={(textValue, filter) => {
        return Boolean(
          fuzzyMatch(filter, textValue, { splitSpace: false, loose: true })
            ?.indexes,
        );
      }}
      ...
      {({ isOpen }) => {
        return (
          <>
            <Label
              aria-label="Filter"
              className="group relative flex flex-1 items-center pt-0"
            >
              {isPullingFile ? (
                <>
                  <Icon
                    icon="spinner"
                    className="absolute left-4 animate-spin text-[--color-font]"
                  />
                  <div
                    slot="input"
                ...
                <>
                  <Icon
                    icon="search"
                    className="absolute left-4 text-[--color-font]"
                  />
                  <Input
                    slot="input"
              ...
              >
                {(section) => (
                  <Section className="flex flex-1 flex-col">
                    <Header className="select-none p-2 text-xs uppercase text-[--hl]">
                      {section.name}
                    </Header>
                      ...
                      {(item) => (
                        ...
                        >
                          ...
                          >
                            ...
                            <Text
                              className="flex-1 truncate px-1 text-sm text-[--hl-md]"
                              slot="description"
                            >
                              {item.description}
                            </Text>

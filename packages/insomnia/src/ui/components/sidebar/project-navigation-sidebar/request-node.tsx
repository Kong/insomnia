...
export const RequestNode = ({ item, onToggleFolder, className }: RequestNodeProps) => {
  ...
  return (
    ...
    >
      ...
      {isPinnedRequest ? (
        <div
          className={`ml-2 flex h-full min-w-0 flex-1 items-center overflow-hidden border-x border-solid border-(--hl-md) bg-(--hl-xs) pr-2 group-hover:bg-(--hl-sm) group-focus:bg-(--hl-sm) ${isLastPinned ? 'rounded-sm border-b' : ''}`}
        >
          {content}
        </div>
      ) : (

...
export const CurlResponsePane: FC<{ requestId: string }> =
  ({
    requestId,
  }) => {
    ...
    return <CurlActiveResponsePane requestId={requestId} response={response} />;
  };

const CurlActiveResponsePane: FC<{ requestId: string; response: Response }> = ({
  requestId,
  response,
}) => {
  ...
  const handleSelection = (event: CurlEvent) => {
    setSelectedEvent((selected: CurlEvent | null) => selected?._id === event._id ? null : event);
  };

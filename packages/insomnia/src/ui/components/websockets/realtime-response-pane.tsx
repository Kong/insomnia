...
const RealtimeActiveResponsePane: FC<{
  ...
  const [selectedEvent, setSelectedEvent] = useState<
    CurlEvent | WebSocketEvent | null
  >(null);
  const [timeline, setTimeline] = useState<ResponseTimelineEntry[]>([]);
  const [clearEventsBefore, setClearEventsBefore] = useState<number | null>(
    null,
  );
  ...
  const allEvents = useRealtimeConnectionEvents({
    responseId: response._id,
    protocol,
  });
  const handleSelection = (event: CurlEvent | WebSocketEvent) => {
    setSelectedEvent((selected: CurlEvent | WebSocketEvent | null) =>
    ...
    <Pane type="response">
      ...
      >
        ...
        >
          <PanelGroup
            direction="vertical"
            className="grid h-full w-full grid-rows-[repeat(auto-fit,minmax(0,1fr))]"
          >
            {response.error ? (
              ...
              <>
                <Panel
                  minSize={10}
                  defaultSize={50}
                  className="box-border flex w-full flex-1 flex-col overflow-hidden"
                >
                  <div
                    style={{
                      display: "flex",
                  ...
                  >
                    <select
                      ...
                      onChange={(e) =>
                        setEventType(e.currentTarget.value as CurlEvent["type"])
                      }
                    >
                ...
                {selectedEvent && (
                  <>
                    <PanelResizeHandle
                      className={"h-[1px] w-full bg-[--hl-md]"}
                    />

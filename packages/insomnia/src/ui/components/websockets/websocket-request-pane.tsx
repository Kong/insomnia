  ...
  return (
    ...
    >
      <p className="notice info no-margin-top no-margin-bottom">
        This section is now locked since the connection has already been
        established. To change these settings, please disconnect first.
      </p>
    </div>
  );
};
...
const WebSocketRequestForm: FC<FormProps> = ({
  ...
  const interpolateOpenAndSend = async (payload: string) => {
    try {
      ...
      const readyState = await window.main.webSocket.readyState.getCurrent({

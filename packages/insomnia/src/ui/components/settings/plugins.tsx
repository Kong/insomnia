  ...
  return (
    <div>
          ...
          <tbody>
            {plugins.map((plugin) => {
              const link = path.join(
                /^insomnia-plugin-/.test(plugin.name)
                  ? PLUGIN_HUB_BASE
                  : NPM_PACKAGE_BASE,
                plugin.name,
              );
              return !plugin.directory ? null : (
                <tr key={plugin.name}>
                  ...
                  >
                    ...
                    <Button
                      onPress={() =>
                        window.shell.showItemInFolder(plugin.directory)
                      }
                    >
                      Reveal Folder
                    </Button>
      ...
      {error && (
        <div className="notice error margin-bottom text-left">
          <button
            ...
            onClick={() => setState((state) => ({ ...state, error: null }))}
          >
          ...
          <div className="selectable force-pre-wrap">
            ...
            <br />
            Try using the install button on{" "}
      ...
      <form
        onSubmit={async (event) => {
          ...
          if (!npmPluginValue.startsWith("insomnia-plugin-")) {
            newState.installPluginErrMsg =
              "Please enter a plugin name starting with insomnia-plugin-";
      ...
      >
        <div className="form-row">
          <div className="form-control">
            ...
            <span>
              Enter the full name of an npm package beginning with
              insomnia-plugin-*
            </span>
          </div>
        </div>
      ...
      <div className="form-row mt-6">
        <div className="form-control">
          <span className="mr-2">
            Plugin Installation Trusted Certificates File (
            {ACCEPTED_NODE_CA_FILE_EXTS.join(", ")})
          </span>
          <Tooltip
        ...
        <div className="form-control width-auto">
          <FileTrigger
            ...
            onSelect={(fileList) => {
              ...
              patchSettings({
                pluginNodeExtraCerts: window.webUtils.getPathForFile(files[0]),
              });
            }}
          >

...
const ThemePreview: FC<{ theme: PluginTheme }> = ({
    ...
    <div className="flex flex-col gap-2">
      <Label className="flex items-center gap-2">
        ...
        <HelpTooltip className="space-left">
          Select different themes for when you’re using light versus dark color
          schemes on your OS. Check this box, then hover over a theme and select
          either light (sun) or dark (moon).
        </HelpTooltip>
      </Label>
      ...
      <ul className="grid grid-flow-row grid-cols-3 gap-4 sm:grid-cols-4 md:grid-cols-5">
        {themes.map((theme) => (
          <div key={theme.name} className="flex flex-col gap-1">
            <div className="select-none truncate text-center text-sm">
              {theme.displayName}
            </div>
            <div

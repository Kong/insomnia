...
export const isKeyCombinationInRegistry = (
  ...
  !!Object.values(hotKeyRegistry).find((bindings) =>
    getPlatformKeyCombinations(bindings).find((keyComb) =>
      areSameKeyCombinations(pressedKeyComb, keyComb),
    ),
  );
      ...
      <div className="row-spaced margin-bottom-xs">
        <div>
          <PromptButton
            ...
            onClick={() =>
              patchSettings({ hotKeyRegistry: newDefaultRegistry() })
            }
          >
            Reset all
          </PromptButton>
      ...
      <table className="table--fancy">
        <tbody>
          {Object.entries(hotKeyRegistry).map(([key, platformCombinations]) => {
            ...
            // smelly
            const keyCombosForThisPlatform = getPlatformKeyCombinations(
            ...
            return (
              <tr key={keyboardShortcut}>
                <td style={{ verticalAlign: "middle" }}>
                  {keyboardShortcutDescriptions[keyboardShortcut]}
                </td>
                        ...
                        onClick={() =>
                          showModal(AddKeyCombinationModal, {
                            ...
                            addKeyCombination: (
                              ...
                              const keyCombs = getPlatformKeyCombinations(
                                hotKeyRegistry[keyboardShortcut],
                              );
                    ...
                    >
                      {
                        ...
                        keyCombosForThisPlatform.map(
                          (keyComb: KeyCombination) => {
                            const display = constructKeyCombinationDisplay(
                              keyComb,
                              false,
                            );
                            return (
                    ...
                    <DropdownSection aria-label="Reset keyboard shortcuts section">
                      <DropdownItem aria-label="Reset keyboard shortcuts">
                        <ItemContent
                          ...
                          onClick={() => {
                            hotKeyRegistry[keyboardShortcut] =
                              newDefaultRegistry()[keyboardShortcut];
                            patchSettings({ hotKeyRegistry });
                          }}
                        />
                      </DropdownItem>

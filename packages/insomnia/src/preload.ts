...
const webSocket: WebSocketBridgeAPI = {
  ...
  readyState: {
    getCurrent: (options) =>
      ipcRenderer.invoke("webSocket.readyState", options),
  },
  event: {
    findMany: (options) =>
      ipcRenderer.invoke("webSocket.event.findMany", options),
...
const grpc: gRPCBridgeAPI = {
  ...
  loadMethodsFromReflection: (options) =>
    ipcRenderer.invoke("grpc.loadMethodsFromReflection", options),
};

const secretStorage: secretStorageBridgeAPI = {
  setSecret: (key, secret) =>
    ipcRenderer.invoke("secretStorage.setSecret", key, secret),
  ...
  encryptString: (raw) =>
    ipcRenderer.invoke("secretStorage.encryptString", raw),
...
const git: GitServiceAPI = {
  loadGitRepository: (options) =>
    ipcRenderer.invoke("git.loadGitRepository", options),
  getGitBranches: (options) =>
    ipcRenderer.invoke("git.getGitBranches", options),
  gitFetchAction: (options) =>
    ipcRenderer.invoke("git.gitFetchAction", options),
  gitLogLoader: (options) => ipcRenderer.invoke("git.gitLogLoader", options),
  gitChangesLoader: (options) =>
    ipcRenderer.invoke("git.gitChangesLoader", options),
  ...
  initGitRepoClone: (options) =>
    ipcRenderer.invoke("git.initGitRepoClone", options),
  ...
  commitToGitRepo: (options) =>
    ipcRenderer.invoke("git.commitToGitRepo", options),
  commitAndPushToGitRepo: (options) =>
    ipcRenderer.invoke("git.commitAndPushToGitRepo", options),
  createNewGitBranch: (options) =>
    ipcRenderer.invoke("git.createNewGitBranch", options),
  checkoutGitBranch: (options) =>
    ipcRenderer.invoke("git.checkoutGitBranch", options),
  mergeGitBranch: (options) =>
    ipcRenderer.invoke("git.mergeGitBranch", options),
  deleteGitBranch: (options) =>
    ipcRenderer.invoke("git.deleteGitBranch", options),
  pushToGitRemote: (options) =>
    ipcRenderer.invoke("git.pushToGitRemote", options),
  pullFromGitRemote: (options) =>
    ipcRenderer.invoke("git.pullFromGitRemote", options),
  continueMerge: (options) => ipcRenderer.invoke("git.continueMerge", options),
  discardChanges: (options) =>
    ipcRenderer.invoke("git.discardChanges", options),
  ...
  unstageChanges: (options) =>
    ipcRenderer.invoke("git.unstageChanges", options),
  diffFileLoader: (options) =>
    ipcRenderer.invoke("git.diffFileLoader", options),
  getRepositoryDirectoryTree: (options) =>
    ipcRenderer.invoke("git.getRepositoryDirectoryTree", options),

  initSignInToGitHub: () => ipcRenderer.invoke("git.initSignInToGitHub"),
  completeSignInToGitHub: (options) =>
    ipcRenderer.invoke("git.completeSignInToGitHub", options),
  signOutOfGitHub: () => ipcRenderer.invoke("git.signOutOfGitHub"),
  getGitHubRepositories: (options) =>
    ipcRenderer.invoke("git.getGitHubRepositories", options),
  getGitHubRepository: (options) =>
    ipcRenderer.invoke("git.getGitHubRepository", options),

  initSignInToGitLab: () => ipcRenderer.invoke("git.initSignInToGitLab"),
  completeSignInToGitLab: (options) =>
    ipcRenderer.invoke("git.completeSignInToGitLab", options),
...
const main: Window["main"] = {
  ...
  completeExecutionStep: (options) =>
    ipcRenderer.send("completeExecutionStep", options),
  updateLatestStepName: (options) =>
  ...
  authorizeUserInWindow: (options) =>
    ipcRenderer.invoke("authorizeUserInWindow", options),
  setMenuBarVisibility: (options) =>
    ipcRenderer.send("setMenuBarVisibility", options),
  installPlugin: (options) => ipcRenderer.invoke("installPlugin", options),
  curlRequest: (options) => ipcRenderer.invoke("curlRequest", options),
  cancelCurlRequest: (options) =>
    ipcRenderer.send("cancelCurlRequest", options),
  ...
  trackSegmentEvent: (options) =>
    ipcRenderer.send("trackSegmentEvent", options),
  trackPageView: (options) => ipcRenderer.send("trackPageView", options),
  showNunjucksContextMenu: (options) =>
    ipcRenderer.send("show-nunjucks-context-menu", options),
  showContextMenu: (options) => ipcRenderer.send("showContextMenu", options),
  database: {
    caCertificate: {
      create: (options) =>
        ipcRenderer.invoke("database.caCertificate.create", options),
    },
  },
  hiddenBrowserWindow: {
    runScript: (options) =>

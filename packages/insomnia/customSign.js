...
exports.default = async function (configuration) {
  ...
  if (
    configuration.options.target.length === 0 ||
    configuration.options.target[0].target !== "squirrel"
  ) {
    console.log(
      "[customSign] Skipping signing because target is not windows squirrel.",
    );
    return;
  }
  ...
  if (!USERNAME || !PASSWORD || !CREDENTIAL_ID || !TOTP_SECRET) {
    console.log(
      "[customSign] Skipping signing,  Missing required environment variables.",
    );
    return;
  }

import type { OAuth2ResponseType, RequestAuthentication } from 'insomnia-database/models/request';
...
export function fromPreRequestAuth(auth: RequestAuth): RequestAuthentication {
  ...
  const findValueInOauth2Options = (targetKey: string, kvs?: { key: string; value: string }[] | OAuth2AuthOption[]) => {
    if (!kvs) {
      return '';
    }

...
describe('NeDBClient', () => {
  ...
  describe('readdir()', () => {
    it('reads model IDs from model type folders', async () => {
      const neDbClient = new NeDBClient('wrk_1', BASE_SPACE_ID);
  ...
  describe('readFile()', () => {
    it('reads file from model/id folders', async () => {
      ...
      const pNeDB = new NeDBClient('wrk_1', BASE_SPACE_ID);
  ...
  describe('writeFile()', () => {
    it('should ignore files not in GIT_INSOMNIA_DIR directory', async () => {
      ...
      const neDbClient = new NeDBClient(workspaceId, BASE_SPACE_ID);
      const env = {
        _id: 'env_1',

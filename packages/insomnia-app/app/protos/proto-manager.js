...
class ProtoManager {
  async deleteFile(protoFile: ProtoFile, callback: string => void): Promise<void> {
    showAlert({
  ...
  async addDirectory(workspaceId: string): Promise<void> {
  ...
  async _readFile(): Promise<{ fileName: string, fileContents: string } | null> {
    try {
      // Select file
      const { filePath, canceled } = await selectFileOrFolder({
        itemTypes: ['file'],
  ...
  async addFile(workspaceId: string, callback: string => void): Promise<void> {
    const result = await this._readFile();
    if (result) {
  ...
  async updateFile(protoFile: ProtoFile, callback: string => void): Promise<void> {
    const result = await this._readFile();
    if (result) {
  ...
  async renameFile(protoFile: ProtoFile, name: string): Promise<void> {

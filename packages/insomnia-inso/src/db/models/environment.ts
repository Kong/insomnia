...
const loadBaseEnvironmentForWorkspace = (
  db: Database,
  workspaceId: string,
): Environment => {
  logger.trace(
  ...
  const items = db.Environment.filter(
    (environment) => environment.parentId === workspaceId,
  );
...
export const promptEnvironment = async (
  ...
  const prompt = new AutoComplete({
    ...
    choices: subEnvironments.map((subEnv) =>
      getDbChoice(generateIdIsh(subEnv, 14), subEnv.name),
    ),
  });

...
} from "insomnia/src/models/environment";
...
import {
  exportSpecification,
  writeFileWithCliOptions,
} from "./commands/export-specification";
import {
  getRuleSetFileFromFolderByFilename,
  lintSpecification,
} from "./commands/lint-specification";
...
export const tryToReadInsoConfigFile = async (
  configFile?: string,
  workingDir?: string,
) => {
  try {
    ...
    const results = configFile
      ...
      : await explorer.search(workingDir || process.cwd());
...
export type ModifiedConsola = ReturnType<typeof consola.create> & {
  __getLogs: () => LogsByType;
};
...
const resolveSpecInDatabase = async (
  identifier: string,
  ...
  if (useLocalAppData) {
    logger.warn(
      "No working directory or export file provided, using local app data directory.",
    );
    pathToSearch = localAppDir;
  } else {
    pathToSearch = path.resolve(
  ...
  if (!db.ApiSpec.length) {
    throw new InsoError(
      `Specification content not found using API spec id: "${identifier}" in "${pathToSearch}"`,
    );
  }
  const specFromDb = identifier
    ? loadApiSpec(db, identifier)
    : await promptApiSpec(db, options.ci);
...
const getWorkspaceOrFallback = async (
  db: Database,
  identifier: string,
  ci: boolean,
) => {
...
const getRequestsToRunFromListOrWorkspace = (
  db: Database,
  workspaceId: string,
  item: string[],
): Request[] => {
  const getRequestGroupIdsRecursively = (from: string[]): string[] => {
    const parentIds = db.RequestGroup.filter((rg) =>
      from.includes(rg.parentId),
    ).map((rg) => rg._id);
    return [
      ...parentIds,
      ...(parentIds.length > 0 ? getRequestGroupIdsRecursively(parentIds) : []),
    ];
  };
  const hasItems = item.length > 0;
  if (hasItems) {
    const folderIds = item.filter((id) =>
      db.RequestGroup.find((rg) => rg._id === id),
    );
    const allRequestGroupIds = [
      ...folderIds,
      ...getRequestGroupIdsRecursively(folderIds),
    ];
    const folderRequests = db.Request.filter((req) =>
      allRequestGroupIds.includes(req.parentId),
    ) as Request[];
    const reqItems = db.Request.filter((req) =>
      item.includes(req._id),
    ) as Request[];
  ...
  return db.Request.filter((req) =>
    [workspaceId, ...allRequestGroupIds].includes(req.parentId),
  ) as Request[];
};
...
const logTestResult = (
  ...
  const fallbackReporter = testResults
    .map((r) => `${r.status === "passed" ? "✅" : "❌"} ${r.testCase}`)
    .join("\n");
...
const pathToIterationData = async (
  pathOrUrl: string,
  env: string[],
): Promise<UserUploadEnvironment[]> => {
  const envAsObject = env
    .map((envString) =>
      Object.fromEntries(new URLSearchParams(envString).entries()),
    )
  ...
  const list = getListFromFileOrUrl(content, fileType).map((data) => ({
    ...data,
    ...envAsObject,
  }));
  return transformIterationDataToEnvironmentList(list);
};
const getListFromFileOrUrl = (
  content: string,
  fileType?: string,
...
const transformIterationDataToEnvironmentList = (
...
`,
  ...
  run
    ...
    .option(
      "-r, --reporter <reporter>",
      `reporter to use, options are [${reporterTypes.join(", ")}]`,
      defaultReporter,
    )
    ...
    .action(
      async (
        ...
        const __configFile = await tryToReadInsoConfigFile(
        ...
        if (
          options.reporter &&
          !reporterTypes.find((r) => r === options.reporter)
        ) {
          logger.fatal(
            `Reporter "${options.reporter}" not unrecognized. Options are [${reporterTypes.join(", ")}].`,
          );
        ...
        // Find suites
        const suites = identifier
          ? loadTestSuites(db, identifier)
          : await promptTestSuites(db, !!options.ci);
        ...
        try {
          ...
          const success = options.verbose
            ? await runTestPromise
            : await noConsoleLog(() => runTestPromise);
  ...
  run
    ...
    .action(
      async (
        ...
        const workspace = await getWorkspaceOrFallback(
          db,
          identifier,
          options.ci,
        );
        if (!workspace) {
          logger.fatal(
            "No workspace found in the provided data store or fallbacks.",
          );
          return process.exit(1);
        }
        ...
        if (options.globals) {
          ...
          if (!isGlobalFile) {
            const globalEnv = db.Environment.find(
              (env) =>
                matchIdIsh(env, options.globals) ||
                env.name === options.globals,
            );
            if (!globalEnv) {
              logger.warn(
                "No global environment found with id or name",
                options.globals,
              );
              return process.exit(1);
            }
          ...
          if (isGlobalFile) {
            const globalEnvDb = await insomniaExportAdapter(options.globals, [
              "Environment",
            ]);
            logger.trace(
        ...
        if (!environment) {
          logger.fatal(
            "No environment identified; cannot run requests without a valid environment.",
          );
          return process.exit(1);
        }

        let requestsToRun = getRequestsToRunFromListOrWorkspace(
          db,
          workspaceId,
          options.item,
        );
        if (options.requestNamePattern) {
          requestsToRun = requestsToRun.filter((req) =>
            req.name.match(new RegExp(options.requestNamePattern)),
          );
        }
        ...
        // sort requests
        const isRunningFolder =
          options.item.length === 1 && options.item[0].startsWith("fld_");
        if (options.item.length && !isRunningFolder) {
          ...
          requestsToRun = requestsToRun.sort(
            (a, b) =>
              (requestOrder.get(a._id) || requestsToRun.length) -
              (requestOrder.get(b._id) || requestsToRun.length),
          );
        } else {
          const getAllParentGroupSortKeys = (doc: BaseModel): number[] => {
            const parentFolder = db.RequestGroup.find(
              (rg) => rg._id === doc.parentId,
            );
            if (parentFolder === undefined) {
              return [];
            }
            return [
              (parentFolder as RequestGroup).metaSortKey,
              ...getAllParentGroupSortKeys(parentFolder),
            ];
          };
          ...
          requestsToRun = requestsToRun
            .map((request) => {
              const allParentGroupSortKeys = getAllParentGroupSortKeys(
            ...
            .sort((a, b) => {
              ...
              for (
                ;
                i < a.ancestors.length && j < b.ancestors.length;
                i++, j++
              ) {
        ...
        try {
          ...
          const iterationData = await pathToIterationData(
          ...
          for (let i = 0; i < iterationCount; i++) {
            ...
            while (reqIndex < requestsToRun.length) {
              ...
              const appendNewLineIfNeeded = (str: string) =>
                str.endsWith("\n") ? str : str + "\n";
...
Test results:`);
                ...
                const hasFailedTests = res.testResults.some(
                  (t) => t.status === "failed",
                );
                if (hasFailedTests) {
                  success = false;
              ...
              await new Promise((r) =>
                setTimeout(r, parseInt(options.delayRequest, 10)),
              );

              if (res.nextRequestIdOrName) {
                const offset = getNextRequestOffset(
                  requestsToRun.slice(reqIndex),
                  res.nextRequestIdOrName,
                );
                reqIndex += offset;
                if (reqIndex < requestsToRun.length) {
                  console.log(
                    `The next request has been pointed to "${requestsToRun[reqIndex].name}"`,
                  );
                } else {
                  console.log(
                    `No request has been found for "${res.nextRequestIdOrName}", ending the iteration`,
                  );
                }
              } else {
                reqIndex++;
  ...
  program
    ...
    .action(async (identifier) => {
      ...
      try {
        isIdentiferAFile =
          identifier && (await fs.promises.stat(identifierAsAbsPath)).isFile();
      } catch {}
      const pathToSearch = "";
      let specContent;
      let rulesetFileName;
      if (isIdentiferAFile) {
        // try load as a file
        logger.trace(
          `Linting specification file from identifier: \`${identifierAsAbsPath}\``,
        );
        specContent = await fs.promises.readFile(identifierAsAbsPath, "utf-8");
        rulesetFileName =
          await getRuleSetFileFromFolderByFilename(identifierAsAbsPath);
        if (!specContent) {
          logger.fatal(
            `Specification content not found using path: ${identifier} in ${identifierAsAbsPath}`,
          );
          return process.exit(1);
        }
      }
      ...
      try {
        const { isValid } = await lintSpecification({
          specContent,
          rulesetFileName,
        });
  ...
  program
    ...
    .option(
      "-s, --skipAnnotations",
      'remove all "x-kong-" annotations, defaults to false',
      false,
    )
    .action(
      async (identifier, cmd: { output: string; skipAnnotations: boolean }) => {
        const globals: GlobalOptions = program.optsWithGlobals();
        const commandOptions = { ...globals, ...cmd };
        const __configFile = await tryToReadInsoConfigFile(
          commandOptions.config,
  ...
  program
    ...
    .action(async (scriptName: string, cmd) => {
      ...
      const passThroughArgs = program.args.slice(
        program.args.indexOf(scriptName) + 1,
      );
...
const getNextRequestOffset = (
  ...
  const idMatchOffset = leftRequestsToRun.findIndex(
    (req) => req._id.trim() === nextRequestIdOrName.trim(),
  );
  if (idMatchOffset >= 0) {

...
describe('writeFileWithCliOptions', () => {
  ...
  it('should write to output file', async () => {
    ...
    await expect(promise).resolves.toBe(`${process.cwd()}/file.yaml`);
  });
  ...
  it('should return an error if make directory fails', async () => {
    ...
    await expect(promise).rejects.toThrow(new InsoError(`Failed to write to "${process.cwd()}/file.yaml"`, error));
  });

...
describe("inso dev bundle", () => {
  describe("exit codes are consistent", () => {
    it.each(shouldReturnSuccessCode)(
      "exit code should be 0: %p",
      async (input) => {
    ...
    it.each(shouldReturnErrorCode)(
      "exit code should be 1: %p",
      async (input) => {
        const result = await runCliFromRoot(input);
        if (result.code !== 1) {
          console.log(result);
  ...
  describe("response and timeline has scripting effects", () => {
    it("console log appears in timeline", async () => {
      ...
      expect(result.stdout).toContain(
        "Preparing request to http://127.0.0.1:4010/",
      );
    ...
    it("send request with settings enabled (by testing followRedirects)", async () => {
      ...
      expect(result.stdout).not.toContain(
        "Issue another request to this URL: 'https://insomnia.rest/'",
      );
    });
    ...
    it("run collection: run requests in specified order", async () => {
      ...
      expect(result.code).toBe(0);
      const firstReqLogPosition = result.stdout.indexOf(
        "Running request: 2 req_6063adcdab5b409e9b4f00f47322df4a",
      );
      const secondReqLogPosition = result.stdout.indexOf(
        "Running request: 1 req_3fd28aabbb18447abab1f45e6ee4bdc1",
      );
...
const packagedSuccessCodes = shouldReturnSuccessCode.map((x) =>
  x.replace(
    "$PWD/packages/insomnia-inso/bin/inso",
    "$PWD/packages/insomnia-inso/binaries/inso",
  ),
);

...
export const getRuleSetFileFromFolderByFilename = async (filePath: string) => {
  try {
    ...
    const rulesetFileName = filesInSpecFolder.find((file) =>
      file.startsWith(".spectral"),
    );
    if (rulesetFileName) {
...
export async function lintSpecification({
  ...
  if (results.some((r) => r.severity === DiagnosticSeverity.Error)) {
    logger.fatal(
      `${results.filter((r) => r.severity === DiagnosticSeverity.Error).length} lint errors found. \n`,
    );
  }
  if (results.some((r) => r.severity === DiagnosticSeverity.Warning)) {
    logger.warn(
      `${results.filter((r) => r.severity === DiagnosticSeverity.Warning).length} lint warnings found. \n`,
    );
  }
  results.forEach((r) =>
    logger.log(
      `${r.range.start.line + 1}:${r.range.start.character + 1} - ${DiagnosticSeverity[r.severity]} - ${r.code} - ${r.message} - ${r.path.join(".")}`,
    ),
  );

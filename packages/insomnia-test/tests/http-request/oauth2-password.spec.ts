...
test("Verify Resource Owner Password Credentials grant fetches tokens with no popup and authorizes the sent request", async ({
  user,
  insomnia,
}) => {
  ...
  expect(lastTokenRequest.grantType).toBe("password");

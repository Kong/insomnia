...
test("Import MCP Client by Url", async ({ user }) => {
  const { importFlow, mcpClientFlow, workspaceFlow } = user.flowManager;
  ...
  await importFlow.importMcp(project, MCP_SERVER);

  const mcpClient = await mcpClientFlow.get(mcp);
